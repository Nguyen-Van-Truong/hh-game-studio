/**
 * Authored street furniture on recycled 4175.
 * Lamps + zebra + planters; sky stays blue; shop E still works.
...
const DEBUG_PORT = Number(process.env.HH_CDP_PORT || 9398);
const CHROME =
  process.env.HH_CHROME ||
  "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe";
const OUT = join(import.meta.dirname, "J5-STREET-PROPS-2026-09-03.txt");
const SHOT_SPAWN = join(import.meta.dirname, "j5-3d-street-props.png");
const SHOT = join(import.meta.dirname, "j5-3d-street-crosswalk.png");
...
const SNAP = `(() => {
  ...
  const props = [...document.querySelectorAll('[data-testid="play-street-props"] li')].map((el) => ({
    ...
    glow: el.getAttribute("data-glow"),
...
const profile = mkdtempSync(join(tmpdir(), "hh-world-j5-props-"));
const chrome = spawn(
  CHROME,
  [
    "--headless=new",
...
try {
  ...
  const lampLeft = bandStats(
    spawnPng.pixels,
    spawnPng.width,
    spawnPng.height,
    canvasBox.left + canvasBox.w * 0.08,
    canvasBox.top + canvasBox.h * 0.28,
    canvasBox.left + canvasBox.w * 0.22,
    canvasBox.top + canvasBox.h * 0.72,
  );
  const lampRight = bandStats(
    spawnPng.pixels,
    spawnPng.width,
    spawnPng.height,
    canvasBox.left + canvasBox.w * 0.78,
    canvasBox.top + canvasBox.h * 0.28,
    canvasBox.left + canvasBox.w * 0.92,
    canvasBox.top + canvasBox.h * 0.72,
  );
  ...
  const lampsOk =
    ...
    Number(walkDom.lampGlows) >= 2 &&
    walkDom.props.filter((row) => row.kind === "lamp").length >= 12 &&
    (lampLeft.darkRatio > 0.08 || lampRight.darkRatio > 0.08);
  const zebraOk =
    Number(walkDom.crosswalks) === 1 &&
    walkDom.props.some((row) => row.kind === "crosswalk") &&
    zebraBand.whiteRatio >= 0.035 &&
    zebraBand.darkRatio >= 0.08 &&
    zebraBand.uniqueQ16 >= 12;
  const planterOk = Number(walkDom.planters) >= 2 && Number(walkDom.planters) <= 6;
  const shopOk =
  ...
  report = {
    run_id: "HH3D-J5-20260903-ASIA-SAIGON-22",
    player: PLAYER,
    verdict:
      skyOk && lampsOk && zebraOk && planterOk && shopOk && honestyOk
        ? "J5_STREET_PROPS_OK"
        : "J5_REWORK",
    not_plan_pass: true,
    not_gate_u1: true,
...
if (report.verdict !== "J5_STREET_PROPS_OK") {
...
console.log(
  report.verdict,
  report.run_id,
  `lamps=${report.walkDom?.lamps}`,
  `crosswalks=${report.walkDom?.crosswalks}`,
  `planters=${report.walkDom?.planters}`,
  `blue=${report.skyBand?.blueRatio}`,
  `zebraWhite=${report.zebraBand?.whiteRatio}`,
  `shop=${report.shopDom?.shopPanel}`,
);
