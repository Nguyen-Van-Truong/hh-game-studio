/**
 * Isolated guest B on recycled 4175. Leftover Menu row textContent
 * must have a · gap before “không trên phố” (not Shared PCkhông).
 * Leftover Shared PC shelf body must not read as a street
 * “Player-opened shop”. Nearby/E, viewing, leftover banners hold.
...
const OUT = join(import.meta.dirname, "J5-LEFTOVER-GAP-2026-09-04.txt");
const SHOT = join(import.meta.dirname, "j5-3d-leftover-gap.png");
const PORT = Number(process.env.HH_CDP_PORT_B || 9791);
const RUN_ID = "HH3D-J5-20260904-ASIA-SAIGON-44";
...
function launchChrome(port, url) {
  const profile = mkdtempSync(join(tmpdir(), `hh-world-j5-leftover-gap-${port}-`));
  return spawn(
    CHROME,
    [
      "--headless=new",
...
try {
  ...
  const ok = gapOk && descOk && !spawnEStolen;
  report = {
    run_id: RUN_ID,
    player: PLAYER,
    verdict: ok ? "J5_LEFTOVER_GAP_OK" : "J5_REWORK",
    not_plan_pass: true,
    reminted: true,
    liveJs: js.path,
    liveJsLen: js.len,
  ...
  if (!ok) {
    ...
    console.log([report.verdict, js.path, `rows=${leftoverRows.length}`, `jam=${jammed.length}`].join(" "));
  }
} catch (err) {
  writeFileSync(
    OUT,
