/**
 * Authored Harbor Walk / Steps East crossing: zebra + stop-line + curb
 * return. Walk Harbor then east; chip→Steps East; minimap agrees after
 * mount; lantern E still shop-lantern-fish. NOT_PLAN_PASS. Not GATE-U1.
 * Not OSM / WAN. Not a fifth street.
 */
...
const OUT = join(import.meta.dirname, "J5-INTERSECTION-2026-09-04.txt");
const SHOT = join(import.meta.dirname, "j5-3d-intersection-corner.png");
const SHOT_STEPS = join(import.meta.dirname, "j5-3d-intersection-steps.png");
const PORTS = { a: Number(process.env.HH_CDP_PORT_A || 9581), b: 9582 };
const RUN_ID = "HH3D-J5-20260904-ASIA-SAIGON-20";
...
function launchChrome(port, url) {
  const profile = mkdtempSync(join(tmpdir(), `hh-world-j5-corner-${port}-`));
  return spawn(
    CHROME,
    [
      "--headless=new",
...
try {
  ...
  await sleep(180);
  const atCorner = await evalExpr(a.ws, 120, SNAP);
  const cornerShot = await cdp(a.ws, 121, "Page.captureScreenshot", { format: "png" });
  const cornerBuf = Buffer.from(cornerShot.data, "base64");
  writeFileSync(SHOT, cornerBuf);
  const png = decodePngRgba(cornerBuf);
  const floor = bandStats(png.pixels, png.width, png.height, 280, 360, 1000, 700);
  const mid = bandStats(png.pixels, png.width, png.height, 360, 280, 920, 580);

  await keyHold(a.ws, 130, "d", "KeyD", 68, 850);
  await sleep(80);
  await keyHold(a.ws, 140, "w", "KeyW", 87, 4500);
  await sleep(200);
  const onSteps = await waitSnap(
    a.ws,
    150,
    (s) =>
      (s.chipText === "Steps East" || s.streetName === "Steps East") &&
      (s.minimap?.present === true || s.minimapDefer === "live"),
    16,
    220,
  );
  const stepsSnap = onSteps.snap;
  const stepsShot = await cdp(a.ws, onSteps.nextId, "Page.captureScreenshot", { format: "png" });
  const stepsBuf = Buffer.from(stepsShot.data, "base64");
  writeFileSync(SHOT_STEPS, stepsBuf);

  await keyHold(a.ws, 180, "a", "KeyA", 65, 850);
  await sleep(60);
  await keyHold(a.ws, 190, "a", "KeyA", 65, 850);
  await sleep(60);
  await keyHold(a.ws, 200, "w", "KeyW", 87, 5000);
  await sleep(200);
  const atLantern = await evalExpr(a.ws, 210, SNAP);
  await keyHold(a.ws, 211, "e", "KeyE", 69, 180);
  await sleep(350);
  const shopOpen = await evalExpr(a.ws, 220, SNAP);
  await evalExpr(a.ws, 221, `document.querySelector('[data-testid="close-shop"]')?.click(); true`);
  await sleep(250);
  const shopClosed = await evalExpr(a.ws, 222, SNAP);
  ...
  const eastM = (stepsSnap.lon - spawn.lon) * metersPerDegLon((stepsSnap.lat + spawn.lat) / 2);
  const northM = (stepsSnap.lat - spawn.lat) * M_PER_DEG_LAT;
  const cornerDom =
    ...
    spawn.curbReturnIds.some((id) => /curb-return-harbor-steps/.test(id));
  const harborChip =
    (onHarbor.chipText === "Harbor Walk" || onHarbor.streetName === "Harbor Walk") &&
    onHarbor.chipOnScreen &&
    onHarbor.streetHud === "named-chip";
  const stepsChip =
    (stepsSnap.chipText === "Steps East" || stepsSnap.streetName === "Steps East") &&
    stepsSnap.chipOnScreen &&
    stepsSnap.chipText !== onHarbor.chipText;
  const walkedOff =
    eastM > 6 &&
    northM > 10 &&
    stepsSnap.insideRing === "0" &&
    stepsSnap.insideAabb === "0" &&
    atCorner.insideRing === "0";
  const miniAfter =
    stepsSnap.minimap?.present === true &&
    stepsSnap.minimap?.kind === "authored-hud-lanes" &&
    stepsSnap.minimap?.official === 2 &&
    stepsSnap.minimap?.inner >= 2 &&
    stepsSnap.minimap?.active === "Steps East" &&
    stepsSnap.minimap?.highlight === "1" &&
    stepsSnap.minimap?.activeRole === "inner" &&
    stepsSnap.chipText === stepsSnap.minimap?.active &&
    stepsSnap.engine === "r3f" &&
    stepsSnap.worldMapPresent === false;
  const stripesOk = floor.whiteRatio >= 0.04 || mid.whiteRatio >= 0.04;
  const curbOk = floor.curbRatio >= 0.02 || mid.curbRatio >= 0.015 || floor.darkRatio >= 0.08;
  const lanternOk =
  ...
  report = {
    run_id: RUN_ID,
    player: PLAYER,
    verdict: ok ? "J5_INTERSECTION_OK" : "J5_REWORK",
    not_plan_pass: true,
    not_gate_u1: true,
    not_osm: true,
    not_wan: true,
    ...
    chipSteps: stepsSnap.chipText,
    miniSteps: stepsSnap.minimap?.active,
    miniHl: stepsSnap.minimap?.highlight,
    ...
    headingCorner: atCorner.heading,
    spawn,
    onHarbor,
    atCorner,
    onSteps: stepsSnap,
    ...
    shots: { corner: SHOT, steps: SHOT_STEPS },
    hashCorner: createHash("sha256").update(cornerBuf).digest("hex").slice(0, 16),
...
if (report.verdict !== "J5_INTERSECTION_OK") {
...
console.log(
  ...
  `mini=${report.miniSteps}`,
  `white=${report.floor?.whiteRatio}`,
  `curb=${report.floor?.curbRatio}`,
  `east=${report.eastM}`,
