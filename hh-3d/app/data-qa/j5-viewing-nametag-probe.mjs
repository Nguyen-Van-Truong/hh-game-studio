/**
 * Isolated seats A+B on live 4175 (no recycle). After accepted Online,
 * B sees A's street body. A walks to lantern sidewalk and presses E
 * (not Menu open-shop). B must lose that body, lose remote-avatar-a,
 * and see “Đang xem cửa hàng”. Close shelf: body and nametag return.
...
const OUT = join(import.meta.dirname, "J5-VIEWING-NAMETAG-2026-09-04.txt");
const SHOT = join(import.meta.dirname, "j5-3d-viewing-nametag.png");
const PORTS = { a: Number(process.env.HH_CDP_PORT_A || 9788), b: Number(process.env.HH_CDP_PORT_B || 9789) };
const RUN_ID = "HH3D-J5-20260904-ASIA-SAIGON-48";
const LANTERN = "shop-lantern-fish";
const STREET = new Set([
  LANTERN,
  "shop-local-mtmh45qxehxhvb",
  "shop-local-mtmrbdjffjdkg8",
  "shop-local-mtmrq33hoq4phb",
]);
...
const SNAP = `(() => {
  ...
  const labels = document.querySelector('[data-testid="play-remote-labels"]');
  const labelStyle = labelA ? window.getComputedStyle(labelA) : null;
  ...
  return {
    ...
    labelVisible: Boolean(labelA) && labelStyle?.display !== "none" && labelStyle?.visibility !== "hidden",
    labelCount: Number(labels?.getAttribute("data-count") ?? "-1"),
    viewingA: viewingA
      ? {
...
function launchChrome(port, url) {
  const profile = mkdtempSync(join(tmpdir(), `hh-world-j5-viewing-nametag-${port}-`));
  return spawn(
    CHROME,
    [
      "--headless=new",
...
try {
  ...
  const menuA = await evalExpr(a.ws, nextA, SNAP);
  nextA += 1;
  if (menuA.menuOpen) {
    await evalExpr(a.ws, nextA, `document.querySelector('[data-testid="play-menu-toggle"]')?.click(); true`);
    nextA += 1;
  ...
  nextA += 1;
  await keyHold(a.ws, nextA, "a", "KeyA", 65, 700);
  nextA += 2;

  let atLantern = null;
  for (const hold of [4500, 3500, 3500, 4000]) {
    const now = await evalExpr(a.ws, nextA, SNAP);
    nextA += 1;
    if (STREET.has(now.nearbyShop)) {
      atLantern = now;
      break;
    }
    await keyHold(a.ws, nextA, "w", "KeyW", 87, hold);
    nextA += 2;
    await sleep(80);
    const afterChunk = await evalExpr(a.ws, nextA, SNAP);
    nextA += 1;
    if (STREET.has(afterChunk.nearbyShop)) {
      atLantern = afterChunk;
      break;
    }
    await keyHold(a.ws, nextA, "d", "KeyD", 68, 180);
    nextA += 2;
  }
  if (!atLantern) {
    atLantern = await evalExpr(a.ws, nextA, SNAP);
    nextA += 1;
  }

  const openedId = STREET.has(atLantern?.nearbyShop) ? atLantern.nearbyShop : "";
  await keyHold(a.ws, nextA, "e", "KeyE", 69, 180);
  nextA += 2;
  const aOpen = await waitSnap(
    a.ws,
    nextA,
    (s) => Boolean(s.shopPanelId) && STREET.has(s.shopPanelId) && s.shopLeftover !== "1",
    20,
    180,
  );
  nextA = aOpen.nextId;
  const panelId = aOpen.snap.shopPanelId;
  ...
  const viewingOk =
    ...
    during.bodyA === false &&
    during.labelA === false &&
    during.labelVisible !== true &&
    during.viewingA?.shop === panelId &&
    during.viewingA?.street === "0" &&
    during.rowA?.street === "0" &&
    during.rowA?.shop === panelId &&
  ...
  report = {
    run_id: RUN_ID,
    player: PLAYER,
    verdict: ok ? "J5_VIEWING_NAMETAG_OK" : "J5_REWORK",
    not_plan_pass: true,
    not_gate_u1: true,
    not_m2: true,
    reminted: true,
    ...
    backOk,
    honestyOk,
    nearbyAtE: atLantern?.nearbyShop ?? "",
    before: {
      bodyA: before.bodyA,
  ...
  if (!ok) {
    ...
    console.log(
      [
        report.verdict,
        js.path,
        `via=e nearby=${atLantern?.nearbyShop ?? ""}`,
        `viewing=${during.viewingA?.shop}`,
        `street ${before.streetCount}->${during.streetCount}->${after.streetCount}`,
      ].join(" "),
    );
  }
} catch (err) {
