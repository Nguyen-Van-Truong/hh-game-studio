/**
 * Authored Harbor Walk / Steps West crossing: zebra + stop-line + curb
 * return. Walk Harbor then west; chip→Steps West; minimap agrees after
 * mount; east mouth still striped; lantern E still shop-lantern-fish.
 * NOT_PLAN_PASS. Not GATE-U1. Not OSM / WAN. Not a fifth street.
 */
...
const OUT = join(import.meta.dirname, "J5-WEST-XING-2026-09-04.txt");
const SHOT_WEST = join(import.meta.dirname, "j5-3d-west-xing-west.png");
const SHOT_EAST = join(import.meta.dirname, "j5-3d-west-xing-east.png");
const PORTS = { a: Number(process.env.HH_CDP_PORT_A || 9683), b: 9684 };
const RUN_ID = "HH3D-J5-20260904-ASIA-SAIGON-28";
...
function launchChrome(port, url) {
  const profile = mkdtempSync(join(tmpdir(), `hh-world-j5-west-xing-${port}-`));
  return spawn(
    CHROME,
    [
      "--headless=new",
...
try {
  ...
  await keyHold(a.ws, 110, "w", "KeyW", 87, 11800);
  await sleep(180);
  await keyHold(a.ws, 130, "a", "KeyA", 65, 850);
  await sleep(80);
  await keyHold(a.ws, 140, "w", "KeyW", 87, 11000);
  await sleep(200);
  let onWest = await waitSnap(
    a.ws,
    150,
    (s) =>
      (s.chipText === "Steps West" || s.streetName === "Steps West") &&
      (s.minimap?.present === true || s.minimapDefer === "live"),
    12,
    220,
  );
  if (onWest.snap.chipText !== "Steps West" && onWest.snap.streetName !== "Steps West") {
    await keyHold(a.ws, 170, "a", "KeyA", 65, 200);
    await sleep(60);
    await keyHold(a.ws, 172, "w", "KeyW", 87, 4500);
    await sleep(200);
    onWest = await waitSnap(
      a.ws,
      176,
      (s) => s.chipText === "Steps West" || s.streetName === "Steps West",
      12,
      220,
    );
  }
  if (onWest.snap.chipText === "Steps West" || onWest.snap.streetName === "Steps West") {
    await keyHold(a.ws, 190, "d", "KeyD", 68, 850);
    await sleep(80);
    await keyHold(a.ws, 192, "w", "KeyW", 87, 2800);
    await sleep(160);
    onWest = await waitSnap(
      a.ws,
      196,
  ...
  const westSnap = onWest.snap;
  const westShot = await cdp(a.ws, onWest.nextId, "Page.captureScreenshot", { format: "png" });
  const westBuf = Buffer.from(westShot.data, "base64");
  writeFileSync(SHOT_WEST, westBuf);
  const westPng = decodePngRgba(westBuf);
  const westFloor = bandStats(westPng.pixels, westPng.width, westPng.height, 280, 360, 1000, 700);
  const westMid = bandStats(westPng.pixels, westPng.width, westPng.height, 360, 280, 920, 580);

  await keyHold(a.ws, 210, "d", "KeyD", 68, 850);
  await sleep(80);
  await keyHold(a.ws, 212, "w", "KeyW", 87, 14000);
  await sleep(200);
  let onEast = await waitSnap(
    a.ws,
    220,
    (s) => s.chipText === "Steps East" || s.streetName === "Steps East",
    10,
    200,
  );
  if (onEast.snap.chipText === "Steps East" || onEast.snap.streetName === "Steps East") {
    await keyHold(a.ws, 232, "a", "KeyA", 65, 850);
    await sleep(160);
    onEast = await waitSnap(
      a.ws,
      234,
      (s) => s.chipText === "Steps East" || s.streetName === "Steps East" || s.eastZebra,
      6,
      180,
    );
  }
  const eastSnap = onEast.snap;
  const eastShot = await cdp(a.ws, onEast.nextId, "Page.captureScreenshot", { format: "png" });
  const eastBuf = Buffer.from(eastShot.data, "base64");
  writeFileSync(SHOT_EAST, eastBuf);
  const eastPng = decodePngRgba(eastBuf);
  const eastFloor = bandStats(eastPng.pixels, eastPng.width, eastPng.height, 280, 360, 1000, 700);
  const eastMid = bandStats(eastPng.pixels, eastPng.width, eastPng.height, 360, 280, 920, 580);

  await keyHold(a.ws, 240, "a", "KeyA", 65, 850);
  await sleep(60);
  await keyHold(a.ws, 242, "w", "KeyW", 87, 5000);
  await sleep(80);
  await keyHold(a.ws, 244, "d", "KeyD", 68, 850);
  await sleep(60);
  await keyHold(a.ws, 246, "w", "KeyW", 87, 4500);
  await sleep(200);
  const atLantern = await evalExpr(a.ws, 250, SNAP);
  await keyHold(a.ws, 251, "e", "KeyE", 69, 180);
  await sleep(350);
  const shopOpen = await evalExpr(a.ws, 260, SNAP);
  await evalExpr(a.ws, 261, `document.querySelector('[data-testid="close-shop"]')?.click(); true`);
  await sleep(250);
  const shopClosed = await evalExpr(a.ws, 262, SNAP);
  ...
  const westM = (westSnap.lon - spawn.lon) * metersPerDegLon((westSnap.lat + spawn.lat) / 2);
  const northM = (westSnap.lat - spawn.lat) * M_PER_DEG_LAT;
  const cornerDom =
    ...
    spawn.curbReturnIds.some((id) => /curb-return-harbor-steps-west/.test(id)) &&
    spawn.curbReturnIds.some((id) => /curb-return-steps-west/.test(id));
  const harborChip =
    (onHarbor.chipText === "Harbor Walk" || onHarbor.streetName === "Harbor Walk") &&
    onHarbor.chipOnScreen &&
    onHarbor.streetHud === "named-chip";
  const stepsChip =
    (westSnap.chipText === "Steps West" || westSnap.streetName === "Steps West") &&
    westSnap.chipOnScreen &&
    westSnap.chipText !== onHarbor.chipText;
  const walkedOff =
    westM < -6 &&
    northM > 10 &&
    westSnap.insideRing === "0" &&
    westSnap.insideAabb === "0";
  const miniAfter =
    westSnap.minimap?.present === true &&
    westSnap.minimap?.kind === "authored-hud-lanes" &&
    westSnap.minimap?.official === 2 &&
    westSnap.minimap?.inner >= 2 &&
    westSnap.minimap?.active === "Steps West" &&
    westSnap.minimap?.highlight === "1" &&
    westSnap.minimap?.activeRole === "inner" &&
    westSnap.chipText === westSnap.minimap?.active &&
    westSnap.engine === "r3f" &&
    westSnap.worldMapPresent === false;
  const westStripesOk = westFloor.whiteRatio >= 0.04 || westMid.whiteRatio >= 0.04;
  const westCurbOk = westFloor.curbRatio >= 0.015 || westMid.curbRatio >= 0.012 || westFloor.darkRatio >= 0.08;
  const eastStillStriped =
    westSnap.eastZebra &&
    eastSnap.eastZebra &&
    eastSnap.crosswalks >= 3 &&
    (eastFloor.whiteRatio >= 0.015 ||
      eastMid.whiteRatio >= 0.015 ||
      eastSnap.chipText === "Steps East" ||
      eastSnap.streetName === "Steps East");
  const lanternOk =
  ...
  report = {
    run_id: RUN_ID,
    player: PLAYER,
    verdict: ok ? "J5_WEST_XING_OK" : "J5_REWORK",
    not_plan_pass: true,
    not_gate_u1: true,
    not_osm: true,
    not_wan: true,
    ...
    chipWest: westSnap.chipText,
    miniWest: westSnap.minimap?.active,
    miniHl: westSnap.minimap?.highlight,
    chipEast: eastSnap.chipText,
    ...
    westM: Number(westM.toFixed(2)),
    northM: Number(northM.toFixed(2)),
    headingWest: westSnap.heading,
    spawn,
    onHarbor,
    onWest: westSnap,
    onEast: eastSnap,
    ...
    shots: { west: SHOT_WEST, east: SHOT_EAST },
    hashWest: createHash("sha256").update(westBuf).digest("hex").slice(0, 16),
    hashEast: createHash("sha256").update(eastBuf).digest("hex").slice(0, 16),
  };
  a.ws.close();
  b.ws.close();
} catch (err) {
...
if (report.verdict !== "J5_WEST_XING_OK") {
...
console.log(
  ...
  `chipWest=${report.chipWest}`,
  `mini=${report.miniWest}`,
  `white=${report.westFloor?.whiteRatio}`,
  `west=${report.westM}`,
  `eastStill=${report.eastStillStriped}`,
