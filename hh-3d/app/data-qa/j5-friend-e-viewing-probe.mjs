 ...
 * (not Menu open-shop). B must lose that body and see “Đang xem cửa
 * hàng” + shop-lantern-fish. Close shelf: body returns.
...
const OUT = join(import.meta.dirname, "J5-FRIEND-E-VIEWING-2026-09-04.txt");
const SHOT = join(import.meta.dirname, "j5-3d-friend-e-viewing.png");
const PORTS = { a: Number(process.env.HH_CDP_PORT_A || 9785), b: Number(process.env.HH_CDP_PORT_B || 9787) };
const RUN_ID = "HH3D-J5-20260904-ASIA-SAIGON-47";
...
function launchChrome(port, url) {
  const profile = mkdtempSync(join(tmpdir(), `hh-world-j5-friend-e-viewing-${port}-`));
  return spawn(
    CHROME,
    [
      "--headless=new",
...
try {
  ...
  report = {
    run_id: RUN_ID,
    player: PLAYER,
    verdict: ok ? "J5_FRIEND_E_VIEWING_OK" : "J5_REWORK",
    not_plan_pass: true,
    not_gate_u1: true,
    not_m2: true,
    reminted: false,
