...
int wmain(void) {
    wchar_t work[256], control[256], logpath[1400], mode[32], root[1400], events[1400], regpath[512], regcontrol[512];
    ...
    FILE *log = NULL; LONG reg_error; DWORD error, regsize, regtype; char regbytes[32]; int code = 95, drop, stopping;
    if (!envw(L"S46_WORK", work, 256) || !envw(L"S46_CONTROL", control, 256) ||
        ...
        !envw(L"S46_REGISTRY", regpath, 512) || !envw(L"S46_REG_CONTROL", regcontrol, 512) ||
        wcsstr(logpath, L"gt02-s46-native-") == NULL) return 91;
    ...
    reg_error = RegOpenKeyExW(HKEY_USERS, regcontrol, REG_OPTION_OPEN_LINK, KEY_QUERY_VALUE | KEY_SET_VALUE | KEY_WOW64_64KEY, &reg);
    if (reg_error != ERROR_SUCCESS) goto done;
    regsize = sizeof(regbytes);
    if (RegQueryValueExW(reg, L"State", NULL, &regtype, (LPBYTE)regbytes, &regsize) != ERROR_SUCCESS ||
        regtype != REG_BINARY || regsize != 7 || memcmp(regbytes, "CONTROL", 7) != 0) goto done;
    if (RegSetValueExW(reg, L"State", 0, REG_BINARY, (const BYTE *)"CHILD_OK", 8) != ERROR_SUCCESS) goto done;
    regsize = sizeof(regbytes);
    if (RegQueryValueExW(reg, L"State", NULL, &regtype, (LPBYTE)regbytes, &regsize) != ERROR_SUCCESS ||
        regtype != REG_BINARY || regsize != 8 || memcmp(regbytes, "CHILD_OK", 8) != 0) goto done;
    RegCloseKey(reg); reg = NULL;
    fputs("\"registry_positive_control_read_write\":true,", log);
