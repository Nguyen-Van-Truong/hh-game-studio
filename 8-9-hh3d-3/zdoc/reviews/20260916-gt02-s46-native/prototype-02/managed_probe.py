...
def run_case(executable, scratch, package, owner, mode, user):
...
def main():
    ...
    created = False; base = mapping = managed = None; owned_ids = []
            ...
            try:
                for mode in ('create','replace_stop','drop_reply'):
                    if mode == 'replace_stop':
                        managed = ManagedFixtureOwner.reopen(owned_ids[0],project_id='project.fixture',now_ms=epoch_ms())
                        ...
                        owned_ids.append(managed.storage_id)
                    record = run_case(image,scratch,package,managed,mode,user)
    ...
    out['diagnostic_complete'] = bool(out.get('marker') and out.get('owned_registry_leaves_removed') == 2 and all(out[key] for key in (
