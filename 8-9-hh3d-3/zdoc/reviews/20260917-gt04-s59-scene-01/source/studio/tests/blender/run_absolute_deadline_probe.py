...
def checked_observation(request, receipt):
    """Bind the received receipt without rebuilding its native revision.

    IPC uses JCS, which changes integral floats to integers. The native scene
    revision was computed before that conversion and remains an opaque token.
    Command identity/digest, completion and operation postconditions are checked
    independently; semantic scene checks belong to the individual probe case.
    """
    from studio.host.blender.durable_session import queue
    from studio.protocol.core import canonical_bytes, parse_json

    # Match the command Blender receives after the same IPC wire conversion.
    transmitted = queue.parse(queue.c.canonical(parse_json(canonical_bytes(request))))
    if type(receipt) is not dict or set(receipt) != {
        "command_id", "command_digest", "state", "public_ack", "result",
    } or receipt["command_id"] != transmitted["command_id"] \
            or receipt["command_digest"] != queue.c.digest(transmitted) \
            or receipt["state"] != "COMPLETED" or receipt["public_ack"] is not False:
        raise ValueError("native receipt binding mismatch")
    result = receipt["result"]
    if transmitted["operation"] == "scene.inspect":
        observed = result
    elif transmitted["operation"] == "mesh.create_box":
        if type(result) is not dict or set(result) != {
            "operation", "before_revision", "after", "native_operator_finished", "status", "public_ack",
        } or result["operation"] != transmitted["operation"] \
                or result["before_revision"] != transmitted["expected_revision"] \
                or result["native_operator_finished"] is not True \
                or result["status"] != "INTERNAL_UI_READBACK" or result["public_ack"] is not False:
            raise ValueError("native create postcondition mismatch")
        observed = result["after"]
    else:
        raise ValueError("unsupported deadline probe observation")
    if type(observed) is not dict or set(observed) != {
        "snapshot", "revision", "context", "public_ack", "undo_supported",
    } or type(observed["revision"]) is not str \
            or re.fullmatch(r"sha256:[0-9a-f]{64}", observed["revision"]) is None \
            or type(observed["snapshot"]) is not dict or type(observed["context"]) is not dict \
            or observed["public_ack"] is not False or observed["undo_supported"] is not True:
        raise ValueError("native observation shape mismatch")
    if transmitted["operation"] != "scene.inspect" and observed["context"] != transmitted["expected_context"]:
        raise ValueError("native context restore mismatch")
    # Strict finite/Unicode/size validation and detached received-wire copy.
    return parse_json(canonical_bytes(observed))
...
def frozen(output, binary, run_id, source_closure_sha256):
    ...
    def evidence(kind, request, result, **extra):
        ...
        events.append({
            ...
            "result_sha256": sha(result_raw), "result_hash_domain": "jcs-received-result-v1",
            "native_revision_recomputed": False, "recorded_at": utc_now(), **extra,
        })

    def inspect(key):
        request = command(key, "scene.inspect")
        receipt = gui.execute(request, timeout=3, deadline_ms=epoch_ms() + 2000)
        observed = checked_observation(request, receipt)
    ...
    try:
        ...
        after = checked_observation(create, created)
