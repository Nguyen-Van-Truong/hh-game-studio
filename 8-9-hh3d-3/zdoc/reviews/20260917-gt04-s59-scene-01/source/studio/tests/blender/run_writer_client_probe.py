"""Frozen external HTTP proof for preview, native edits and protected publication.

Credentials enter the separately owned CLI through gated stdin only. This probe
does not claim live-scene restart durability or accept GT04.
"""
...
CLIENT_CHECKS = frozenset({
    ...
    "create_preview_no_effect", "transform_preview_no_effect", "material_preview_no_effect",
    "undo_preview_no_effect", "redo_preview_no_effect", "preview_repeat_no_id_reservation",
    "create_observed_diff", "transform_observed_diff", "material_observed_diff",
    "undo_observed_diff", "redo_observed_diff", "publication_protected_committed",
    "publication_exact_retry", "publication_common_lookup",
    "publication_capacity_rejected", "capacity_rejection_preserves_live_owner",
    "file_preview_live_preflight", "file_preview_no_id_reservation",
})
NATIVE_CHECKS = frozenset({
    "separate_cli_exit_zero_and_owned_tree_empty", "client_complete_and_scoped",
    "seventeen_native_data_requests_only", "final_owner_observation_matches_client",
    "protected_bundle_matches_public_receipt", "export_actual_exit_and_tree_zero",
    "publication_native_owners_closed",
...
def parser():
    ...
    value.add_argument('--publication-profile', choices=('export.publish','scene.save','checkpoint.save'), default='export.publish')
    return value
...
def client_report(stdout):
    ...
    if type(report) is not dict or report.get("passed") is not True \
            ...
            or report.get("publication_profile") not in ("export.publish", "scene.save", "checkpoint.save"):
...
def client():
    ...
    try:
        ...
        def call(path, body, *, role="writer", port=None):
            ...
            if remaining <= 0 or len(calls) >= 48:
                raise TimeoutError("bounded writer HTTP client exhausted")
            encoded = canonical_bytes(body)
            limit = 25 if body.get('operation') in ('export.publish','scene.save','checkpoint.save') else 5
            connection = http.client.HTTPConnection("127.0.0.1", selected, timeout=min(limit, remaining))
            try:
        ...
        check("writer_discovery_exact", status == 200 and discovery.project_id == config["project_id"]
              ...
              and {cap.operation for cap in discovery.capabilities} == catalog.EDIT_OPERATIONS | {catalog.READ, config['publication_profile']}
        ...
        def request(key, operation, state, arguments=None, *, expired=False):
            ...
            maximum = (catalog.MAX_LEASE_MS if operation in ('export.publish','scene.save','checkpoint.save') else
                       catalog.MAX_UI_MS if write else catalog.MAX_READ_MS)
        ...
        def submit(label, value):
            if value.operation in catalog.EDIT_OPERATIONS:
                from studio.host.blender.client_preview import actual_diff
                status, advisory, preview_wire = call('/v1/preview', value.as_dict())
                check(label + '_preview_no_effect', status == 200
                    and advisory.get('schema') == 'HH-BLENDER-CLIENT-PREVIEW-1'
                    and advisory.get('command_id') == value.command_id and advisory.get('request_digest') == value.digest
                    and advisory.get('no_effect') is True and advisory.get('apply_id_reserved') is False
                    and advisory.get('authority_granted') is False
                    and advisory.get('before', {}).get('revision') == value.expected_revision
                    and advisory.get('source_sha256') == config['source_sha256']
                    and advisory.get('generation') == config['generation'])
                if label == 'create':
                    status, _, repeated = call('/v1/preview', value.as_dict())
                    check('preview_repeat_no_id_reservation', status == 200 and repeated == preview_wire)
            ...
            if value.operation in catalog.EDIT_OPERATIONS:
                check(label + '_observed_diff', post.get('actual_diff') == actual_diff(
                    advisory['before'], scene, operation=value.operation) and post.get('affected_files') == [])
        ...
        export_request = request('Writer.Export', config['publication_profile'], final)
        status, advisory, preview_wire = call('/v1/preview', export_request.as_dict())
        slot = {'scene.save':'fixture', 'checkpoint.save':'checkpoint', 'export.publish':'export'}[config['publication_profile']]
        check('file_preview_live_preflight', status == 200
              and advisory.get('schema') == 'HH-BLENDER-CLIENT-PREVIEW-1'
              and advisory.get('command_id') == export_request.command_id
              and advisory.get('request_digest') == export_request.digest
              and advisory.get('operation') == config['publication_profile']
              and advisory.get('before') == final and advisory.get('staging_slot') == slot+'.blend'
              and advisory.get('affected_files') == ['checkpoint.blend','scene.glb','manifest.json','active.json']
              and advisory.get('requested_diff') == {'kind':'create-immutable-bundle', 'existing_bundle':None,
                  'create_files':['checkpoint.blend','scene.glb','manifest.json','active.json'], 'artifact_hashes':None}
              and advisory.get('source_sha256') == config['source_sha256']
              and advisory.get('generation') == config['generation']
              and advisory.get('no_effect') is True and advisory.get('apply_id_reserved') is False
              and advisory.get('authority_granted') is False and advisory.get('public_ack') is False
              and advisory.get('scene_state_durable') is False)
        status, _, repeated = call('/v1/preview', export_request.as_dict())
        check('file_preview_no_id_reservation', status == 200 and repeated == preview_wire)
        status, exported, export_wire = call('/v1/commands', export_request.as_dict())
        export_response = Response.from_dict(exported)
        publication = export_response.postconditions.get('publication', {})
        receipt = publication.get('receipt', {})
        check('publication_protected_committed', status == 200 and export_response.status.value == 'COMMITTED'
              and export_response.code == 'BLENDER_CLIENT_PUBLICATION'
              and export_response.command_id == export_request.command_id
              and export_response.result_revision == final['revision']
              and export_response.result_hash == sha(canonical_bytes(publication))
              and export_response.postconditions.get('durable_publication') is True
              and export_response.postconditions.get('public_ack') is False
              and export_response.postconditions.get('scene_state_durable') is False
              and publication.get('source_sha256') == config['source_sha256']
              and publication.get('generation') == config['generation']
              and publication.get('native_revision') == final['revision']
              and publication.get('operation') == config['publication_profile']
              and receipt.get('status') == 'COMMITTED' and receipt.get('public_ack') is False
              and set(receipt.get('artifacts', {})) == {'checkpoint.blend', 'scene.glb'})
        observed['publication'] = publication
        status, _, repeated = call('/v1/commands', export_request.as_dict())
        check('publication_exact_retry', status == 200 and repeated == export_wire)
        status, _, repeated = call('/v1/lookup', {**project, 'command_id': export_request.command_id})
        check('publication_common_lookup', status == 200 and repeated == export_wire)
        status, denied, _ = call('/v1/commands', request('Writer.SecondBundle', config['publication_profile'], final).as_dict())
        check('publication_capacity_rejected', status == 200 and denied.get('status') == 'REJECTED'
              and denied.get('code') == 'PUBLICATION_ONE_BUNDLE_CAPACITY'
              and 'publication' not in denied.get('postconditions', {}))
        status, remaining, _ = call('/v1/discovery', project)
        check('capacity_rejection_preserves_live_owner', status == 200
              and {cap['operation'] for cap in remaining['capabilities']} == catalog.EDIT_OPERATIONS | {catalog.READ, config['publication_profile']})
    ...
    report = {"passed": failure is None and checks_complete(rows, CLIENT_CHECKS), "failure": failure,
              ...
              "publication_profile": config.get("publication_profile"),
...
def frozen(output, binary, run_id, closure, profile):
    ...
    gui = server = process = client_job = owner = publisher = None
    ...
    try:
        ...
        publisher = BlenderPublicationOwner.create(output, session=durable, profile=profile)
        owner = BlenderWriterClientOwner.from_session(durable, publications={profile: publisher})
        operations = catalog.EDIT_OPERATIONS | {catalog.READ, profile, "control.lookup", "control.stop"}
        ...
        server = BlenderWriterClientTransport(owner)
        server.start()
        before = gui.channels["data"].sent
        config = {"publication_profile": profile, "project_id": owner.project_id, "port": server.port, "control_port": server.control_port,
        ...
        check("seventeen_native_data_requests_only", gui.channels["data"].sent == before + 17)
        ...
        publication = child['observed']['publication']
        receipt = publisher.lookup_bytes(publication['receipt']['command_id'])
        check('protected_bundle_matches_public_receipt', publication['storage_id'] == publisher.storage_id
            and publication['receipt_sha256'] == sha(receipt) and parse_json(receipt) == publication['receipt']
            and canonical_bytes(manifest['snapshot']) == canonical_bytes(child['observed']['final_inspect']['snapshot'])
            and all({'sha256': sha(data).removeprefix('sha256:'), 'size_bytes': len(data)} ==
                    publication['receipt']['artifacts'][name] for name, data in artifacts.items()))
        export_dir = publisher._job.directory
        export_report = parse_json((export_dir/'host-result.json').read_bytes())
        export_cleanup = publisher._job.retry_cleanup()
        check('export_actual_exit_and_tree_zero', export_report.get('completed') is True
            and export_report.get('snapshot_geometry_bound') is True
            and export_cleanup['actual_process_exit']['exit_code'] == 0
            and export_cleanup['wrapper_exit_code'] == 0 and export_cleanup['job']['active_count'] == 0
            and export_cleanup['job']['zero_observed'] is True and export_cleanup['job']['closed'] is True
            and export_cleanup['threads_drained'] is True and export_cleanup['cleanup_held'] is False)
        _, events = publisher.log.fold([], lambda rows, row: rows+[parse_json(row.event)])
        write_json(output, 'publication.json', {'publication': publication, 'manifest': manifest, 'events': events,
            'export_directory': export_dir.relative_to(output).as_posix(), 'export_cleanup': export_cleanup,
            'roots': {name: getattr(publisher,name).root.name for name in ('files','store','log')}})
    except BaseException as error:
        failure = safe_failure(error)
    finally:
            ...
            for attempt in range(2):
                try:
                    if owner is not None and not owner.sessions._stopped.is_set():
                        publisher.stop() if publisher is not None else owner._durable.stop()
                    server.close()
                    drained = True
                    break
                except Exception as error:
        ...
        if publisher is not None and (server is None or drained):
            for attempt in range(2):
                try:
                    publisher.close()
                    break
                    ...
                    cleanup_errors.append(safe_failure(error))
        record('publication_native_owners_closed', publisher is not None and publisher._closed)
    ...
    report = {"publication_profile": profile, "run_id": run_id, "source_closure_sha256": closure, "finished_at": utc_now(),
...
def native_completion(output, host):
    try:
        ...
        if type(report.get("run_id")) is not str or not report["run_id"] \
                ...
                or report.get("publication_profile") != manifest.get("publication_profile") \
                or report.get("publication_profile") not in ("export.publish", "scene.save", "checkpoint.save") \
        ...
        return report.get("passed") is True and type(report.get("probe_pid")) is int \
            ...
            and child["publication_profile"] == report["publication_profile"] \
...
def unit_patterns(snapshot, suite):
    ...
    patterns = ["test_writer_client_probe.py", "test_client_write_catalog.py", "test_client_writer_session.py",
                ...
                'test_native_preview.py', 'test_client_preview.py', 'test_client_writer_preview.py',
                'test_client_writer_publication.py', 'test_publication_deadline.py', 'test_publication_profiles.py',
                'test_publication_preview.py']
...
def main(argv=None):
    ...
    if args.frozen:
        ...
        if STUDIO != output / "source/studio" or sources(STUDIO) != manifest["files"] \
                ...
                or manifest["run_id"] != args.run_id or manifest["publication_profile"] != args.publication_profile:
            raise ValueError("writer probe requires exact frozen source")
        return frozen(output, args.binary, args.run_id, manifest["source_closure_sha256"], args.publication_profile)
    ...
    write_json(output, "source-closure.json", {"run_id": args.run_id, "recorded_at": utc_now(),
               "files": original, "source_closure_sha256": closure, "publication_profile": args.publication_profile})
    ...
    if host_artifact_passed(output, unit) and unit_ok:
        ...
        host = runner.run_process([
            ...
            "--frozen", "--output", str(output), "--run-id", args.run_id, "--binary", str(binary.resolve()), "--publication-profile", args.publication_profile,
    ...
    result = {"publication_profile": args.publication_profile, "run_id": args.run_id, "source_closure_sha256": closure, "finished_at": utc_now(),
