...
from .glb_preflight import inspect_glb
...
class ExportJob:
    def __init__(self,host,prepared):
    ...
    def run(self):
        ...
        process=job=None;threads=[];failure=None;limits=None
        report={'public_ack':False,'candidate_only':True,'completed':False}
        try:
            need(not self._stop.is_set(),'EXPORT_STOPPED')
            ...
            (self.directory/'launch.json').write_bytes(canonical_bytes({'argv':argv,'source_files':self.host._source,'public_ack':False}))
            process=subprocess.Popen([sys.executable,'-B','-c',HELPER,str(self.directory/'process'),*argv],
                stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,cwd=self.directory,env=env,
                creationflags=subprocess.CREATE_NO_WINDOW)
            job=cli_job.create(process);limits=configure_limits(job)
            (self.directory/'limits.json').write_bytes(canonical_bytes(limits))
            for name,pipe in (('stdout',process.stdout),('stderr',process.stderr)):
                thread=threading.Thread(target=self._drain,args=(name,pipe),daemon=True);threads.append(thread);thread.start()
            need(not self._stop.is_set(),'EXPORT_STOPPED')
            ...
            for thread in threads:thread.join(2);need(not thread.is_alive(),'EXPORT_LOG_DRAIN_UNKNOWN')
            ...
            need(not self._stop.is_set(),'EXPORT_CANCELLED')
            report.update(completed=True,native=native,preflight=preflight,artifact={'name':'output.glb',
            ...
            try:
                if job is None and process is not None:job=cli_job.owner_for_process(process)
                if job is not None:job.close()
                elif process is not None and process.poll() is None:process.kill()
                if process is not None:process.wait(timeout=3)
                for thread in threads:
                    if thread.ident is not None:thread.join(2);need(not thread.is_alive(),'EXPORT_THREAD_DRAIN_UNKNOWN')
                if process is not None:
                    for pipe in (process.stdin,process.stdout,process.stderr):
                        if pipe is not None and not pipe.closed:pipe.close()
                report.update(job=job.snapshot() if job else None,wrapper_exit_code=process.returncode if process else None,
                    limits=limits,actual_process_exit=parse_json((self.directory/'process-exit.json').read_bytes())
                    if (self.directory/'process-exit.json').exists() else None)
            except BaseException as exc:
                report['completed']=False;report['cleanup_held']=True;failure=exc
            (self.directory/'host-result.json').write_bytes(canonical_bytes(report));self.done.set()
