...
class FixtureSelector:
    ...
    def __init__(self, log, store, consumer, *, project_id, initial_revisions=None):
        ...
        with _BIND_LOCK:
            _need(getattr(log, '_fixture_selector', None) is None
                  and getattr(store, '_fixture_selector', None) is None, 'SELECTOR_ALREADY_BOUND')
            log._fixture_selector = store._fixture_selector = self
        try:
    ...
    def close(self):
        with self._mutex:
            self._closed = True
            with _BIND_LOCK:
                for resource in (self.log, self.store):
