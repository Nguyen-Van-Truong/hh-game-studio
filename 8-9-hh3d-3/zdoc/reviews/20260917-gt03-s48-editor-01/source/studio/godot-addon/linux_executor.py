...
LOCK_SHA256 = '2cfb076917b162cb053e98eefc7c97632b6970acb782a892c64a2c91cf134614'
...
def _create_args(name: str, mode: str, tool: Path, snapshot: Path) -> list[str]:
    ...
    return args + ['--entrypoint', '/tool/' + BINARY_NAME, IMAGE_ID, *_command(mode)]


def _validate_inspect(value: dict, *, name: str, container_id: str, mode: str,
                      tool: Path, snapshot: Path, image_environment: list[str]) -> None:
    ...
    _need(config.get('User') == '65532:65532' and config.get('WorkingDir') == '/project'
          and config.get('Entrypoint') == ['/tool/' + BINARY_NAME] and config.get('Cmd') == _command(mode),
          'EXECUTOR_COMMAND')
    _need(host.get('ReadonlyRootfs') is True and host.get('NetworkMode') == 'none'
          ...
          and host.get('Privileged') is False and not host.get('Devices') and not host.get('DeviceRequests')
...
def run(project: Path, *, mode: Literal['parse', 'import'], output: Path, timeout_seconds: int = 20) -> dict:
    ...
    try:
        ...
        args = _create_args(name, mode, binary.parent, snapshot)
        ...
        row, _ = call(['start', '--attach', container_id], 'engine', timeout=timeout_seconds)
        ...
        try:
            if owned and container_id is None:
