...
class SessionTests(unittest.TestCase):
    ...
    def test_catalog_shaped_object_is_not_a_trusted_profile(self):
        profile=SimpleNamespace(**{name:getattr(catalog,name) for name in ('CATALOG_DIGEST','OPERATIONS','MAX_WIRE')})
        with self.assertRaisesRegex(SafetyViolation,'TRUSTED_CATALOG_REQUIRED'):
            BlenderClientSession('blender.test',Path('owned'),owner_deadline_ms=epoch_ms()+10000,catalog_profile=profile)
    def test_trusted_writer_profile_requires_its_exact_digest_and_explicit_grant(self):
        from studio.host.blender import client_write_catalog as writer
        with self.assertRaisesRegex(SafetyViolation,'CATALOG_MISMATCH'):
            BlenderClientSession('blender.test',Path('owned'),owner_deadline_ms=epoch_ms()+10000,catalog_profile=writer)
        session=BlenderClientSession('blender.test',Path('owned'),owner_deadline_ms=epoch_ms()+10000,
            catalog_profile=writer,catalog_digest=writer.CATALOG_DIGEST)
        default=session.issue();grant=session.authenticate('Bearer '+default.bearer)
        self.assertEqual(frozenset(grant.operations),catalog.OPERATIONS)
        self.assertEqual(default.scopes,frozenset())
        explicit=session.issue(operations=frozenset({'mesh.create_box'}))
        session.authorize('Bearer '+explicit.bearer,'mesh.create_box',project_id='blender.test',catalog_digest=writer.CATALOG_DIGEST)
        with self.assertRaisesRegex(SafetyViolation,'INVALID_GRANT'):
            self.session.issue(operations=frozenset({'mesh.create_box'}))
