...
def child(arm, ordinal):
    ...
    helper = compact if arm == 'compact' else original
    ...
    phases = (BASE / 'cost_phases.gd').read_text(encoding='utf-8')
    if arm == 'sham':
...
if __name__ == '__main__':
    ...
    assert arm in ('original', 'compact', 'sham') and ordinal.isdigit() and 1 <= int(ordinal) <= 4
    if mode == ['--child']:
