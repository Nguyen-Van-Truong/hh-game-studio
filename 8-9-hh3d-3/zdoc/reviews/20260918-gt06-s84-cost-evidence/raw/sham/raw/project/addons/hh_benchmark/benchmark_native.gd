...
func _object_probe_tree_item(item: TreeItem, rows: Dictionary, tree_id: String) -> void:
    ...
    var row: Dictionary = rows[str(item.get_instance_id())]
    var tree: Tree = item.get_tree()
    row["owner_tree_id"] = str(tree.get_instance_id())
...
func _object_probe_sample(label: String, force: bool = false) -> void:
    ...
    var owner_tree_item_counts: Dictionary = {}
    ...
    for key: String in rows:
        ...
        if _object_probe_sequence == 0:
            initial_id_classes[key] = class_name_value
        if class_name_value == "TreeItem" and row.has("owner_tree_id"):
            var owner_id: String = str(row["owner_tree_id"])
            owner_tree_item_counts[owner_id] = int(owner_tree_item_counts.get(owner_id, 0)) + 1
        if not _object_probe_previous.has(key):
            ...
            if _object_probe_sequence > 0 or row["class"] in ["Tree", "RichTextLabel"]:
                added.append(row)
    ...
    var point: Dictionary = {"schema_id": "hh-studio.object-attribution-diagnostic",
        ...
        "initial_inventory_ids_omitted": false,
        "initial_id_classes": initial_id_classes,
        "owner_tree_item_counts": owner_tree_item_counts,
