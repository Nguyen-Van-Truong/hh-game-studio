...
def child(arm, ordinal):
    ...
    helper = original if arm == 'original' else compact
    ...
    script += '\n' + probe_script + '\n' + (BASE / 'cost_phases.gd').read_text(encoding='utf-8')
    ...
    helpers = {p.relative_to(ROOT).as_posix(): c.sha(p.read_bytes())
               for p in (Path(__file__), BASE / 'cost_phases.gd', helper)}
    ...
    try:
        ...
        assert all(p['native']['objects'] == observations[0]['native']['objects'] for p in observations)
...
if __name__ == '__main__':
    ...
    assert arm in ('original', 'compact') and ordinal.isdigit() and 1 <= int(ordinal) <= 4
    if mode == ['--child']:
