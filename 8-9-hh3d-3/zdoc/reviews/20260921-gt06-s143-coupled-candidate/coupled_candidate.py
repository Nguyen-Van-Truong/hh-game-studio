...
ROOT = BASE.parents[2]
RUN_ID = 'gt06-s143-coupled-candidate-01'
...
def helper_layout(root, base, run_id):
    ...
    if owned.is_relative_to((root / 'studio').resolve()):
        raise RuntimeError('S143_HELPER_INSIDE_RUNTIME_SCOPE')
    return owned
...
def boundary_screen(original, sample, baseline, gate_rows):
    ...
    if len(gate_rows) == PREFIX_BATCHES:
        raise Boundary('S143_BOUNDARY_CAPTURED')


def check(root=ROOT):
    support_path = root / 'zdoc/reviews/20260919-gt06-s102-observability/preflight.py'
    util = load(support_path, 'S143_support')
    campaign, _, _, sources = util.load_campaign(root)
    util.need(campaign.closure(sources) == CLOSURE, 'S143_SOURCE_DRIFT')
    util.need(util.sha(root / 'studio/tests/replay/benchmark_native.gd') == NATIVE_HASH, 'S143_NATIVE_DRIFT')
    ...
    util.need(util.sha(binary) == lock['gui_sha256'], 'S143_BINARY_DRIFT')
...
def launch():
    ...
    util.need(not run.exists() and not owned_helpers.exists(), 'S143_FRESH_ID_REQUIRED')
    ...
    freeze = {'schema': 'S143.coupled-journal.freeze.1', 'run_id': RUN_ID,
        ...
        'scope': 'Original campaign prefix, unmodified native; stop at original rejection or after batch6 gate'}
    ...
    try:
        ...
        while owner.tick() is None:
            util.need(time.monotonic() - started < OUTER_SECONDS, 'S143_OUTER_LIMIT')
            time.sleep(.1)
        ...
        for name in ('source_unchanged', 'execution_unchanged'):
            if observations.get(name) is not True:
                errors.append((name, RuntimeError('S143_PIN_CHANGED')))
        util.write(run / 'result.json', {'schema': 'S143.coupled-journal.result.1', 'run_id': RUN_ID,
...
def child(run):
    ...
    util = load(helpers / 'support.py', 'S143_support')
    root = Path(freeze['original_root'])
    util.need(Path(__file__).resolve() == helpers / 'coupled_journal.py', 'S143_COPIED_CHILD_REQUIRED')
    util.need(not helpers.resolve().is_relative_to((root / 'studio').resolve()), 'S143_HELPER_INSIDE_RUNTIME_SCOPE')
    execution = json.loads((run / 'execution-source-files.json').read_bytes())
    util.need(all(util.sha(root / p) == h for p, h in execution.items()), 'S143_EXECUTION_DRIFT')
    campaign, _, _, sources = util.load_campaign(root)
    util.need(campaign.closure(sources) == CLOSURE, 'S143_SOURCE_DRIFT')
    ...
    with patch.object(campaign, 'screen_sample', screen):
        try:
            ...
            errors.append(('unexpected_full_run', RuntimeError('S143_BOUNDARY_NOT_REACHED')))
    ...
    util.write(run / 'diagnostic-summary.json', {'schema': 'S143.coupled-candidate.summary.1',
