"""Owned bounded native custody probe, including outer process-tree evidence."""
import ctypes as C
...
for name in ('registry_probe.py','boundary_child.c','capture.py'):
...
host=runner.run_process([sys.executable,'-B',str(BASE/'registry_probe.py'),sys.argv[2]],
    cwd=PRODUCT/'studio',output=out,timeout=100,label='registry-native')
