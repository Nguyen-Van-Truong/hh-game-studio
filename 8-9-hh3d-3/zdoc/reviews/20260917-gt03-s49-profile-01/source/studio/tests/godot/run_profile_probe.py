...
def evaluate(executor_result, stdout, stderr, bundle, comparator):
    ...
    lifecycle=(executor_result.get('diagnostic_process_clean') is True
        ...
        and host.get('readers_stopped') is True and host.get('stream_reader_eof')==[True,True]
        and host.get('stream_reader_errors')==[None,None]
        and not any(host.get(k) for k in ('timed_out','stream_cap_exceeded','host_error','evidence_write_error')))
...
def frozen_cases(output):
    ...
    result={'passed':len(rows)==3 and all(row['passed'] for row in rows),'cases':rows,
