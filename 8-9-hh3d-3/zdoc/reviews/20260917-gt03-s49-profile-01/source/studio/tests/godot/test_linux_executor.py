...
class PolicyTests(unittest.TestCase):
    ...
    def test_failed_stream_reader_is_never_clean_even_with_zero_process_exit(self):
        ...
        class Owner:
            ...
            def close(self): pass
            def snapshot(self): return {'closed':True,'zero_observed':True,'tainted':False,'handle_retained':False,'active_count':0}
...
class AdmissionTests(unittest.TestCase):
    ...
    def test_unknown_context_container_denies_without_removal(self):
        ...
        def call(args, label, **kwargs):
            calls.append(args)
            return {'exit_code':0,'job_active_count':0,'readers_stopped':True,
                    'stream_reader_eof':[True,True],'stream_reader_errors':[None,None]}, CID.encode()
        with tempfile.TemporaryDirectory() as folder:
            path = Path(folder) / 'owner.json'
    ...
    def test_missing_create_reply_retains_ambiguity_and_never_admits(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            def call(args,label,**kwargs):
                ...
                return {'exit_code':1,'job_active_count':0,'readers_stopped':True,'stderr':'missing.txt',
                        'stream_reader_eof':[True,True],'stream_reader_errors':[None,None]},b''
