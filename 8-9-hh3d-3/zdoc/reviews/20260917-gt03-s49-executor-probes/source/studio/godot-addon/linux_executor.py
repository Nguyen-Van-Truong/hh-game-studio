...
LOCK_SHA256 = 'de8d1c9e31e4326d0608d2a7fa0d69d571bc11ef709c447e924b77b6901a0114'
...
def _supervised_command(mode: str, timeout_seconds: int = 20) -> list[str]:
    ...
    return ['-I', '-B', '-c', SUPERVISOR_BOOTSTRAP, '--signal=TERM', '--kill-after=1s',
            str(max(1, timeout_seconds - 1)) + 's', '/tool/' + BINARY_NAME, *_command(mode)]
...
class _Admission:
    ...
    def __init__(self):
        ...
        self.acquired, self.abandoned = True, status == 0x80
...
def _missing(row: dict, output: Path, selector: str) -> bool:
    return (row.get('exit_code') == 1 and not row.get('timed_out') and not row.get('stream_cap_exceeded')
            and row.get('job_active_count') == 0 and row.get('readers_stopped') is True
            and row.get('stream_reader_eof') == [True, True] and row.get('stream_reader_errors') == [None, None]
            and not row.get('host_error') and not row.get('evidence_write_error')
...
def _command(mode: str) -> list[str]:
    _need(type(mode) is str and mode in ('parse', 'import'), 'EXECUTOR_MODE')
    if mode == 'parse':
...
def _cli(owned, args: list[str], output: Path, label: str, *, timeout: int = 20,
    ...
    try:
        ...
        job = owned._job_for_process(process)
        ...
        row['host_error'] = type(error).__name__
    finally:
...
def _cli_clean(row: dict) -> bool:
    return (row.get('exit_code') == 0 and not row.get('timed_out') and not row.get('stream_cap_exceeded')
            and row.get('job_active_count') == 0 and row.get('readers_stopped') is True and not row.get('host_error')
            and row.get('stream_reader_eof') == [True, True] and row.get('stream_reader_errors') == [None, None]
            and not row.get('evidence_write_error'))
...
def _validate_inspect(value: dict, *, name: str, container_id: str, mode: str,
    ...
    _need(len(mounts) == 2 and {mount.get('Destination') for mount in mounts} == set(expected_mounts),
          'EXECUTOR_MOUNTS')
    for mount in mounts:
...
def run(project: Path, *, mode: Literal['parse', 'import'], output: Path, timeout_seconds: int = 20) -> dict:
    ...
    initial = _project_bytes(project)
    ...
    try:
        ...
        _need(_manifest(_project_bytes(project)) == before and _manifest(_project_bytes(snapshot)) == before,
        ...
        result['errors'].append(str(error) if isinstance(error, ExecutorError) else 'EXECUTOR_HOST_' + type(error).__name__)
    finally:
        try:
            ...
            if owned and container_id:
                ...
                result['owned_removed'] = (_cli_clean(row) and gone.get('exit_code') == 1
                    and not gone.get('timed_out') and not gone.get('stream_cap_exceeded')
                    and gone.get('job_active_count') == 0 and gone.get('readers_stopped') is True
                    and gone.get('stream_reader_eof') == [True, True] and gone.get('stream_reader_errors') == [None, None]
                    and not gone.get('host_error') and not gone.get('evidence_write_error')
            ...
            result['errors'].append(str(error) if isinstance(error, ExecutorError) else 'EXECUTOR_CLEANUP_' + type(error).__name__)
        if admission:
            try:
                admission.close()
                ...
                result['errors'].append(str(error) if isinstance(error, ExecutorError) else 'EXECUTOR_ADMISSION_CLOSE_UNCERTAIN')
                ...
                if owner_path and owner_record and admission.acquired:
                    owner_record['phase'] = 'admission_uncertain'
                    try:
                        _persist_owner(owner_path, owner_record)
                    except Exception:
                        result['errors'].append('EXECUTOR_ADMISSION_UNCERTAINTY_PERSIST_FAILED')
        ...
        for path, field in ((project, 'input_unchanged'), (snapshot, 'snapshot_unchanged')):
            try:
                after = _manifest(_project_bytes(path))
        ...
        result['diagnostic_process_clean'] = bool(result['commandhost'] and _cli_clean(result['commandhost'])
            ...
            and result['binary_unchanged'] and result['log_clean'] and not result['errors'])
