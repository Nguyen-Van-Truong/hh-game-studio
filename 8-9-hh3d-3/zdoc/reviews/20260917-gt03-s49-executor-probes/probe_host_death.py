"""S49 exact-owner host death, idle child, and independent process admission proof."""
...
import threading
import time
from probe_supervisor import executor
...
def main(root):
    ...
    evidence={'public_ack':False,'sandbox_acceptance':False}
    ...
    def launch(prj,out,label):
        ...
        handle=owned._job_for_process(proc)
    ...
    try:
        ...
        try:
            ...
            if second.poll() is None:owned._terminate_job(second_job);second.wait(timeout=3)
            owned._close_job(second_job);secondout.close();seconderr.close()
        evidence['host_killed_at_utc']=datetime.datetime.now(datetime.timezone.utc).isoformat()
        owned._terminate_job(job);process.wait(timeout=3)
        settle=time.monotonic()+1
        while owned._job_active_count(job) and time.monotonic()<settle:time.sleep(.02)
        evidence['host_job_active_count']=owned._job_active_count(job)
        evidence['host_actual_exit']=process.returncode
        owned._close_job(job);job=None;hostout.close();hosterr.close()
        thread.join(timeout=15)
        executor._need(not thread.is_alive() and observer,'PROBE_OBSERVER_INCOMPLETE')
        evidence['observer_host']=observer[0][0]
        evidence['post_death_body_observed']=b'S49_IDLE_BODY_AFTER_HOST_DEATH' in observer[0][1]
        ...
        executor._need(evidence['post_death_body_observed'] and evidence['container_lifetime_seconds']<=10,
        ...
        if job:
            owned._terminate_job(job);process.wait(timeout=3);owned._close_job(job)
    ...
    print(json.dumps({'observed':evidence.get('observed',False),'container_lifetime_seconds':evidence.get('container_lifetime_seconds'),
                      'post_death_body_observed':evidence.get('post_death_body_observed'),'host_job_active_count':evidence.get('host_job_active_count')}))
