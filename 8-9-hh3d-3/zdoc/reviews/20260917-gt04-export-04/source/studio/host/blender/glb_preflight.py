...
def inspect_glb(raw):
    ...
    need(type(value) is dict and set(value)<= {'asset','scene','scenes','nodes','meshes','accessors','bufferViews','buffers'},
    ...
    for node in nodes:
        ...
        need(type(primitive) is dict and set(primitive)<= {'attributes','indices','mode'} and primitive.get('mode',4)==4
    ...
    return {'schema':'HH-GLB-BOX-PREFLIGHT-1','sha256':hashlib.sha256(raw).hexdigest(),'size_bytes':len(raw),
        ...
        'materials':0,'images':0,'extensions':[],'uri_count':0,'meshes':reports,'public_ack':False}
