...
run_id='gt06-s171-godot-htrace-06'; cmd_id='cmd.gt06.s171.godot-htrace.6'; start=time.time(); deadline=start+90
paths={k:RUN/f for k,f in {'go':'godot.stdout.txt','ge':'godot.stderr.txt','co':'cdb.stdout.txt','ce':'cdb.stderr.txt','cl':'cdb.logo.txt'}.items()}; [p.unlink() for p in paths.values() if p.exists()]
gl=[];ge=[];cl=[];ce=[]
def pump(stream,arr,p):
 with p.open('w',encoding='utf-8',errors='replace') as f:
  for line in iter(stream.readline,''):
   arr.append(line.rstrip('\r\n'));f.write(line);f.flush()
t=subprocess.Popen([str(GODOT),'--headless','--path',str(RUN),'--script','res://main.gd'],cwd=str(RUN),stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,bufsize=1); a=threading.Thread(target=pump,args=(t.stdout,gl,paths['go']),daemon=True); b=threading.Thread(target=pump,args=(t.stderr,ge,paths['ge']),daemon=True);a.start();b.start()
...
dbg=subprocess.Popen([str(CDB),'-p',str(t.pid),'-pd','-nosqm','-logo',str(paths['cl']),'-cf',str(RUN/'cdb-commands.txt')],cwd=str(RUN),stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,bufsize=1);c=threading.Thread(target=pump,args=(dbg.stdout,cl,paths['co']),daemon=True);d=threading.Thread(target=pump,args=(dbg.stderr,ce,paths['ce']),daemon=True);c.start();d.start()
