...
GODOT=Path(r"8-9-hh3d-3/studio/.local/tooling/godot-4.7.2-stable/Godot_v4.7.2-stable_win64_console.exe").resolve()
...
run_id="gt06-s171-godot-htrace-02"; cmd_id="cmd.gt06.s171.godot-htrace.2"
...
start=time.time(); deadline=start+90
...
target=subprocess.Popen([str(GODOT),'--headless','--path',str(RUN),'--script','res://main.gd'],cwd=str(RUN),stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,bufsize=1)
...
while time.time()<deadline and target.poll() is None and not any('S171_READY' in x for x in lines): time.sleep(.05)
if not any('S171_READY' in x for x in lines):
    if target.poll() is None: target.kill(); target.wait(timeout=10)
    raise SystemExit('target did not reach READY: '+repr(lines))
...
while time.time()<deadline and (target.poll() is None or proc.poll() is None): time.sleep(.1)
...
markers={'cdb_attached':'CDB_ATTACHED' in ct,'htrace_enable':'Handle tracing enabled.' in ct,'create_break':'CREATE_BREAK' in ct,'close_break':'CLOSE_BREAK' in ct,'htrace_diff':('!htrace -diff' in ct or 'Outstanding handles opened' in ct),'module_offset_resolved':False,'attribution':'UNKNOWN'}
if re.search(r'Godot_v4\.7\.2[^\s+]*\+[0-9a-fA-F`]+',ct): markers['module_offset_resolved']=True
