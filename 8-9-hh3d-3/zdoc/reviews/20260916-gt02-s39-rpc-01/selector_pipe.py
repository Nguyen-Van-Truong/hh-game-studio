...
from .fixture_selector import FixtureSelector, SelectorError
        ...
        try:
            ...
            if job.cancel.is_set():
                if current['phase'] in ('INTENT','STAGING','STAGED'):
                    return _reply(self.selector.reconcile(job.command_id, 'cancel'), job.command_id)
                self._hold.set()
    ...
    def serve_one(self):
        ...
        try:
            ...
            if type(body) is dict and type(body.get('command_id')) is str:
                from .fixture_selector import _name
