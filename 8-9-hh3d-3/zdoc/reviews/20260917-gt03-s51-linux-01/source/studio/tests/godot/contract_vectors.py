...
inspection AND an explicit trusted selected-bundle project_revision to bind
the host contract; engine projections retain the native scene revision verbatim. Cases
...
FENCE = 7
SYNTHETIC_PROJECT_REVISION = 'sha256:' + hashlib.sha256(
    b'SYNTHETIC_TEST_ONLY: unselected scene+script bundle placeholder').hexdigest()
...
def context_from_snapshot(snapshot, *, project_revision, can_undo=False, can_redo=False, file_hashes=None):
    ...
    project_revision is supplied independently by the trusted bundle owner,
    never inferred from the native scene hash or caller file-hash subset.
    ...
    _require(type(project_revision) is str and re.fullmatch(r'sha256:[0-9a-f]{64}', project_revision),
             'VECTOR_PROJECT_REVISION_REQUIRED')
    ...
    context=contract.ValidationContext(PROJECT_ID,observed['revision'],project_revision,observed['generation'],
...
def _project_payload(operation, payload, project_revision):
    payload=_copy(payload)
    if operation != 'scene.inspect':
        payload.setdefault('expected_project_revision',project_revision)
    if operation == 'scene.preview':
        payload['payload']=_project_payload(payload['operation'],payload['payload'],project_revision)
    return payload


def _request(identifier,operation,target,payload,context,**changes):
    payload=_project_payload(operation,payload,context.project_revision)
...
def generate_vectors(actual_snapshot=None, *, project_revision=None, can_undo=None, can_redo=None, file_hashes=None):
    ...
    observations to obtain undo/redo vectors. Actual snapshots require an
    explicit project_revision from the trusted bundle owner. Only no-snapshot
    synthetic cases may use a clearly tagged placeholder. No action is applied here.
    """
    synthetic=actual_snapshot is None
    supplied_project_revision=project_revision is not None
    if synthetic and not supplied_project_revision:
        project_revision=SYNTHETIC_PROJECT_REVISION
    _require(type(project_revision) is str and re.fullmatch(r'sha256:[0-9a-f]{64}',project_revision),
             'VECTOR_PROJECT_REVISION_REQUIRED')
    observed=_synthetic_snapshot() if synthetic else _copy(actual_snapshot)
    context=context_from_snapshot(observed,project_revision=project_revision,
        can_undo=synthetic if can_undo is None else can_undo,
    ...
    def good(name,operation,node_id,payload,expectation):
        ...
        valid.append({'id':name,'request':request.as_dict(),'wire_json':canonical_bytes(request.as_dict()).decode(),
            ...
            'expect':{'python':'VALIDATED','runtime_authorized':False,'committed':False,
                      'before_revision':context.revision,'before_project_revision':context.project_revision,
                      'generation':generation,**expectation}})
    ...
    def changed(name,code,**fields):
        ...
        body['payload']=_project_payload(body['operation'],body['payload'],context.project_revision)
    ...
    result={'schema':'hh-gt03-contract-vectors-2','catalog_digest':contract.CATALOG_DIGEST,
        'snapshot_mode':'SYNTHETIC_TEST_ONLY' if synthetic else 'SUPPLIED_NATIVE_OBSERVATION',
        'project_revision_mode':'SUPPLIED_TRUSTED_PROJECT_REVISION' if supplied_project_revision else 'SYNTHETIC_TEST_ONLY',
