...
class ExportJob:
    def __init__(self,host,prepared,*,admission_probe=False,diagnostic=None,deadline_ms=None,phase_guard=None,input_name='export.blend'):
        if type(input_name) is not str or input_name not in ('export.blend','fixture.blend','checkpoint.blend'):
            raise ValueError('EXPORT_FIXED_INPUT')
        ...
        if phase_guard is not None and not callable(phase_guard):raise ValueError('EXPORT_PHASE_GUARD_REQUIRED')
        self._deadline=AbsoluteDeadline(deadline_ms,host_deadline=getattr(host,'_deadline',None))
        ...
        self._started=False;self._gate_released=False;self._cleanup_attempt=0;self._cleanup_result=None
    ...
    def _check_active(self):
        ...
        stopped_code='EXPORT_CANCELLED' if self._gate_released else 'EXPORT_STOPPED'
        need(not self._stop.is_set() and not self.host._stopped,stopped_code)
        self._deadline.check()
        if self._phase_guard is not None:self._phase_guard()
        self._deadline.check()
        need(not self._stop.is_set() and not self.host._stopped,stopped_code)
    ...
    def run(self):
        ...
        try:
            self._check_active()
            artifact=self.prepared['artifact'];need(artifact['name']==self._input_name,'EXPORT_FIXED_INPUT')
            ...
            while process.poll() is None:
                ...
                time.sleep(.02)
            self._check_active();need(process.returncode==0,'EXPORT_NATIVE_FAILED')
            ...
            try:
                ...
                if failure is None:
                    try:self._check_active()
                        ...
                        failure=exc;report.update(completed=False,failure=str(exc) if isinstance(exc,HostError)
                            else type(exc).__name__,status='EXPORT_COMPLETION_EXPIRED_OR_REVOKED')
            except BaseException as exc:
