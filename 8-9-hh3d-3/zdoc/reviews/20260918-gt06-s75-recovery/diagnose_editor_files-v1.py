...
func _diagnostic_scan_changed() -> void:
...
func _diagnostic_files() -> void:
    var filesystem: EditorFileSystem = EditorInterface.get_resource_filesystem()
    filesystem.filesystem_changed.connect(_diagnostic_scan_changed)
    ...
    filesystem.filesystem_changed.disconnect(_diagnostic_scan_changed)
...
def run(ignored):
    ...
    # Include the imported CLI owner's non-Python schema explicitly.
    schema = 'host/blender/cli-protocol.schema.json'
    if (n.STUDIO / schema).is_file():
    ...
    run_id = 'gt06-s75-editor-files-' + ('ignored' if ignored else 'indexed') + '-01'
    ...
    n.write(root / 'diagnostic.json', {'full_benchmark': False, 'formal_acceptance': False,
        ...
        'explicit_scan_intervention': True, 'outer_wall_seconds': 60,
