...
internal static class S163DbgEngCreateProcess
{
    ...
    public static int Main(string[] args) {
        ...
        string fixture = Path.GetFullPath(args[0]); string dir = Path.GetFullPath(args[1]); string commands = Path.GetFullPath(args[2]); Directory.CreateDirectory(dir);
        string log = Path.Combine(dir, "dbgeng-output.log"); string summary = Path.Combine(dir, "run-summary.txt"); string output = Path.Combine(dir, "fixture-output.jsonl"); string gate = Path.Combine(dir, "debugger-gate.txt");
        File.Delete(gate);
        File.WriteAllText(commands, "open\nchurn 16\nclose\nexit\n");
        IntPtr client = IntPtr.Zero, control = IntPtr.Zero, systems = IntPtr.Zero, commandPtr = IntPtr.Zero; bool com = false; int createHr=-1, waitHr=-1, runHr=-1, finalWaitHr=-1, exitHr=-1; uint pid=0, exitCode=uint.MaxValue;
        try {
            ...
            string cmd = "\"" + fixture + "\" \"" + output + "\" \"" + commands + "\" \"" + gate + "\""; commandPtr = Marshal.StringToHGlobalAnsi(cmd);
            ...
            waitHr=M<WaitForEventDelegate>(control,93)(control, DEBUG_WAIT_DEFAULT, 10000); Need("WaitForEvent(initial)", waitHr);
            ...
            M<GetCurrentProcessSystemIdDelegate>(systems,27)(systems, out pid);
            var execute=M<ExecuteDelegate>(control,66); Need("reload", execute(control,0,".reload /f kernelbase.dll",DEBUG_EXECUTE_DEFAULT));
            Need("bp CreateEventW", execute(control,0,"bp kernelbase!CreateEventW \".echo S163_CREATE_EVENT_ENTRY; gu; .echo S163_CREATE_EVENT_RETURN; ? @rax; g\"",DEBUG_EXECUTE_DEFAULT));
            Need("bp CreateIoCompletionPort", execute(control,0,"bp kernelbase!CreateIoCompletionPort \".echo S163_CREATE_IOCP_ENTRY; gu; .echo S163_CREATE_IOCP_RETURN; ? @rax; g\"",DEBUG_EXECUTE_DEFAULT));
            Need("bp CloseHandle", execute(control,0,"bp kernelbase!CloseHandle \".echo S163_CLOSE_ENTRY; ? @rcx; gu; .echo S163_CLOSE_RETURN; ? @rax; g\"",DEBUG_EXECUTE_DEFAULT));
            File.WriteAllText(gate, "go\n");
            runHr=execute(control,0,"g",DEBUG_EXECUTE_DEFAULT);
            for (int i=0;i<15;i++) { finalWaitHr=M<WaitForEventDelegate>(control,93)(control,DEBUG_WAIT_DEFAULT,1000); if (finalWaitHr<0) break; }
            exitHr=M<GetExitCodeDelegate>(client,27)(client,out exitCode);
            File.WriteAllText(summary,"create_hr="+Hr(createHr)+Environment.NewLine+"wait_hr="+Hr(waitHr)+Environment.NewLine+"run_hr="+Hr(runHr)+Environment.NewLine+"final_wait_hr="+Hr(finalWaitHr)+Environment.NewLine+"exit_hr="+Hr(exitHr)+Environment.NewLine+"pid="+pid+Environment.NewLine+"debugger_exit_code="+exitCode+Environment.NewLine);
            return 0;
        } catch (Exception ex) { File.WriteAllText(summary,"exception="+ex.GetType().FullName+Environment.NewLine+"message="+ex.Message+Environment.NewLine+"create_hr="+Hr(createHr)+Environment.NewLine+"wait_hr="+Hr(waitHr)+Environment.NewLine+"pid="+pid+Environment.NewLine); return 1; }
        finally {
            ...
            if (commandPtr != IntPtr.Zero) Marshal.FreeHGlobal(commandPtr); if (com) CoUninitialize();
        }
    }
}
