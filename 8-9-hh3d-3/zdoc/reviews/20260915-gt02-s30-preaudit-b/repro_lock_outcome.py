"""Preaudit B: real journal lock/read failures after one confirmed mock effect."""
...
with tempfile.TemporaryDirectory(prefix='gt02-s30-lock-audit-b-') as temporary:
    ...
    print(json.dumps({'proof_class':'REAL_LOOPBACK_REAL_GUARD_AND_READ_FAILURES_NOT_CANDIDATE',
        ...
        'rows':rows,'source_unchanged':all((ROOT/'studio'/name).read_bytes()==data for name,data in source.items())},indent=2))
