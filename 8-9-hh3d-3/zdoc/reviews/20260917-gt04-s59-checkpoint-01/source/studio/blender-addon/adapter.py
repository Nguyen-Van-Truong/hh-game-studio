...
class FixtureAdapter:
    ...
    def _check_preconditions(self, command, before):
    ...
    def _preflight_edit(self, command, before):
        """Read-only edit checks shared by native preview and actual apply."""
        self._guard()
        command = contract.validate(dict(command, schema=contract.SCHEMA))
        if command["operation"] == "scene.inspect":
            raise Rejected("EDIT_OPERATION_REQUIRED")
        if self._stopped:
