...
class ExportDeadlineTests(unittest.TestCase):
    ...
    def test_stop_after_child_gate_is_cancelled_and_cleans_same_owned_job(self):
        job = self.job(deadline_ms=1001000)
        def poll():
            job.request_stop()
            return None
        self.process.poll = poll
        with self.assertRaisesRegex(HostError, 'EXPORT_CANCELLED'):
            job.run()
        self.assertTrue(job._gate_released)
        self.assertTrue(self.process.gate.closed)
        self.assertIs(job._job, self.native_job)
        job._cleanup.assert_called_once()
