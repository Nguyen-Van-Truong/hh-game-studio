...
def edit(root):
    ...
    OWNER = owner = ui.UIAdapter(owned_root=root)
    initial = owner.inspect()
    check("empty_owned_ui_fixture", initial["snapshot"]["objects"] == [])
    checkpoint_row = yield from queued(owner, make(owner, "save-checkpoint", "checkpoint.save", {"slot": "checkpoint"}))
    checkpoint = checkpoint_row["result"]["artifact"]
    ...
    third = yield from queued(owner, make(owner, "edit-create", "mesh.create_box", {"object_id": "third", "size": [1, 1, 1]}))
    check("edit_create_restores_context", owner.inspect()["context"] == edit_before["context"] and len(bpy.data.objects) == 3)
    yield from queued(owner, make(owner, "undo-edit-create", "history.undo"))
    yield from queued(owner, make(owner, "undo-edit-again", "history.undo"))
    check("native_two_step_edit_undo", owner.inspect() == edit_before)
    yield from queued(owner, make(owner, "redo-edit-again", "history.redo"))
    yield from queued(owner, make(owner, "redo-edit-create", "history.redo"))
    check("native_two_step_edit_redo", owner.inspect() == third["result"]["after"])
    yield from queued(owner, make(owner, "undo-for-branch", "history.undo"))
    branched = yield from queued(owner, make(owner, "branch-transform", "object.transform.set",
                  {"object_id": "anchor", "location": [4, -1, 6], "rotation": [0.2, 0.3, 0.4], "scale": [1, 2, 0.5]}))
    no_redo = yield from queued(owner, make(owner, "redo-after-branch", "history.redo"), expected="REJECTED")
    check("new_edit_discards_redo_branch", no_redo["reason"] == "NO_OWNED_HISTORY" and owner.inspect() == branched["result"]["after"])
    with owner._view():
        owner._mode("OBJECT")
    ...
    saved_row = yield from queued(owner, make(owner, "save-final", "checkpoint.save", {"slot": "fixture"}))
    saved = saved_row["result"]["artifact"]
    check("native_gui_save_exact_revision", owner.inspect() == current and saved["size_bytes"] > 0)
    check("duplicate_save_original_receipt", owner.queue.submit(c.canonical(make(owner, "save-final", "checkpoint.save", {"slot": "fixture"}))) == saved_row)
    exists = yield from queued(owner, make(owner, "save-again", "checkpoint.save", {"slot": "fixture"}), expected="REJECTED")
    check("save_refuses_overwrite", exists["reason"] == "SAVE_SLOT_EXISTS" and support.hash_file(root / "fixture.blend") == saved["sha256"])
