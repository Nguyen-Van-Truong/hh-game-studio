...
class PublicationOwnerTests(unittest.TestCase):
    ...
    def test_discovery_reports_capacity_and_never_waits_for_work(self):
        self._inert_read_owner()
        value=self.value;value.auth_model=auth
        value.editor_model=types.SimpleNamespace(MAX_EFFECTS=16)
        value._editor=types.SimpleNamespace(_intents=dict.fromkeys(range(14)))
        value._validator=Mock();value._validator.snapshot.return_value={'attempts':[1,2,3]}
        value.validation_model=types.SimpleNamespace(MAX_RUNS=4)
        credential=value.sessions.issue()
        arguments={'authorization':'Bearer '+credential.bearer,'catalog_digest':value.contract.CATALOG_DIGEST}
        body={'project_id':value.project_id}
        found=value.discover(body,**arguments)
        capabilities={item.operation for item in found.capabilities}
        self.assertIn('scene.save',capabilities)
        self.assertFalse(capabilities & auth.EDIT_OPERATIONS)
        self.assertEqual(found.limits['validation_attempts_remaining'],1)
        self.assertEqual(found.limits['editor_effects_remaining'],2)
        value._validator.snapshot.return_value={'attempts':[1,2,3,4]}
        found=value.discover(body,**arguments)
        self.assertEqual({item.operation for item in found.capabilities},auth.READ_OPERATIONS)
        value._work.acquire()
        value._validator.snapshot.side_effect=AssertionError('must not query busy owner')
        try:
            busy=value.discover(body,**arguments)
            self.assertEqual(busy.capabilities,())
            self.assertEqual(busy.limits['owner_busy'],1)
        finally:value._work.release()

    def test_exhausted_fifo_discovery_keeps_existing_save_but_no_new_writer(self):
        self._inert_read_owner();value=self.value;value.auth_model=auth
        value.editor_model=types.SimpleNamespace(MAX_EFFECTS=16)
        value._editor=types.SimpleNamespace(_intents={})
        value._validator=Mock();value._validator.snapshot.return_value={'attempts':[1]}
        value.validation_model=types.SimpleNamespace(MAX_RUNS=4)
        credential=value.sessions.issue()
        arguments={'authorization':'Bearer '+credential.bearer,'catalog_digest':value.contract.CATALOG_DIGEST}
        body={'project_id':value.project_id}
        lease=value.lease({**body,'ttl_ms':30000},**arguments)
        # Fill retained outcomes with real queue/cancel transitions, without sleeps.
        for index in range(63):
            queued=value.lease_enqueue({**body,'request_id':'ticket.fill.'+str(index),
                'ttl_ms':30000,'wait_ms':90000},**arguments)
            value.lease_cancel({**body,'ticket_id':queued['ticket_id']},**arguments)
        found=value.discover(body,**arguments)
        self.assertEqual(found.limits['writer_tickets_remaining'],0)
        self.assertIn('scene.save',{item.operation for item in found.capabilities})
        stranger=value.sessions.issue()
        found=value.discover(body,authorization='Bearer '+stranger.bearer,
            catalog_digest=value.contract.CATALOG_DIGEST)
        self.assertEqual({item.operation for item in found.capabilities},auth.READ_OPERATIONS)
        with self.assertRaisesRegex(owner.SafetyViolation,'TICKET_LIMIT'):
            value.lease({**body,'ttl_ms':30000},**arguments)
        with patch.object(auth,'epoch_ms',return_value=lease['expires_ms']+1):
            found=value.discover(body,**arguments)
            self.assertEqual({item.operation for item in found.capabilities},auth.READ_OPERATIONS)
