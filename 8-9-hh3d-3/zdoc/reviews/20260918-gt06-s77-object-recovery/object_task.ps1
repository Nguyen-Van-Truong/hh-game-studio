...
$name = 'HHStudio.GT06.gt06-s77-object-diagnostic-03'
...
if ($Command -eq 'run') {
    if ($null -ne $task -or (Test-Path -LiteralPath (Join-Path $PSScriptRoot 'object-owner-03'))) { throw 'DIAGNOSTIC_ALREADY_EXISTS' }
    $definition = $service.NewTask(0)
    $definition.RegistrationInfo.Description = 'HH3D S77 supplemental6x100 native object attribution plus60s idle diagnostic; not full campaign or acceptance.'
    ...
    $record | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $PSScriptRoot 'object-launch-03.json') -Encoding utf8
}
