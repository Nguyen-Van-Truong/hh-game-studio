...
`operations.json` catalog `gt03-godot-2` defines nine operations. Its static `implemented` and
...
only knows the semantic scene revision; the future host inspection backend
must combine it with the independently verified selected-bundle observation.
...
field. Changing it on a same-ID request changes that digest. A future dispatcher
must retain the originally admitted scene/project/generation observations and
...
save, undo or redo. Script preview remains declaration-only until its backend
is integrated. Preview checks the same proposed mutation against current context;
