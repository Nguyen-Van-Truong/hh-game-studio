...
spec = importlib.util.spec_from_file_location('s46_candidate_helpers', STUDIO/'tests/protocol/run_gt02_candidate.py')
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='hh-s46-public-frozen-') as temporary:
        ...
        code = '\n'.join([
            ...
            "modules=['test_selector_public','test_selector_pipe','test_file_consumer','test_managed_fixture']",
            ...
            "print('HH_S46_PUBLIC_RESULT '+json.dumps({'tests_run':r.testsRun,'test_ids':ids,'failures':len(r.failures),'errors':len(r.errors),'skips':len(r.skipped)}),flush=True)",
    ...
    markers=[line[len('HH_S46_PUBLIC_RESULT '):] for line in stdout.splitlines() if line.startswith('HH_S46_PUBLIC_RESULT ')]
    ...
    passed=(run['exit_code']==run['wrapper_exit_code']==host.get('exit_code')==0
        ...
        and all(after.get(key)==before.get(key) for key in ('host/core/selector_pipe.py','host/core/selector_contract.py','host/core/SELECTOR_PUBLIC.md','tests/protocol/test_selector_public.py')) and snapshot_unchanged)
    ...
    record={'schema':'hh-s46-public-focused-v1','run_id':'GT02-S46-PUBLIC-01-'+args.attempt,
        'command_id':'cmd.gt02-s46-public-01.'+args.attempt,'seed':8785,'started_utc':started,
        ...
        'parallel_source_changes':[key for key in sorted(set(before)|set(after)) if before.get(key)!=after.get(key)],
        'scope':'IMPLEMENTER_FOCUSED_SNAPSHOT_NOT_FREEZE',
