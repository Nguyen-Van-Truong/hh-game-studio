...
RUN_ID = 'gt06-s88-object-attribution-01'
PREFLIGHT_ID = 'gt06-s88-object-attribution-preflight-01'
...
def helpers():
    return {p.relative_to(ROOT).as_posix(): hashlib.sha256(p.read_bytes()).hexdigest()
            for p in (Path(__file__), BASE / 'object_probe_s88.gd')}
...
def install_prepare(c, output):
    ...
    def prepared(project, factory, trusted, binding):
        ...
        probe = (BASE / 'object_probe_s88.gd').read_text(encoding='utf-8-sig')
        ...
        c.write(output / 'native-overlay.json', {
            ...
            'probe_sha256': c.sha((BASE / 'object_probe_s88.gd').read_bytes()),
