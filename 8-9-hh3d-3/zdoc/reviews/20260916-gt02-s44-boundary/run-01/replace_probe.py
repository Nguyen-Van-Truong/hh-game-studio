...
def main():
    ...
    out = {'run_id':'GT02-S44-BOUNDARY-01','timestamp_utc':datetime.now(timezone.utc).isoformat(),
    ...
    out['diagnostic_sources'] = {p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path(__file__),BASE/'boundary_child.c',HELPER/'boundary_probe.py',HELPER/'windows_api.py')}
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='gt02-s44-native-') as tmp:
            ...
            prior.BASE = BASE  # Select this diagnostic's C source for fixed compiler helper.
            executable = prior.compile_native(base, out)
