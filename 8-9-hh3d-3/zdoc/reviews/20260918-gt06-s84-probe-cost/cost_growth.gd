...
var _cost_injected_tree: Tree = null
...
func _cost_tick() -> bool:
    ...
    if not _cost_waiting:
        var receipt: Dictionary = _write_new("cost-%02d.json" % _cost_phase,
            ...
            "label": ["before", "retained", "growth", "released"][_cost_phase],
    ...
    if _cost_phase == 0:
        ...
        # Controlled diagnostic UI objects only; no fixture scene mutation.
        _cost_injected_tree = Tree.new()
        _cost_injected_tree.name = "DiagnosticCensusTree"
        get_tree().root.add_child(_cost_injected_tree)
        var item: TreeItem = _cost_injected_tree.create_item()
        item.set_text(0, "diagnostic-added-item")
        _object_probe_sample("cost_growth", true)
    elif _cost_phase == 2:
        _cost_injected_tree.free()
        _cost_injected_tree = null
        _object_probe_sample("cost_restore", true)
