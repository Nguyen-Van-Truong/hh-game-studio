...
def discovery(project_id: str, *, implemented: frozenset[str] = frozenset(),
    ...
    return Discovery(PROTOCOL_VERSION, SCHEMA_VERSION, CATALOG_DIGEST,
                     'gt03-godot-catalog', '1.0', project_id, caps, dict(_LIMITS))
...
def _context(context: ValidationContext) -> tuple[dict[str, dict], dict[str, str]]:
    ...
    for value in (context.generation, context.now_ms, context.lease_expires_ms, context.fencing_epoch):
        _integer(value)
    ...
    for node in nodes.values():
        ...
        siblings = [x['name'] for x in nodes.values() if x['parent_id'] == node['parent_id']]
...
def _semantic(operation: str, target: dict, payload: dict, context: ValidationContext,
    ...
    _integer(payload['expected_generation'])
    ...
    projection = {'operation': operation, 'expected_generation': context.generation,
                  'target_stable_id': target_id, 'payload': payload}
    ...
    if operation == 'scene.inspect':
        ...
        _need(not any(n['parent_id'] == target_id and n['name'] == payload['name'] for n in nodes.values()),
        ...
        new = {key: value for key, value in payload.items() if key != 'expected_generation'}
        ...
        if 'name' in changes:
            _need(not any(n['stable_id'] != target_id and n['parent_id'] == nodes[target_id]['parent_id']
                          and n['name'] == changes['name'] for n in nodes.values()), 'GODOT_DUPLICATE_SIBLING_NAME')
        ...
        _need(type(nested) is str and nested in _OPS and _OPS[nested]['kind'] == 'mutation', 'GODOT_PREVIEW_OPERATION_FORBIDDEN')
        inner, preview = _semantic(nested, payload['target'], payload['payload'], context, nodes, files)
        projection = {'operation': operation, 'expected_generation': context.generation,
                      'target_stable_id': ROOT_ID, 'payload': payload, 'preview_command': inner}
        ...
        preview['checkpoint_required'] = True
        preview['affected_files'] = [SCENE_PATH, SCRIPT_PATH]
...
def validate_request(request: Request | Mapping[str, Any], context: ValidationContext) -> ValidatedCommand:
    ...
    _need(context.now_ms < checked.deadline_ms <= min(context.lease_expires_ms,
          context.now_ms + _LIMITS['max_deadline_ms']), 'GODOT_DEADLINE_EXPIRED')
    _need(not context.stopped or checked.operation == 'scene.inspect', 'GODOT_STOPPED')
