...
class BlenderUIHost:
    def __init__(self,parent:Path,*,binary:Path,session_seconds=90):
        ...
        self._process=self._job=self._api=None
        ...
        try:
            ...
            self._root=self._api.open(self.project,directory=True)
            self._identity=self._api.inspect(self._root,self.project,directory=True)
            self._api.check_security(self._root)
    ...
    def submit(self,command,*,ttl_ms=5000):
        ...
        current=self._api.inspect(self._root,self.project,directory=True)
        need(current.same_file(self._identity),'BLENDER_ROOT_CHANGED'); self._api.check_security(self._root)
