...
epoch in 1–2147483647 in every payload; it must match the trusted observation. It does not
...
| `scene.preview` | `{"stable_id":"root"}` | `operation`, `target`, `payload` for one node create/update/remove or declared script replacement; nested generation must agree |
