...
int wmain(void) {
    wchar_t work[256], control[256], logpath[1400], mode[32], private_root[1400], events[1400];
    char frame[32768], reply[8192], ack[] = "S39_FINAL_ACK";
    DWORD n, error;
    HANDLE wp = INVALID_HANDLE_VALUE, cp = INVALID_HANDLE_VALUE, probe;
    FILE *log = NULL;
    int code = 95;
    if (!envw(L"S39_WORK", work, 256) || !envw(L"S39_CONTROL", control, 256) ||
        !envw(L"S39_LOG", logpath, 1400) || !envw(L"S39_MODE", mode, 32) ||
        wcsstr(logpath, L"gt02-s39-native-") == NULL) return 91;
    if (_wfopen_s(&log, logpath, L"wb") != 0) return 92;
    setvbuf(log, NULL, _IONBF, 0);
    fputs("{\"started\":\"S39_NATIVE_STARTED\",", log);
    ...
    if (probe != INVALID_HANDLE_VALUE) { CloseHandle(probe); goto done; }
    if (error != ERROR_ACCESS_DENIED) goto done;
    if (!envw(L"S39_PRIVATE_ROOT", private_root, 1400) || !envw(L"S39_EVENTS", events, 1400)) goto done;
    probe = CreateFileW(private_root, FILE_LIST_DIRECTORY, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                        NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, NULL);
    error = probe == INVALID_HANDLE_VALUE ? GetLastError() : 0;
    fprintf(log, "\"private_read_error\":%lu,", error);
    if (probe != INVALID_HANDLE_VALUE) { CloseHandle(probe); goto done; }
    if (error != ERROR_ACCESS_DENIED) goto done;
    probe = CreateFileW(private_root, FILE_ADD_FILE, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                        NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, NULL);
    error = probe == INVALID_HANDLE_VALUE ? GetLastError() : 0;
    fprintf(log, "\"private_write_error\":%lu,", error);
    if (probe != INVALID_HANDLE_VALUE) { CloseHandle(probe); goto done; }
    if (error != ERROR_ACCESS_DENIED) goto done;
    probe = CreateFileW(events, FILE_READ_DATA | FILE_WRITE_DATA, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                        NULL, OPEN_EXISTING, FILE_FLAG_OPEN_REPARSE_POINT, NULL);
    error = probe == INVALID_HANDLE_VALUE ? GetLastError() : 0;
    fprintf(log, "\"events_access_error\":%lu,", error);
    if (probe != INVALID_HANDLE_VALUE) { CloseHandle(probe); goto done; }
    if (error != ERROR_ACCESS_DENIED) goto done;
    ...
    n = GetEnvironmentVariableA("S39_FRAME0", frame, sizeof(frame));
    ...
    n = GetEnvironmentVariableA("S39_FRAME1", frame, sizeof(frame));
    if (n == 0 || n >= sizeof(frame) || !send_frame(cp, frame) || !receive(cp, reply, sizeof(reply))) goto done;
    hex_reply(log, "control_reply_hex", reply);
    if (wcscmp(mode, L"submit") == 0) {
        n = GetEnvironmentVariableA("S39_FRAME2", frame, sizeof(frame));
        if (n == 0 || n >= sizeof(frame) || !send_frame(wp, frame) || !receive(wp, reply, sizeof(reply))) goto done;
        hex_reply(log, "retry_reply_hex", reply);
    }
    if (!send_frame(cp, ack) || !receive(cp, reply, sizeof(reply)) || strcmp(reply, "S39_FINAL_BYE") != 0) goto done;
    code = 61;
done:
    if (wp != INVALID_HANDLE_VALUE) CloseHandle(wp);
    if (cp != INVALID_HANDLE_VALUE) CloseHandle(cp);
    fprintf(log, "\"complete\":\"%s\"}", code == 61 ? "S39_NATIVE_COMPLETE" : "S39_NATIVE_FAILED");
    fclose(log);
    return code;
}
