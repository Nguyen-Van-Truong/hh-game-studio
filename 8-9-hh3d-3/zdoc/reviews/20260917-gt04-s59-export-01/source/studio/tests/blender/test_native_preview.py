...
class QueuePreviewTests(unittest.TestCase):
    ...
    def test_inspect_preview_is_explicitly_unsupported(self):
        value = dict(command(), operation="scene.inspect", expected_revision=None, expected_context=None, payload={})
        with self.assertRaisesRegex(q.c.Rejected, "PREVIEW_OPERATION_UNSUPPORTED"):
            self.preview(value)
        self.callback.assert_not_called()

    def test_file_preview_repeats_without_reserving_apply_id(self):
        for operation, slot in [('checkpoint.save', 'fixture'), ('checkpoint.save', 'checkpoint'), ('export.prepare', 'export')]:
            value = dict(command(), operation=operation, payload={'slot': slot})
            self.assertEqual(self.preview(value), self.preview(value))
        self.assertEqual(self.queue._rows, {})
        self.assertEqual(list(self.queue._pending), [])
        self.apply.assert_not_called()
