...
AUTHORITY=0. This component is used by the explicit writer integration candidate;
it does not by itself attach a public route or grant any capability.
...
normalized into the private grammar. Public `(project_id, command_id)` remains
reserved across authenticated sessions within this configured project ledger.
A foreign session cannot admit that ID again or read its receipt: reuse is
`BLENDER_LEDGER_COMMAND_OWNER`, while foreign lookup remains not-found. History
validation also rejects two intents sharing a public ID under different sessions.
...
Save/export are integration shapes, not connected writer operations. The
integrating facade must perform capability/schema validation,
...
`replay(grant, request, private_operation)` validates the current registered
grant and exact historical bindings without admitting new work. It returns
`None` for a miss or an Admission with exact historical bytes and no permit.
It does not require a fresh native lease/deadline or a still-live native owner.
Uncertain history/guard failures hold admission and report a typed UNKNOWN.
