"""Explicit GT04 client candidate for native edits and fixed protected bundles.
...
Public durable ACK and writable GUI restart remain unavailable on this owner.
"""
...
from .client_preview import actual_diff, normalize_preview, normalize_publication_preview
from .publication_owner import BlenderPublicationOwner
from . import publication_state
from .deadline import DeadlineError
...
class BlenderWriterClientOwner:
    ...
    @classmethod
    def from_session(cls, durable, *, project_id='blender.owned-fixture', publication=None, publications=None):
        need(type(durable) is DurableBlenderSession, 'BLENDER_EXACT_DURABLE_REQUIRED')
        need(publication is None or publications is None, 'BLENDER_PUBLICATION_MAPPING_REQUIRED')
        if publications is None:
            publications = {} if publication is None else {'export.publish': publication}
        need(type(publications) is dict and set(publications) <= publication_state.PROFILES,
             'BLENDER_PUBLICATION_MAPPING_REQUIRED')
        publishers = dict(publications)
        need(len({id(p) for p in publishers.values()}) == len(publishers), 'BLENDER_PUBLICATION_MAPPING_REQUIRED')
        bindings = {}
        for operation, publisher in publishers.items():
            need(type(publisher) is BlenderPublicationOwner and publisher.session is durable
                 and publisher.host is durable.host and not publisher._closed
                 and not publisher._readonly and not publisher._held,
            ...
            with publisher._mutex:
                ...
                need(state['phase'] == 'EMPTY' and not state['stopped']
                     and publication_state.profile(state['config']) == operation
                ...
                bindings[operation] = (publisher, publisher.storage_id, canonical_bytes(state['config']))
        need(len({row[1] for row in bindings.values()}) == len(bindings), 'BLENDER_PUBLICATION_MAPPING_REQUIRED')
        ...
        self._publications = publishers
        self._publication_bindings = bindings
        self._supported = SUPPORTED | set(publishers)
        self._check_native()
        return self
    ...
    def _check_native(self, *, control=False):
        ...
        need(set(self._publications) == set(self._publication_bindings)
             and self._supported == SUPPORTED | set(self._publication_bindings), 'BLENDER_PUBLICATION_BINDING')
        for operation, publisher in self._publications.items():
            need(type(publisher) is BlenderPublicationOwner and publisher.session is self._durable
                 ...
                 and (publisher, publisher.storage_id, canonical_bytes(publisher._state['config'])) == self._publication_bindings[operation],
                 'BLENDER_PUBLICATION_BINDING')
    ...
    def preview(self, body, *, authorization, catalog_digest):
        ...
        need(request.operation in self._supported-{catalog.READ}, 'BLENDER_PREVIEW_OPERATION_UNSUPPORTED')
        ...
        try:
            ...
            native_lease = self._native_preview_lease(grant, lease)
            if request.operation in publication_state.PROFILES:
                publisher = self._publications[request.operation]
                private = command if request.operation=='export.publish' else publication_state.save_request(command,request.operation)
                row = publisher.preview(private,native_lease,deadline_ms=request.deadline_ms,
                    phase_guard=lambda:self.sessions.check_write_permit(permit))
                result = normalize_publication_preview(request,command,row,source_sha256=self._source_sha256,
                                                       generation=self._host._session,storage_id=publisher.storage_id)
            else:
                row = self._host.preview(command, lease={'fencing_epoch': lease.fencing_epoch,
                    'expires_ms': native_lease.expires_ms}, deadline_ms=request.deadline_ms, ttl_ms=catalog.MAX_UI_MS)
                result = normalize_preview(request, command, row, source_sha256=self._source_sha256,
                                           generation=self._host._session)
            need(canonical_bytes(result['before']) == canonical_bytes(before), 'BLENDER_STALE_REVISION')
            self._check_native()
            self.sessions.check_write_permit(permit)
    ...
    def _publish(self, request, command, before, native_lease, permit):
        publisher = self._publications.get(request.operation)
        need(publisher is not None, 'BLENDER_OPERATION_NOT_CONNECTED')
        private = command if request.operation == 'export.publish' else publication_state.save_request(command, request.operation)
        publish = publisher.publish if request.operation == 'export.publish' else publisher.publish_save
        try:
            raw = publish(command, native_lease, deadline_ms=request.deadline_ms,
                          phase_guard=lambda: self.sessions.check_write_permit(permit))
        except (publication_state.PublicationError, DeadlineError) as error:
            # Publisher wraps every post-INTENT failure as outcome_unknown.
            # Its explicit pre-effect capacity/deadline rejection must not
            # poison the live editor or pretend an unstarted effect is unknown.
            if error.outcome_unknown:
                raise
            return response(Status.REJECTED, error.code, request.command_id)
        result = parse_json(raw)
        with publisher._mutex:
            state = publisher._refresh()
            need(state['phase'] == 'TERMINAL' and state['intent']['request'] == private
                 and state['intent']['request_sha256'] == publication_state.sha(canonical_bytes(private))
    ...
    def submit(self, body, *, authorization, catalog_digest):
        ...
        try:
            ...
            if not known:
                self._hold()
            code = error.code if isinstance(error, (SafetyViolation, ValidationError, JournalError,
                                                    publication_state.PublicationError, DeadlineError)) else 'BLENDER_CLIENT_UNKNOWN'
    ...
    def stop(self, body, *, authorization, catalog_digest):
        ...
        try:
            ...
            # Signal every exact publisher before any persistent Stop can wait.
            for publisher in self._publications.values(): publisher.request_stop()
            if self._publications:
                for publisher in self._publications.values():
                    native = publisher.stop()
