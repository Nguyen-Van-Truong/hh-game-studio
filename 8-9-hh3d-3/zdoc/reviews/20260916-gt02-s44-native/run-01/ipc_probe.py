...
sys.path.insert(0, str(BASE.parent / '20260916-gt02-s44-boundary'))
...
from studio.host.core.fixture_selector import FixtureSelector, FileFixtureReleaseConsumer
...
def run_case(executable, scratch, broker_root, package, mode):
    ...
    with ExitStack() as stack:
        file_parent = broker_root / 'managed-files'
        file_parent.mkdir()
        files = stack.enter_context(ProtectedFileRoot.create(file_parent))
        log = stack.enter_context(PrivateEventLog.create(broker_root))
        store = stack.enter_context(PrivateBlobStore.create(broker_root))
        consumer = FileFixtureReleaseConsumer('project.fixture', files)
        ...
        seed_payload = {'assets':{'scene':{'value':'baseline','references':['leaf']},
                                  'leaf':{'value':'baseline leaf','references':[]}},
        ...
        seed_target = {'stable_id':'active-release'}
        seed_request = Request('baseline.'+mode,'project.fixture','fixture.release.activate',lease['lease_id'],lease['fencing_epoch'],
            snap['revisions']['game_revision'],seed_target,seed_payload,
            payload_digest('fixture.release.activate',seed_target,seed_payload,'hh-studio-0.1'),epoch_ms()+10_000)
        assert broker.dispatch('/v1/commands',seed_request.as_dict(),session,control=False).status is Status.ACCEPTED_PENDING
        for _ in range(3): baseline_receipt = broker.advance()
        assert baseline_receipt.status is Status.COMMITTED
        baseline_version, baseline_bytes = files.read('active.json')
        record['baseline_file'] = asdict(baseline_version)
        snap = selector.snapshot()
        ...
        try:
            ...
            values.update(WINDIR=os.environ['SystemRoot'], PATH=str(Path(os.environ['SystemRoot'])/'System32'),
                          ...
                          S39_FRAME2=frames[2], S39_PRIVATE_ROOT=str(files.root), S39_EVENTS=str(log.root/'.events'))
            ...
            if mode in ('submit','drop_reply'):
                assert record['snapshot']['generation'] == 2 and record['snapshot']['ready']
                assert record['blob_count'] == 6 and consumer.adoption_count == 2
                ...
                assert record['snapshot']['generation'] == 1 and consumer.adoption_count == 1 and record['blob_count'] == 6
                ...
                assert record['snapshot']['generation'] == 2 and consumer.adoption_count == 1 and record['blob_count'] == 6
                assert record['snapshot']['stopped'] and not record['snapshot']['ready']
                assert record['work_receipt']['status'] == 'UNKNOWN' and record['event_kinds'].count('TERMINAL') == 1
                ...
                assert record['restored_snapshot']['generation'] == 3 and record['restored_snapshot']['ready']
                assert record['restored_snapshot']['stopped']
                assert len(list(store.root.glob('blob-*'))) == 6
            else:
                assert record['snapshot']['generation'] == 1 and consumer.adoption_count == 1 and record['blob_count'] == 3
                assert record['work_receipt']['code'] == 'COMMAND_NOT_FOUND'
                assert record['event_kinds'].count('INTENT') == 1
            ...
            final_version, final_bytes = files.read('active.json')
            record['final_file'] = asdict(final_version)
            record['file_changed'] = not baseline_version.identity.same_file(final_version.identity)
            if mode in ('submit','drop_reply'):
                assert record['file_changed']
                assert json.loads(final_bytes)['assets']['scene']['value'] == 'native inert scene'
            elif mode == 'stop_selected':
                assert record['file_changed']
                assert json.loads(final_bytes)['assets'] == json.loads(baseline_bytes)['assets']
            else:
                assert not record['file_changed'] and final_bytes == baseline_bytes
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='gt02-s39-native-') as tmp:
            ...
            image=compile_native(base,BASE/'boundary_child.c',out)
            ...
            out['marker']='S44_FILE_SELECTOR_IPC_DIAGNOSTIC_COMPLETE'
    except Exception as exc:
