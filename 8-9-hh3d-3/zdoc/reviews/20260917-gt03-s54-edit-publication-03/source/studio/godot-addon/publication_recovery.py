...
class PublicationRecovery:
    ...
    def _view(self,command_id,digest):
        snapshot=self._snapshot();command=self._journal.lookup(command_id,digest)
        _need(command is not None,'RECOVERY_COMMAND_UNKNOWN')
        bundle=self._journal.read_selected_bundle();selection=self._journal.selection_facts()
        _need(canonical_bytes(snapshot['selected']['bundle_manifest'])==bundle.manifest_bytes
              and _same(selection,{'selector':snapshot['selected']['selector'],
                                  'selector_version':snapshot['selected']['selector_version']}),'RECOVERY_SELECTION_CHANGED')
...
def _fold_recovery(publication,tail):
    ...
    result={'attempts':[],'blocked_original_commands':[],'held':False,'selected':None,'last_observed_ms':snapshot['last_observed_ms']}
    ...
    for index,raw in enumerate(tail):
        ...
        if kind=='HOLD':
            ...
            if result['attempts']:result['attempts'][-1]['phase']='HELD'
        ...
        attempt.update(phase='TERMINAL',**expected)
    return result
...
class RecoveryJournal(_v4.PublicationJournalV4):
    ...
    def _validate(self,records):
        ...
        return publication,recovery,events,selected
    ...
    def _refresh(self):
        self._verify_owners()
        publication,recovery,events,selected=self._validate(self._read_unbound())
    ...
    def snapshot(self):
        with self._locked():
            ...
            return {**publication.snapshot(),'selected':selected,'recovery':recovery,'journal_read_only':True,
    ...
    def authority_context(self):
        ...
        with self._locked():
            publication,recovery,selected=self._refresh();snapshot=publication.snapshot()
    ...
    def admit(self,command_id,digest,*,recovery_id,admission,observed_ms):
        ...
        with self._locked():
            ...
            _need(len(tail)+3<=MAX_RECOVERY_EVENTS,'RECOVERY_TERMINAL_RESERVE')
...
class ReconciliationAuthority:
    ...
    def bind(self,owner,command_id,digest,*,deadline_ms):
        with self._mutex:
            ...
            context=owner.authority_context();snapshot=owner._journal.snapshot()
            command=owner._journal.lookup(command_id,digest)
            _need(command is not None,'RECOVERY_COMMAND_UNKNOWN')
