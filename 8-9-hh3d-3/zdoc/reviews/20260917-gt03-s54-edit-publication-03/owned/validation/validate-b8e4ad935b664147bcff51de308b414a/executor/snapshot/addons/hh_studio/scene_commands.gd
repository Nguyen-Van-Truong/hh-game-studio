...
## Shared read-only value observation. This class has no editor authorization,
## mutation, history, or save API. Live callers must retain their context guards.
class StoredSceneSnapshot extends RefCounted:
	var _root: Node3D
	var _scan_error: String = ""
	var _nodes: Dictionary = {}
	var _scan_meshes: Array[BoxMesh] = []

	func capture(root: Node3D) -> Dictionary:
		if not _on_main():
	...
	func _walk(node: Node, parent_id: String, depth: int, rows: Array[Dictionary]) -> void:
		if not _scan_error.is_empty():
			return
		if node.is_queued_for_deletion() or depth > MAX_DEPTH or rows.size() >= MAX_NODES or node.get_class() not in ["Node3D", "MeshInstance3D"]:
		...
		if node is MeshInstance3D:
			var mesh: BoxMesh = (node as MeshInstance3D).mesh as BoxMesh
			if _scan_meshes.has(mesh):
				_scan_error = "SCENE_SHARED_MESH_UNSUPPORTED"
				return
			_scan_meshes.append(mesh)
	...
	func _stored_value(value: Variant, depth: int) -> Variant:
		if depth > 16:
			_scan_error = "SCENE_RESOURCE_LIMIT"
			return null
		# Unset typed native object properties retain TYPE_OBJECT even when null.
		# Check nullness before any Resource/Script inspection. A non-null invalid
		# Object remains a rejection, never an empty resource snapshot.
		if typeof(value) == TYPE_OBJECT:
			if value == null:
				return null
			if not is_instance_valid(value):
				_scan_error = "SCENE_INVALID_STORED_OBJECT"
				return null
		...
		if value is Resource:
			if not value is BoxMesh:
				...
				return null
			if not _local_mesh(value as BoxMesh):
				_scan_error = "SCENE_EXTERNAL_MESH_UNSUPPORTED"
				return null
		...
		match typeof(value):
			...
			TYPE_VECTOR2, TYPE_VECTOR2I:
				return {"type": typeof(value), "value": _stored_value([value.x, value.y], depth + 1)}
			TYPE_VECTOR3, TYPE_VECTOR3I:
				return {"type": typeof(value), "value": _stored_value([value.x, value.y, value.z], depth + 1)}
			TYPE_VECTOR4, TYPE_VECTOR4I, TYPE_QUATERNION:
				return {"type": typeof(value), "value": _stored_value([value.x, value.y, value.z, value.w], depth + 1)}
			TYPE_COLOR:
				return {"type": typeof(value), "value": _stored_value([value.r, value.g, value.b, value.a], depth + 1)}
			TYPE_BASIS:
				return {"type": typeof(value), "value": _stored_value([value.x, value.y, value.z], depth + 1)}
			TYPE_TRANSFORM3D:
				return {"type": typeof(value), "value": _stored_value([value.basis, value.origin], depth + 1)}
			TYPE_TRANSFORM2D:
				return {"type": typeof(value), "value": _stored_value([value.x, value.y, value.origin], depth + 1)}
			TYPE_AABB, TYPE_RECT2, TYPE_RECT2I:
				return {"type": typeof(value), "value": _stored_value([value.position, value.size], depth + 1)}
			TYPE_PACKED_BYTE_ARRAY, TYPE_PACKED_INT32_ARRAY, TYPE_PACKED_INT64_ARRAY, TYPE_PACKED_FLOAT32_ARRAY, TYPE_PACKED_FLOAT64_ARRAY, TYPE_PACKED_STRING_ARRAY, TYPE_PACKED_VECTOR2_ARRAY, TYPE_PACKED_VECTOR3_ARRAY, TYPE_PACKED_VECTOR4_ARRAY, TYPE_PACKED_COLOR_ARRAY:
				...
				for item: Variant in value:
					packed_items.append(_stored_value(item, depth + 1))
				return {"type": typeof(value), "items": packed_items}
			_:
	...
	func _node_signature(node: Node3D) -> String:
		...
		var row: Dictionary = {"class": node.get_class(), "stored": _stored_properties(node, 0),
			"editable": _editable_values(node), "child_count": node.get_child_count(true), "groups": groups}
	...
	func _resource_signature(mesh: BoxMesh) -> String:
		_scan_error = ""
		if not is_instance_valid(mesh) or not _local_mesh(mesh):
			return ""
		var canonical: Dictionary = JCS.new().canonicalize(_stored_properties(mesh, 0))
		return canonical.sha256 if canonical.ok and _scan_error.is_empty() else ""


	func _local_mesh(mesh: BoxMesh) -> bool:
		return mesh.resource_path.is_empty() or mesh.resource_path.begins_with(SCENE_PATH + "::")
...
func _transition(record: Dictionary, forward: bool) -> void:
	...
	if not snapshot.ok or snapshot.revision != expected:
		...
		_blocked = true
		return
	if not is_instance_valid(record.node) or not is_instance_valid(record.parent):
	...
	if attach:
		# A semantic revision cannot distinguish a replaced parent with identical
		# properties. Resolve its identity again before reattaching a retained node.
		if _nodes.get(str(parent.get_meta(ID_META, ""))) != parent or record.owner != _root:
			_last = _failure("SCENE_HISTORY_OBJECT_CHANGED")
			_blocked = true
			return
		if node.get_parent() != null or _node_signature(node) != record.detached_signature:
			...
			_blocked = true
			return
		if node is MeshInstance3D and _scan_meshes.has((node as MeshInstance3D).mesh):
			_last = _failure("SCENE_SHARED_MESH_UNSUPPORTED")
			_blocked = true
			return
	...
	if record.kind == "update" and node is MeshInstance3D:
		...
		if expected_mesh.is_empty() or _resource_signature(mesh) != expected_mesh:
			...
			_blocked = true
			return
		if _scan_meshes.has(mesh) and (node as MeshInstance3D).mesh != mesh:
			_last = _failure("SCENE_SHARED_MESH_UNSUPPORTED")
			_blocked = true
			return
	...
	if not after.ok or not _effect_verified(record, snapshot, after, forward):
		_blocked = true
		_last = _failure("SCENE_POSTCONDITION_UNKNOWN")
		return
...
func _effect_verified(record: Dictionary, before: Dictionary, after: Dictionary, forward: bool) -> bool:
	if not is_instance_valid(record.node) or not is_instance_valid(record.parent):
		return false
	var node: Node3D = record.node as Node3D
	var parent: Node = record.parent as Node
	var stable_id: String = str(node.get_meta(ID_META, ""))
	var parent_id: String = str(parent.get_meta(ID_META, ""))
	var attach: bool = (record.kind == "create" and forward) or (record.kind == "remove" and not forward)
	var detach: bool = (record.kind == "remove" and forward) or (record.kind == "create" and not forward)
	if detach:
		if node.get_parent() != null or _nodes.has(stable_id):
			return false
	else:
		if _nodes.get(stable_id) != node or (attach and (node.get_parent() != parent or node.owner != record.owner)):
			return false
		var desired: Dictionary = record.after_values if forward else record.before_values
		if _editable_values(node) != desired:
			return false
	var expected_others: Array[Dictionary] = []
	var actual_others: Array[Dictionary] = []
	for original: Dictionary in before.state.nodes:
		if original.stable_id == stable_id:
			continue
		var row: Dictionary = original.duplicate(true)
		if row.parent_id == parent_id:
			if detach and int(row.sibling_index) > int(record.index):
				row.sibling_index = int(row.sibling_index) - 1
			elif attach and int(row.sibling_index) >= int(record.index):
				row.sibling_index = int(row.sibling_index) + 1
		expected_others.append(row)
	for row: Dictionary in after.state.nodes:
		if row.stable_id != stable_id:
			actual_others.append(row)
	return expected_others == actual_others
...
func _snapshot() -> Dictionary:
	var serializer := StoredSceneSnapshot.new()
	var result: Dictionary = serializer.capture(_root)
	_nodes = serializer._nodes
	_scan_meshes = serializer._scan_meshes
	return result


func _node_signature(node: Node3D) -> String:
	return StoredSceneSnapshot.new()._node_signature(node)


func _resource_signature(mesh: BoxMesh) -> String:
	return StoredSceneSnapshot.new()._resource_signature(mesh)


func _editable_values(node: Node3D) -> Dictionary:
	return StoredSceneSnapshot.new()._editable_values(node)
...
func _set_values(node: Node3D, values: Dictionary) -> void:
	...
	# Node3D setters cache Euler/scale inputs, while transform assignment and
	# scene reload derive them from the float32 basis. Normalize that cache now
	# without changing the stored transform, before taking exact history facts.
	# Undo/redo can then restore the same complete semantic snapshot exactly.
