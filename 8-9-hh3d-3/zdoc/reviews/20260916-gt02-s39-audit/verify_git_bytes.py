...
PACK='zdoc/reviews/20260916-gt02-s39-02/'
AUDIT='zdoc/reviews/20260916-gt02-s39-audit/'
...
def main():
    ...
    candidate=json.loads(blob(PACK+'candidate.json')); closure=json.loads(blob(PACK+'source-closure.json'))
    expected={'studio/'+n:d for n,d in closure['files'].items()}
    expected.update({PACK+n:d for n,d in candidate['artifacts'].items()})
    expected[PACK+'candidate.json']=hashlib.sha256(blob(PACK+'candidate.json')).hexdigest()
    expected[AUDIT+'diagnostic-manifest.json']=hashlib.sha256(blob(AUDIT+'diagnostic-manifest.json')).hexdigest()
    dm=json.loads(blob(AUDIT+'diagnostic-manifest.json')); expected.update(dm['files'])
    ...
    verifier=blob(AUDIT+'verify_evidence.py')
