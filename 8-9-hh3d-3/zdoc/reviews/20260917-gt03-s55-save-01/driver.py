...
STUDIO = Path('D:\\dataDiskD\\intellji\\hoanhaosocial\\hoanhaonew-20-6-2025\\hh-game-studio\\8-9-hh3d-3\\zdoc\\reviews\\20260917-gt03-s55-source-01\\source\\studio')
...
def frozen(output, binary, closure):
    ...
    try:
        ...
        def call(path, body, *, control=False, bearer=None):
            connection = http.client.HTTPConnection('127.0.0.1',server.stop_port if path == '/v1/stop' else server.control_port if control else server.port,timeout=95)
            try:
        ...
        save(output/'events.json',[parse_json(raw) for raw in owner._journal._state.events])
        ...
        check('readonly_reopen_original_response',reopened.lookup_response(request.command_id,request.digest)
...
def main():
    ...
    host=runner.run_process([sys.executable,'-B',str(source/'tests/godot/run_publication_probe.py'),
        ...
        cwd=source,output=output,timeout=180,label='publication',env=environment)
