...
no design critic verdict has been ACCEPT from S6 to S20; the tools plan pins Godot 4.7.2-stable
...
def validate(manifest, inputs, root):
    ...
    for index, name in enumerate(NAMES):
        ...
        check(re.findall(r"(?m)^PLAN_DESIGN_CRITICS=(.+)$",s)==["NO_ACCEPTED_VERDICT_S6_TO_S20"], name+": PLAN_DESIGN_CRITICS header states no accepted design verdict")
    ...
    tools_policy=policy.replace("NO_CODEX_WORKER/", "AUX_WORKER=codex-gpt-5.6-sol-high; AUX_SCOPE=readonly-audit|isolated-helper<=3-files; ")
    check(t.count(tools_policy)==1 and g.count(policy)==1 and all("--no-subagents" in x for x in (t,g)),
          "S20 exact official Grok policy and bounded Codex auxiliary")
    ...
    check(len(pending)==1 and set(pending[0].split(","))=={"TQ01_TX12_TX14","TWO_CRITICS"},
    ...
    check("CAPACITY_CLAIMS=GAME_P8_03_MEASURED_SIGNED_CURRENT_ONLY" in specs["H2-P8-03"]
          ...
          "S20 capacity claims belong to measured game release")
