...
import validate_plans_s20 as validator
...
def manifest_for(inputs):
    ...
    return {'revision': 'S20', 'files': rows, 'manifest_sha256': digest}
...
class S20Tests(unittest.TestCase):
    ...
    def test_design_critics_header_cannot_claim_accept(self):
        self.check_mutation(0, 'PLAN_DESIGN_CRITICS=NO_ACCEPTED_VERDICT_S6_TO_S20', 'PLAN_DESIGN_CRITICS=ACCEPTED', 'PLAN_DESIGN_CRITICS')

    def test_design_critics_header_required_in_game_plan(self):
        self.check_mutation(1, 'PLAN_DESIGN_CRITICS=NO_ACCEPTED_VERDICT_S6_TO_S20\n', '', 'PLAN_DESIGN_CRITICS')
    ...
    def test_auxiliary_scope_cannot_expand(self):
        self.check_mutation(0, 'AUX_SCOPE=readonly-audit|isolated-helper<=3-files', 'AUX_SCOPE=unrestricted', 'Codex auxiliary')
    ...
    def test_pending_remint_is_required(self):
        self.check_mutation(0, 'GT01_PENDING=TQ01_TX12_TX14,TWO_CRITICS', 'GT01_PENDING=TWO_CRITICS', 'pending header')

    def test_pending_evidence_is_required(self):
        self.check_mutation(0, 'GT01_PENDING=TQ01_TX12_TX14,TWO_CRITICS', 'GT01_PENDING=TQ01_TX12_TX14', 'pending header')
...
if __name__ == '__main__':
    ...
    report = {'result': 'PASS' if result.wasSuccessful() else 'FAIL', 'tests_run': result.testsRun,
              ...
              'manifest_sha256': manifest_for(S20Tests.inputs)['manifest_sha256'],
              'limits': 'Static and rehashed mutation checks only; no runtime/critic/legal acceptance.'}
    (validator.OUT / 'mutation-s20.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
