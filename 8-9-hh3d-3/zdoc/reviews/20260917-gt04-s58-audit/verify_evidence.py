"""File-only S58 fixed-profile audit; each process imports one frozen closure."""
...
PACKAGE = REVIEWS / ('20260917-gt04-s58-' + PROFILE + ('-02' if PROFILE == 'scene' else '-01'))
SOURCE = PACKAGE / 'source/studio'
CLOSURE = '0785f748ee93882e38f4b261a6db3764271c7fbdb84508e6df00231df4e19efc'
COUNT = 571 if PROFILE == 'scene' else 305
...
def verify():
    ...
    calls = client['calls']
    need(len(calls) == 34 and len(client['checks']) == 47 and len(report['checks']) == 10, 'bounded full integration trace')
    values = []
    for call in calls:
    ...
    return {'passed':True,'formal_acceptance':False,'source_closure_sha256':CLOSURE,'source_files':len(manifest['files']),
        'unit_tests':COUNT,'client_checks':47,'profile':PROFILES[PROFILE],'native_checks':10,'http_calls':len(calls),'common_commands':len(history),
        'preview_operations':len(previews),'assertions':len(base.CHECKS),'native_handles_opened':0,
        'scope':'GT04 S58 fixed-profile component integration; final matrix and two critics still pending'}
