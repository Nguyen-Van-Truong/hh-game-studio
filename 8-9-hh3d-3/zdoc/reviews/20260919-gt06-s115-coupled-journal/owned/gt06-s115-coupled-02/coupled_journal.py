"""One original-gate, stock-native prefix with additive host journal timings.
...
def boundary_screen(original, sample, baseline, gate_rows):
    ...
    if len(gate_rows) == PREFIX_BATCHES:
        raise Boundary('S115_BOUNDARY_CAPTURED')
...
def check(root=ROOT):
    ...
    util = load(support_path, 's115_support')
    campaign, _, _, sources = util.load_campaign(root)
    util.need(campaign.closure(sources) == CLOSURE, 'S115_SOURCE_DRIFT')
    util.need(util.sha(root / 'studio/tests/replay/benchmark_native.gd') == NATIVE_HASH, 'S115_NATIVE_DRIFT')
    ...
    util.need(util.sha(binary) == lock['gui_sha256'], 'S115_BINARY_DRIFT')
...
def launch():
    ...
    try:
        ...
        while owner.tick() is None:
            util.need(time.monotonic() - started < OUTER_SECONDS, 'S115_OUTER_LIMIT')
            time.sleep(.1)
        ...
        for name in ('source_unchanged', 'execution_unchanged'):
            if observations.get(name) is not True:
                errors.append((name, RuntimeError('S115_PIN_CHANGED')))
        util.write(run / 'result.json', {'schema': 's115.coupled-journal.result.1', 'run_id': RUN_ID,
...
def child(run):
    ...
    with ExitStack() as stack:
        ...
        try:
            ...
            errors.append(('unexpected_full_run', RuntimeError('S115_BOUNDARY_NOT_REACHED')))
    ...
    util.write(run / 'timing-summary.json', {'schema': 's115.coupled-journal.timings.1',
        ...
        'limits': 'Nested wall/CPU spans overlap; Python fsync excludes SQLite internal sync. No I/O/rootcause/no-leak claim.',
