    ...
    def dispatch(self,lane,value):
        ...
        try:
            if lane=='control':
                ...
                need(kind=='submit','IPC_DATA_OPERATION')
                exact(body,('command','ttl_ms','lease') if 'lease' in body else ('command','ttl_ms'))
                if getattr(self,'recovery',None) is not None and (type(body['command']) is not dict
                    or body['command'].get('operation')!='scene.inspect' or body.get('lease') is not None):
                    raise self.ui.c.Rejected('RECOVERY_READONLY')
                result=self.owner.queue.submit(self.ui.c.canonical(body['command']),ttl_ms=body['ttl_ms'],lease=body.get('lease'))
