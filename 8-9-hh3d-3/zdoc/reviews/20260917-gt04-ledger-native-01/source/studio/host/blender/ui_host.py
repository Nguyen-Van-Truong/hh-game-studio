...
class BlenderUIHost:
    ...
    def submit(self,command,*,ttl_ms=5000,lease=None):
    ...
    def execute(self,command,*,timeout=6,lease=None):
        need(type(timeout) in (int,float) and 0<timeout<=10,'BLENDER_TIMEOUT_LIMIT')
        row=self.submit(command,lease=lease); deadline=time.monotonic()+timeout
