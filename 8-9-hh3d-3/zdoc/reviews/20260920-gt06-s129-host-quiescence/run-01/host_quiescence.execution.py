...
def child(out):
    ...
    events, samples, batches, workers = [], [], [], []
    active = {}
    lock = threading.Lock()
    sequence = 0
    phase = 'baseline'
    ...
    def request_thread(listener, request, client_address):
        ...
        with lock:
            ...
            # Private join ownership only; logs contain no thread references.
    ...
    def sample(label, producer=None):
        ...
        with lock:
            row = dict(label=label, phase=phase, mono_ns=time.perf_counter_ns(),
                       ...
                       request_threads_alive=sum(thread.is_alive() for thread in workers),
    ...
    def join_workers():
        ...
        need(not any(thread.is_alive() for thread in workers), 'REQUEST_THREAD_JOIN_DEADLINE')
    ...
    result = dict(schema='hh3d.s129.host-quiescence.diagnostic.v1', utc=stamp(),
        ...
        active_requests_final=len(active), request_threads_created=len(workers),
        request_threads_joined=sum(not thread.is_alive() for thread in workers),
    ...
    print(json.dumps(dict(status=status, elapsed_seconds=result['elapsed_seconds'],
                         request_threads_created=len(workers), active_requests_final=len(active))))
