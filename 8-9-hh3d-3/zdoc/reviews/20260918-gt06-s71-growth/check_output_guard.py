"""Fresh copied native fixture with deliberately wrong Output setting; no cycles."""
...
root = n.STUDIO / '.local/reviews/gt06-s71-output-guard-01'
...
assert raw.count(b'run/output/max_lines=100') == 1
config.write_bytes(raw.replace(b'run/output/max_lines=100', b'run/output/max_lines=101'))
...
n.write(root / 'diagnostic.json', {'source_files': source, 'initial_project_files': initial,
    ...
    'formal_acceptance': False, 'expected_failure': 'BENCHMARK_OUTPUT_LOG_LIMIT'})
...
try:
    ...
    raise AssertionError('Wrong setting was not rejected')
...
assert len(failed) == 1 and failed[0]['code'] == 'BENCHMARK_OUTPUT_LOG_LIMIT'
...
print('OUTPUT_GUARD_REJECTED_NATIVE_86_CLEAN_TREE', flush=True)
