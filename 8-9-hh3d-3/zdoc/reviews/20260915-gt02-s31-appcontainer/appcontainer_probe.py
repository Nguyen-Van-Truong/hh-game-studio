...
RUN_ID = "GT02-S31-APPCONTAINER-04"
SCRIPT = r'''@echo off
...
def launch(exe, scratch, mode, package=None):
    ...
    try:
        ...
        out["environment_names"] = sorted(values, key=str.lower)
        ...
        for attempt in range(101):
            ...
            out["job_drain_samples"].append(int(accounting.active_processes))
        ...
        process_list = PIDLIST()
        checked(K.QueryInformationJobObject(job, 3, C.byref(process_list), C.sizeof(process_list), None))
        out["job_process_ids_at_final"] = list(process_list.pids)[:process_list.count]
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="gt02-s31-appcontainer-") as tmp:
            ...
            try:
                ...
                out["profile_folder_api_hresult"] = "0x%08X" % (hr & 0xffffffff)
                out["profile_api_path"] = folder.value
                # Microsoft documents LOCALAPPDATA for this profile as <root>/AC.
