...
def validate_request(value, campaign_id, mode, script, windowed, console):
...
def execute(campaign_id, mode, output, request_sha):
    ...
    try:
        ...
        binding = {'run_id': campaign_id + '.probe',
...
def main():
    ...
    output = STUDIO / '.local/reviews' / (args.campaign_id + '-supervisor')
    ...
    with (output / 'stdout.txt').open('x', encoding='utf-8', buffering=1) as stdout, \
        ...
        try:
            ...
            write(output / 'start.json', {**observation, 'campaign_id': args.campaign_id,
            ...
            code = execute(args.campaign_id, args.mode, output, sha(request))
            validate_request(value, args.campaign_id, args.mode, Path(__file__), windowed, console)
        except BaseException as error:
            code = 1
            traceback.print_exc()
            write(output / 'failure.json', {'code': getattr(error, 'code', type(error).__name__),
                'formal_acceptance': False})
        finally:
            write(output / 'return.json', {'returned_exit_code': code,
