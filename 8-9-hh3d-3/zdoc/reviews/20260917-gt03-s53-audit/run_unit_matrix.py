...
def main():
    ...
    inventory=load('s53_unit_inventory',STUDIO/'tests/godot/run_editor_probe.py')
    ...
    runner=load('s53_unit_runner',source/'build/bootstrap/run_fixture.py')
...
journal=sys.argv[1]=='journals'
chosen=[case for case in all_tests if case.id().startswith('test_publication_journal')==journal]
...
"""
    save(output/'invocation.json',{'code':code,'partitions':['journals','other'],
        'runner_sha256':sha(source/'build/bootstrap/run_fixture.py'),'controller_sha256':sha(Path(__file__))})
    def lane(label):
        host=runner.run_process([sys.executable,'-B','-c',code,label],cwd=source,output=output,timeout=600,label=label)
        ...
        save(output/(label+'-result.json'),result);print(json.dumps({'lane':label,**result['counts']}),flush=True)
        return result
    with ThreadPoolExecutor(max_workers=2) as pool:
        lanes=list(pool.map(lane,('journals','other')))
    a,b=(row['inventory'] for row in lanes)
    partition=bool(a and b and a['all']==b['all'] and not set(a['chosen'])&set(b['chosen'])
        and set(a['chosen'])|set(b['chosen'])==set(a['all'])
        and all(row['counts']['run']==len(row['inventory']['chosen']) for row in lanes))
