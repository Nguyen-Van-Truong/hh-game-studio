...
Because this version owns only bootstrap selection, every refresh/reopen also
requires actual selector bytes and FileVersion still match the durable record.
Deleted/recreated/same-byte-but-new-identity selectors fail closed. A future
publisher must explicitly extend that expected-selector chain.
