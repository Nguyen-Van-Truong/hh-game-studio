...
class EditorOwner:
    ...
    def __init__(self, evidence_parent: Path, initial_bundle, *, editor_binary: Path):
        ...
        try:
            ...
            _need(set(hello) == {'kind', 'token', 'editor_session_id', 'pid', 'project_root', 'editor_hint', 'main_thread', 'version'}
                  ...
                  and hello['version'].startswith('4.7.2.stable.official.ed1daf0bf')
    ...
    def _intent(self, value, kind):
        _need(type(value) is EditorEffectIntent and value.kind == kind and self._pending is value, 'EDITOR_REGISTERED_INTENT_REQUIRED')
    ...
    def _receipt(self, receipt, kind=None):
        self._healthy()
        _need(type(receipt) in (EditorCaptureReceipt, EditorAdoptionReceipt) and (kind is None or type(receipt) is kind), 'EDITOR_RECEIPT_TYPE')
