...
class BlenderPublicationOwner:
    ...
    def preview(self,request,lease,*,deadline_ms,phase_guard=None):
        request=model.validate_request(request)
        need(type(deadline_ms) is int,'PUBLICATION_PREVIEW_DEADLINE_REQUIRED')
        need(phase_guard is None or callable(phase_guard),'PUBLICATION_PHASE_GUARD_REQUIRED')
        budget=AbsoluteDeadline(deadline_ms,host_deadline=self.host._deadline)
        def check():
            budget.check();self._authorize(lease)
            if phase_guard is not None:phase_guard()
            budget.check()
        with self._mutex:
            check();state=self._refresh()
            need(state['phase']=='EMPTY','PUBLICATION_ONE_BUNDLE_CAPACITY')
            self.files.check_mutation_available()
            need({p.name for p in self.files.root.iterdir()}=={'.writer'},'PUBLICATION_INITIAL_NAMESPACE')
            command=model.preparation_command(request,state['config'])
            check()
            native=self.host.preview(command,lease={'fencing_epoch':lease.fencing_epoch,'expires_ms':lease.expires_ms},
                                     deadline_ms=deadline_ms,ttl_ms=5000)
            check()
            return {'schema':'HH-BLENDER-PUBLICATION-PREVIEW-1','command_id':request['command_id'],
                'request_sha256':model.sha(canonical_bytes(request)), 'profile':model.profile(state['config']),
                'storage_id':self.storage_id,'native_command':command,'native_preview':native,
