...
class BlenderClientSession:
    def __init__(self,project_id,root,*,owner_deadline_ms,catalog_digest=catalog.CATALOG_DIGEST,
        ...
        # A caller cannot invent operations/limits by supplying a catalog-shaped
        # object. The optional writer profile is a separate trusted module; the
        # default issuer and its empty core fixture scopes remain unchanged.
        if catalog_profile is not catalog:
            from . import client_write_catalog
            need(catalog_profile is client_write_catalog,'BLENDER_TRUSTED_CATALOG_REQUIRED')
        need(type(project_id) is str and re.fullmatch(r'[a-z][a-z0-9._-]{0,63}',project_id),'BLENDER_INVALID_PROJECT')
        need(catalog_digest==catalog_profile.CATALOG_DIGEST,'BLENDER_CATALOG_MISMATCH')
        ...
        self._issuer=SessionAuthority(project_id,Path(root),TransportLimits(max_sessions=8,max_session_history=32,
            max_session_ttl_ms=120000,max_pending=1,max_response_bytes=catalog_profile.MAX_WIRE))
    ...
    def issue(self,*,operations=catalog.OPERATIONS,ttl_ms=60000):
        need(type(operations) is frozenset and operations and operations<=self._catalog.OPERATIONS,'BLENDER_INVALID_GRANT')
        with self._mutex:
    ...
    def rotate(self,credential,*,ttl_ms=60000):
        with self._mutex:
            ...
            self._invalidate_leases(grant.session_id)
            return rotated
    ...
    def revoke(self,credential):
        with self._mutex:
            need(type(credential) is SessionCredential,'BLENDER_CREDENTIAL_REQUIRED')
            grant=self.authenticate('Bearer '+credential.bearer)
            self._issuer.revoke(credential);self._grants.pop(grant.session_id);self._invalidate_leases(grant.session_id)
            return {'revoked':True,**self._activity(),'public_ack':False}

    def _invalidate_leases(self,session_id):
        self._leases.pop(session_id,None)

    def _activity(self):
        return {'read_draining':self._active is not None}

    def read_lease(self,grant,*,ttl_ms):
        need(type(ttl_ms) is int and 0<ttl_ms<=self._catalog.MAX_LEASE_MS,'BLENDER_INVALID_LEASE')
        with self._mutex:
    ...
    def check_read(self,grant,lease,*,deadline_ms):
        with self._mutex:
            ...
            self._check_deadline(lease,deadline_ms,self._catalog.MAX_READ_MS)

    def _check_deadline(self,lease,deadline_ms,maximum_ms):
        now=epoch_ms()
        need(type(deadline_ms) is int and now<deadline_ms<=min(lease.expires_ms,self._owner_deadline_ms,now+maximum_ms),
    ...
    def stop(self,grant):
        with self._mutex:
            ...
            return {'stopped':True,**self._activity(),'public_ack':False}
