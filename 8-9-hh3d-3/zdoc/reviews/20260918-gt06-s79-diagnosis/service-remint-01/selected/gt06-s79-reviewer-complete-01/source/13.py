"""Bounded main-thread queue; no transport, threads, bpy, or public ACK."""
from collections import deque
import importlib.util
import json
from pathlib import Path
import time

_path = Path(__file__).with_name("adapter.py")
_spec = importlib.util.spec_from_file_location("_gt04_ui_base", _path)
base = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(base)
c = base.contract
SCHEMA = "HH-BLENDER-UI-COMMAND-1"
MAX_PENDING = 8
MAX_REQUESTS = 64
PREVIEW_SCHEMA = "HH-BLENDER-UI-PREVIEW-1"
MAX_PREVIEW_BYTES = 32768
PREVIEW_OPERATIONS = frozenset({
    "mesh.create_box", "object.transform.set", "material.set_principled", "history.undo", "history.redo",
}) | {'checkpoint.save', 'export.prepare'}


def parse(raw):
    if type(raw) is not bytes or not 0 < len(raw) <= c.MAX_BYTES:
        raise c.Rejected("BOUNDED_BYTES_REQUIRED")
    try:
        value = json.loads(raw.decode("utf-8"), object_pairs_hook=c._pairs,
                           parse_constant=lambda _: (_ for _ in ()).throw(c.Rejected("NONFINITE_JSON")))
        c.exact(value, ("schema", "command_id", "operation", "expected_revision", "expected_context", "payload"), "command")
        if value["schema"] != SCHEMA:
            raise c.Rejected("UI_SCHEMA_REQUIRED")
        check = dict(value, schema=c.SCHEMA)
        if value["operation"] in ("history.undo", "history.redo"):
            c.exact(value["payload"], (), "history")
            # Reuse the exact revision/context/identifier validation, without
            # extending the background adapter's admitted operations.
            check.update(operation="mesh.create_box", payload={"object_id": "history", "size": [1, 1, 1]})
        elif value["operation"] in ("checkpoint.save", "export.prepare"):
            c.exact(value["payload"], ("slot",), "checkpoint")
            slots = ("export",) if value["operation"] == "export.prepare" else ("checkpoint", "fixture")
            if value["payload"]["slot"] not in slots:
                raise c.Rejected("FIXED_SAVE_SLOT_REQUIRED")
            check.update(operation="mesh.create_box", payload={"object_id": "save", "size": [1, 1, 1]})
        c.validate(check)
        return json.loads(c.canonical(value))
    except (ValueError, UnicodeError, TypeError, RecursionError, OverflowError) as exc:
        raise c.Rejected("INVALID_UI_COMMAND") from exc


class CommandQueue:
    """Caller is a trusted main-thread poller. Entries/receipts live for this owner only."""
    def __init__(self, dispatch, *, preview=None, clock=time.monotonic, lease_clock=lambda:time.time_ns()//1_000_000):
        base.main_thread()
        if preview is not None and not callable(preview):
            raise c.Rejected("TRUSTED_PREVIEW_CALLBACK_REQUIRED")
        self._dispatch, self._clock = dispatch, clock
        self._preview_callback = preview
        self._pending, self._rows = deque(), {}
        self._stopped = False
        self._lease=None;self._lease_clock=lease_clock

    def arm_lease(self,lease):
        base.main_thread()
        c.exact(lease,('fencing_epoch','expires_ms'),'lease')
        if (type(lease['fencing_epoch']) is not int or not 1<=lease['fencing_epoch']<2**53
            or type(lease['expires_ms']) is not int or not self._lease_clock()<lease['expires_ms']<2**53):
            raise c.Rejected('INVALID_WRITER_LEASE')
        if self._stopped:raise c.Rejected('STOPPED')
        if self._lease is not None:
            if lease==self._lease:return dict(self._lease)
            if lease['fencing_epoch']<=self._lease['fencing_epoch']:raise c.Rejected('STALE_WRITER_FENCE')
        self._lease=dict(lease)
        return dict(self._lease)

    def _check_lease(self,lease,*,read_only=False):
        # Authenticated inspect is independently read-authorized by the host.
        # An explicit writer lease still requests the full fence check; only
        # an absent lease on the exact non-mutating operation is exempt.
        if read_only and lease is None:return
        if lease is not None:
            c.exact(lease,('fencing_epoch','expires_ms'),'lease')
            if any(type(lease[name]) is not int for name in lease):raise c.Rejected('INVALID_WRITER_LEASE')
        if self._lease is None:
            if lease is not None:raise c.Rejected('WRITER_LEASE_NOT_ARMED')
        elif lease!=self._lease or self._lease_clock()>=self._lease['expires_ms']:
            raise c.Rejected('WRITER_FENCED_OR_EXPIRED')

    @staticmethod
    def _deadline_syntax(ttl_ms, deadline_ms):
        if type(ttl_ms) is not int or not 1 <= ttl_ms <= 5000:
            raise c.Rejected("DEADLINE_LIMIT")
        if deadline_ms is not None and (type(deadline_ms) is not int or not 1 <= deadline_ms < 2**53):
            raise c.Rejected("ABSOLUTE_DEADLINE_LIMIT")

    def _fresh_guard(self, command, lease, ttl_ms, deadline_ms):
        if self._stopped:
            raise c.Rejected("STOPPED")
        self._check_lease(lease,read_only=command['operation']=='scene.inspect')
        remaining_ms = ttl_ms
        if deadline_ms is not None:
            remaining_ms = min(remaining_ms, deadline_ms - self._lease_clock())
            if remaining_ms <= 0:
                raise c.Rejected("COMMAND_DEADLINE")
        if len(self._pending) >= MAX_PENDING or len(self._rows) >= MAX_REQUESTS:
            raise c.Rejected("QUEUE_CAPACITY")
        return remaining_ms

    def submit(self, raw, *, ttl_ms=1000, lease=None, deadline_ms=None):
        base.main_thread()
        command = parse(raw)
        self._deadline_syntax(ttl_ms, deadline_ms)
        key, digest = command["command_id"], c.digest(command)
        if key in self._rows:
            if self._rows[key]["command_digest"] != digest:
                raise c.Rejected("COMMAND_CONFLICT")
            return self.result(key)
        remaining_ms = self._fresh_guard(command, lease, ttl_ms, deadline_ms)
        self._rows[key] = {"command_id": key, "command_digest": digest, "state": "PENDING", "public_ack": False}
        # Preserve the absolute caller deadline and a monotonic bound. A late
        # IPC arrival cannot renew it, and a wall-clock rollback cannot extend
        # the lifetime already admitted by this main-thread queue.
        self._pending.append((key, command, self._clock() + remaining_ms / 1000,
                              dict(lease) if lease is not None else None, deadline_ms))
        return self.result(key)

    def preview(self, raw, *, lease, deadline_ms, ttl_ms=1000):
        """Synchronous trusted observation; never reserve a future apply ID."""
        base.main_thread()
        command = parse(raw)
        self._deadline_syntax(ttl_ms, deadline_ms)
        if deadline_ms is None:
            raise c.Rejected("ABSOLUTE_DEADLINE_REQUIRED")
        if command["operation"] not in PREVIEW_OPERATIONS or self._preview_callback is None:
            raise c.Rejected("PREVIEW_OPERATION_UNSUPPORTED")
        if lease is None or self._lease is None:
            raise c.Rejected("PREVIEW_WRITER_LEASE_REQUIRED")
        remaining_ms = self._fresh_guard(command, lease, ttl_ms, deadline_ms)
        if command["command_id"] in self._rows:
            raise c.Rejected("PREVIEW_APPLY_ID_EXISTS")
        monotonic_deadline = self._clock() + remaining_ms / 1000
        try:
            result = self._preview_callback(command)
            if type(result) is not dict:
                raise c.Rejected("PREVIEW_OBJECT_REQUIRED")
            encoded = c.canonical(result)
            if len(encoded) > MAX_PREVIEW_BYTES:
                raise c.Rejected("PREVIEW_OUTPUT_LIMIT")
            detached = json.loads(encoded)
            # A clock rollback cannot renew the initially admitted budget.
            if self._clock() >= monotonic_deadline:
                raise c.Rejected("PREVIEW_DEADLINE")
            self._fresh_guard(command, lease, ttl_ms, deadline_ms)
            return detached
        except c.Rejected:
            raise
        except BaseException:
            self.stop()
            raise

    def result(self, key):
        base.main_thread()
        return json.loads(c.canonical(self._rows[c.identifier(key)]))

    def tick(self):
        base.main_thread()
        if not self._pending or self._stopped:
            return
        key, command, deadline, lease, absolute_deadline = self._pending.popleft()
        row = self._rows[key]
        if self._clock() >= deadline:
            row.update(state="EXPIRED", reason="QUEUE_DEADLINE")
            return
        try:
            self._check_lease(lease,read_only=command['operation']=='scene.inspect')
            if absolute_deadline is not None and self._lease_clock() >= absolute_deadline:
                row.update(state="EXPIRED", reason="COMMAND_DEADLINE")
                return
            row.update(result=self._dispatch(command), state="COMPLETED")
        except c.Rejected as exc:
            row.update(state="REJECTED", reason=str(exc))
        except BaseException as exc:
            row.update(state="HELD", reason=type(exc).__name__)
            self.stop()
            raise

    def stop(self):
        base.main_thread()
        self._stopped = True
        while self._pending:
            key, _, _, _, _ = self._pending.popleft()
            self._rows[key].update(state="CANCELLED", reason="STOPPED")
