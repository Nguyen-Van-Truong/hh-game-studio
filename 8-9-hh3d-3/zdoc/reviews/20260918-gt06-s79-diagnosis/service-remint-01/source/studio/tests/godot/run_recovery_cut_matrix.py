...
def audit_logs(roots):
    """Fail closed on engine/host diagnostics; parse known Docker probe output."""
    pattern = rb'(?i)\b(warning|error|fatal|traceback)\b'

    def owned_host(directory, label, exit_code, operation, container_id):
        host = json.loads((directory / (label + '-host.json')).read_bytes())
        job = host['job_owner']
        return (type(host['exit_code']) is int and host['exit_code'] == exit_code
            and host['timed_out'] is False and host['stream_cap_exceeded'] is False
            and host['job_active_count'] == 0 and host['readers_stopped'] is True
            and host['stream_reader_eof'] == [True, True]
            and host['stream_reader_errors'] == [None, None]
            and job['closed'] is True and job['zero_observed'] is True and job['active_count'] == 0
            and job['tainted'] is False and job['handle_retained'] is False
            and job['failed_operations'] == [] and job['native_error'] is None
            and host['argv'][-2:] == [operation, container_id]
            and host['stdout'] == label + '-stdout.txt' and host['stderr'] == label + '-stderr.txt')

    logs = sorted({p for root in roots for p in root.rglob('*.txt') if p.name in ('stdout.txt', 'stderr.txt')
                   or p.name.endswith(('-stdout.txt', '-stderr.txt'))})
    rows = []
    for path in logs:
        raw = path.read_bytes()
        clean = re.search(pattern, raw) is None
        reason = 'no_diagnostic'
        if not clean and path.parent.name == 'executor':
            try:
                directory = path.parent
                if path.name.endswith('-inspect-stdout.txt'):
                    document = json.loads(raw)
                    assert type(document) is list and len(document) == 1
                    container = document[0]
                    container_id = container['Id']
                    assert re.fullmatch('[0-9a-f]{64}', container_id)
                    assert container['State']['Error'] == ''
                    assert owned_host(directory, path.name.removesuffix('-stdout.txt'), 0, 'inspect', container_id)
                    del container['State']['Error']
                    clean = re.search(pattern, json.dumps(document).encode()) is None
                    reason = 'parsed_empty_docker_state_error'
                elif path.name == 'removed-inspect-stderr.txt':
                    created = json.loads((directory / 'created-inspect-stdout.txt').read_bytes())
                    assert type(created) is list and len(created) == 1
                    container_id = created[0]['Id']
                    assert re.fullmatch('[0-9a-f]{64}', container_id)
                    assert created[0]['State']['Error'] == ''
                    assert owned_host(directory, 'created-inspect', 0, 'inspect', container_id)
                    assert owned_host(directory, 'owned-remove', 0, 'rm', container_id)
                    assert (directory / 'owned-remove-stdout.txt').read_bytes().strip() == container_id.encode()
                    assert (directory / 'owned-remove-stderr.txt').read_bytes() == b''
                    assert owned_host(directory, 'removed-inspect', 1, 'inspect', container_id)
                    assert json.loads((directory / 'removed-inspect-stdout.txt').read_bytes()) == []
                    clean = raw.strip() == b'error: no such object: ' + container_id.encode()
                    reason = 'exact_owned_container_absence_after_successful_removal'
            except (AssertionError, KeyError, TypeError, ValueError, OSError):
                clean = False
                reason = 'invalid_executor_probe_evidence'
        rows.append({'path': str(path), 'sha256': hashlib.sha256(raw).hexdigest(),
                     'clean': clean, 'reason': reason})
    return {'clean': all(row['clean'] for row in rows), 'logs': rows}
...
def run_case(output, binary, closure, case, run_id, original_case=None):
    ...
    try:
        ...
        # Exercise the missing operation grant before minting the actual
        # recovery lease. An exact permission error proves this body never
        # reaches stale original-lease validation or any recovery effect.
        readonly = actor.sessions.issue(operations=frozenset({'control.lookup'}))
        denied_body = {'project_id': actor.project_id, 'command_id': command, 'digest': digest,
            'lease_id': original['request']['lease_id'],
            'fencing_epoch': original['request']['fencing_epoch'], 'deadline_ms': epoch_ms() + 29000}
        before_denied = actor._recovery.authority_context()
        denied, denied_response, _ = call(server, '/v1/reconcile', denied_body, readonly.bearer, module.CATALOG_DIGEST)
        save(output / 'denied-recovery-preflight.json', {'request': denied_body,
            'http_status': denied, 'response': denied_response})
        check('lookup_only_grant_rejected_before_recovery_effect', denied == 400
              and denied_response.get('code') == 'GODOT_OPERATION_FORBIDDEN'
...
def main():
    ...
    for case in cases:
        ...
        log_audit = audit_logs(log_roots)
        save(case_output / 'log-audit.json', log_audit)
        clean_logs = log_audit['clean']
