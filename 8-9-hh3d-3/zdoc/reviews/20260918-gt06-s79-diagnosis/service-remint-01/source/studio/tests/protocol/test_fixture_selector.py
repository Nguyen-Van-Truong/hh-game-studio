...
'''
    ...
    def test_blob_slots_are_reserved_before_intent_and_failure_does_not_poison(self):
        self.store.put_bytes(b'old-a'); self.store.put_bytes(b'old-b')
        before = self.log.binding().witnessed
        with mock.patch('host.core.fixture_selector.MAX_STAGED_OBJECTS', 4):
            with self.assertRaisesRegex(SelectorError, 'SELECTOR_STAGE_CAPACITY') as caught:
                self.selector.prepare(self.request(), now_ms=101)
            self.assertFalse(caught.exception.outcome_unknown)
        self.assertEqual(self.log.binding().witnessed, before)
        self.assertEqual(len(list(self.store.root.glob('blob-*'))), 2)
        self.selector.prepare(self.request(), now_ms=102)

    def test_two_selectors_cannot_reserve_the_same_store_and_failed_init_unbinds(self):
        with PrivateEventLog.create(self.base) as other_log:
            with self.assertRaisesRegex(SelectorError, 'SELECTOR_ALREADY_BOUND'):
                FixtureSelector(other_log, self.store, self.consumer, project_id='project-one')
            self.assertEqual(other_log.binding().witnessed.sequence, 1)
        self.selector.close()
        with self.assertRaisesRegex(SelectorError, 'SELECTOR_BINDING_MISMATCH'):
            FixtureSelector(self.log, self.store, FixtureReleaseConsumer('other'), project_id='other')
        self.assertIsNone(self.log._fixture_selector)
        self.assertIsNone(self.store._fixture_selector)
        self.selector = FixtureSelector(self.log, self.store, self.consumer, project_id='project-one')
        self.assertEqual(self.selector.snapshot()['generation'], 0)

    def test_explicit_recovery_stage_and_selection_bind_fresh_fence_in_history(self):
        request = self.request(deadline=105)
        self.selector.prepare(request, now_ms=101)
        self.reopen()
        self.lease = self.selector.lease('recovery-owner', now_ms=31_000)
        self.selector.reconcile('cmd-one', 'stage', now_ms=31_001)
        self.selector.reconcile('cmd-one', 'select', now_ms=31_002)
        self.selector.reconcile('cmd-one', 'adopt', now_ms=31_003)
        authority_records = []
        for sequence in range(1, self.log.binding().witnessed.sequence+1):
            value = json.loads(self.log.read(sequence).event)
            if value.get('kind') in ('STAGING','SELECT','TERMINAL'):
                authority_records.append(value['authority'])
        self.assertEqual(len(authority_records), 3)
        self.assertTrue(all(r['recovery'] and r['lease_id'] == self.lease['lease_id']
                            and r['fencing_epoch'] == self.lease['fencing_epoch'] for r in authority_records))

    def test_consumer_cannot_be_shared_by_two_independent_selectors(self):
        with PrivateEventLog.create(self.base) as log, PrivateBlobStore.create(self.base) as store:
            with self.assertRaisesRegex(SelectorError, 'SELECTOR_ALREADY_BOUND'):
                FixtureSelector(log, store, self.consumer, project_id='project-one')
            self.assertIsNone(getattr(log, '_fixture_selector', None))
            self.assertIsNone(getattr(store, '_fixture_selector', None))
            self.assertEqual(log.binding().witnessed.sequence, 1)

    def test_stop_during_quota_scan_prevents_intent_or_staging(self):
        for phase in ('prepare', 'stage'):
            with self.subTest(phase=phase):
                before = self.log.binding().witnessed
                capacity = self.selector._stage_capacity
                def stop_during_capacity(payload):
                    capacity(payload)
                    self.selector._stop_requested.set()
                with mock.patch.object(self.selector, '_stage_capacity', side_effect=stop_during_capacity):
                    with self.assertRaisesRegex(SelectorError, 'SELECTOR_STOPPED'):
                        if phase == 'prepare': self.selector.prepare(self.request(), now_ms=101)
                        else: self.selector.stage('cmd-one', now_ms=102)
                self.assertEqual(self.log.binding().witnessed, before)
                self.assertEqual(len(list(self.store.root.glob('blob-*'))), 0)
                self.reopen()
                if phase == 'prepare': self.selector.prepare(self.request(), now_ms=101)

    def test_stop_arriving_during_pin_prevents_later_selection_and_adoption(self):
        self.selector.prepare(self.request(), now_ms=101); self.selector.stage('cmd-one', now_ms=102)
        pin = self.selector._pin
        def stop_during_pin(selected):
            result = pin(selected)
            self.selector._stop_requested.set()
            return result
        before = self.log.binding().witnessed
        with mock.patch.object(self.selector, '_pin', side_effect=stop_during_pin):
            with self.assertRaisesRegex(SelectorError, 'SELECTOR_STOPPED'):
                self.selector.select('cmd-one', now_ms=103)
        self.assertEqual(self.log.binding().witnessed, before)
        self.assertIsNone(self.consumer.readback())
        # Reopen reconstructs durable STAGED, then explicitly reconcile SELECT.
        self.reopen()
        self.selector.reconcile('cmd-one', 'select', now_ms=104)
        pin = self.selector._pin
        before = self.log.binding().witnessed
        with mock.patch.object(self.selector, '_pin', side_effect=stop_during_pin):
            with self.assertRaisesRegex(SelectorError, 'SELECTOR_STOPPED'):
        ...
        self.assertEqual(self.log.binding().witnessed, before)
        self.assertIsNone(self.consumer.readback())
        self.assertEqual(self.selector.lookup('cmd-one')['phase'], 'SELECTED')
