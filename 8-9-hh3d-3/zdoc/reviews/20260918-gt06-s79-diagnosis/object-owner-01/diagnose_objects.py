...
ROOT = BASE.parents[2]
RUN_ID = 'gt06-s79-object-diagnostic-01'


def outer():
    output = BASE / 'object-owner-01'
    ...
    (output / 'invocation.json').write_text(json.dumps({
        'argv': argv, 'helper_files': before, 'timeout_seconds': 720,
        'runner_sha256': hashlib.sha256(runner.read_bytes()).hexdigest(),
        'formal_acceptance': False, 'full_benchmark': False}, indent=2) + '\n', encoding='utf-8')
    result = owned.run_process(argv, cwd=ROOT, output=output, timeout=720, label='probe')
...
def child():
    ...
    n.write(root / 'diagnostic.json', {
        ...
        'cycles_each': 100, 'idle_seconds': 120, 'inner_wall_seconds': 660,
    ...
    try:
        ...
        while owner.tick(stop=time.monotonic() - started >= 660) is None:
            time.sleep(.1)
