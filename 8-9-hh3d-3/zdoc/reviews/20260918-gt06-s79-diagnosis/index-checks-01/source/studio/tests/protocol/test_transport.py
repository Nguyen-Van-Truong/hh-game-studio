...
class TransportTests(unittest.TestCase):
    ...
    def test_queued_revision_and_lease_are_checked_again_before_apply(self) -> None:
        ...
        # A 40 ms wall-clock window could expire before submit under disk/load,
        # testing admission rejection instead of the queued recheck. Hold the
        # real executor until durable admission, then advance only its trusted
        # test clock to the exact lease boundary. No storage/apply check is mocked.
        lease = self.client.lease(ttl_ms=3000)
        third = self.client.request("lease.expires", value=3, lease=lease)
        entered, release = threading.Event(), threading.Event()
        execute = self.host._execute
        def wait_before_execute(job):
            entered.set()
            if not release.wait(2):
                raise RuntimeError('test executor gate timed out')
            return execute(job)
        with mock.patch.object(self.host, '_execute', side_effect=wait_before_execute):
            try:
                self.assertIs(self.client.submit(third).status, Status.ACCEPTED_PENDING)
                self.assertTrue(entered.wait(1))
                with mock.patch('studio.host.core.transport.epoch_ms', return_value=lease.expires_ms):
                    release.set()
