"""Fixed MSVC lanes: bounded Job, explicit build-helper teardown after exit."""
import hashlib
import os
...
def run_tool(argv, cwd, environment, label):
    ...
    handles = []
    out = {'timeout_ms':25000,'child_process_creation_disabled':False}
    try:
        ...
        allowlist = (W.HANDLE*len(handles))(*handles)
        ...
        limits = JEXT(); limits.basic.flags = 0x2000
        ...
        out['helpers_active_after_compiler_exit'] = int(account.active_processes)
        if account.active_processes:
            # The root compiler has a captured exit already. This job belongs
            # solely to this build step; never enumerate/kill unrelated vctip.
            checked(K.TerminateJobObject(job,0))
            out['helper_cleanup'] = 'owned_build_job_after_root_exit'
        for _ in range(101):
            checked(K.QueryInformationJobObject(job,1,C.byref(account),C.sizeof(account),None))
...
def compile_native(base, source, out):
    ...
    for label,argv in (('compile',compile_args),('link',link_args)):
        ...
        assert lane['host_exit']==0 and lane['job_active']==0 and not lane.get('forced_termination'), label+' failed'
    return exe
