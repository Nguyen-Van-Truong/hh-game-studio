...
* Godot's 4.7 command-line reference defines `--headless`, `--check-only`, `--quit-after`, display/audio drivers and export flags, but explicitly says unknown arguments have no effect. The runner should probe `--help`/`--version` and reject an unrecognised or unsupported flag rather than silently proceeding. [Godot command-line tutorial](https://docs.godotengine.org/en/4.7/tutorials/editor/command_line_tutorial.html)
...
9. **CLI contract:** validate every Godot/Blender flag against the pinned binary's help/version output; fail closed on unknown flags because Godot documents that unknown arguments can have no effect.
