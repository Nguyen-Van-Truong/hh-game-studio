...
class ObservationTests(unittest.TestCase):
            ...
            for mutate in mutations:
                with self.subTest(role=role, mutation=mutations.index(mutate)):
                    ...
                    self.assertEqual(set(self.writes), {f'post-failure-{role}-error.json',
                                                       f'post-failure-{role}-00-incomplete-pss.json'})
        ...
        for mutate in mutations:
            with self.subTest(mutation=mutations.index(mutate)):
                ...
                self.assertEqual(set(self.writes), {'post-failure-editor-error.json',
                                                   'post-failure-editor-00-incomplete-pss.json'})

    def test_actual_adapter_cleanup_failure_preserves_sanitized_unknown_evidence(self):
        returned = []
        def capture(probe):
            api = FakeApi(probe, r'C:\Pinned\Engine.exe')
            api.free_snapshot = lambda snapshot: 5
            try:
                result = p.capture_owned(probe, _api=api, _clock=lambda: 100)
            finally:
                # Only fake resources were acquired; isolate the adapter's fake latch.
                p._HELD_RESOURCES.clear()
            result['exception_text'] = 'SECRET-ARBITRARY-CALLBACK-EXTRA'
            result['cleanup']['snapshot']['raw_pointer'] = 'SECRET-ARBITRARY-CALLBACK-EXTRA'
            result['errors'][0]['message'] = 'SECRET-ARBITRARY-CALLBACK-EXTRA'
            returned.append(deepcopy(result))
            return result
        with self.assertRaisesRegex(h.ObservationError, 'S129_PSS_INCOMPLETE'):
            self.observe(capture=capture)
        failed = self.writes['post-failure-editor-00-incomplete-pss.json']
        self.assertIs(failed['completed'], False)
        self.assertIs(failed['observation_validated'], False)
        self.assertEqual(failed['pss']['status'], 'UNKNOWN')
        self.assertEqual(failed['pss']['cleanup']['snapshot']['native_code'], 5)
        self.assertIs(failed['pss']['cleanup']['all_released'], False)
        self.assertEqual(failed['pss']['cleanup']['held_resources'], ['snapshot'])
        self.assertEqual(failed['pss']['errors'][0]['code'], 'PSS_CLEANUP_UNVERIFIED')
        self.assertEqual(failed['pss']['identity'], returned[0]['identity'])
        self.assertNotIn('SECRET-ARBITRARY-CALLBACK-EXTRA', json.dumps(failed))
        self.assertNotIn('post-failure-editor-00.json', self.writes)
    ...
    def test_second_capture_executable_drift_keeps_partial_sequence_incomplete(self):
        ...
        incomplete = self.writes['post-failure-editor-05-incomplete-pss.json']
        self.assertEqual(incomplete['validated_offsets'], [0, 1, 3])
        self.assertIs(incomplete['observation_validated'], False)
        self.assertEqual(incomplete['pss']['identity']['executable'], r'C:\Wrong.exe')
        with self.assertRaises(h.ObservationError):
    ...
    def test_failed_snapshot_receipt_write_preserves_original_observation_error(self):
        expected = h.ObservationError('S129_PSS_INCOMPLETE')
        def write(name, row):
            if name.endswith('-incomplete-pss.json'):
                raise OSError('synthetic supplemental receipt failure')
            self.writes[name] = deepcopy(row)
        original = h.validate_pss
        try:
            h.validate_pss = Mock(side_effect=expected)
            with self.assertRaises(h.ObservationError) as raised:
                self.observe(write=write)
        finally:
            h.validate_pss = original
        self.assertIs(raised.exception, expected)
        self.assertEqual(self.writes['post-failure-editor-error.json']['error_code'], expected.code)

    def test_advancing_clock_never_requests_negative_sleep_at_offset_boundary(self):
        class AdvancingClock(FakeClock):
            ...
            def sleep(self, duration):
                if duration <= 0:
                    raise AssertionError('non-positive sleep requested')
                super().sleep(duration)
        self.clock = AdvancingClock()
        rows = self.observe()
        self.assertEqual([row['offset_seconds'] for row in rows], [0, 1, 3, 5])
        self.assertTrue(all(row['elapsed_seconds'] < 8 for row in rows))
...
class GateTests(unittest.TestCase):
    ...
    def test_editor_failure_does_not_suppress_host_observation_or_primary_gate(self):
        editor = Mock(side_effect=h.ObservationError('S129_PSS_INCOMPLETE'))
        host = Mock()
        observer = self.observer(observe_fn=editor, observe_host_fn=host)
        self.assert_primary(observer)
        editor.assert_called_once()
        host.assert_called_once()
        receipt = self.writes['post-failure-roles.json']
        self.assertEqual(receipt['roles'], {
            'editor': {'disposition': 'failed', 'error_code': 'S129_PSS_INCOMPLETE'},
            'host': {'disposition': 'observed'}})
        self.assertIs(receipt['completed'], False)
        self.assertEqual(self.writes['post-failure-error.json']['errors'],
                         [{'stage': 'editor', 'code': 'S129_PSS_INCOMPLETE'}])

    def test_two_role_failures_are_retained_independently(self):
        observer = self.observer(observe_fn=Mock(side_effect=RuntimeError('editor failure')),
            observe_host_fn=Mock(side_effect=h.ObservationError('S129_HOST_IDENTITY')))
        self.assert_primary(observer)
        observer.observe_host_fn.assert_called_once()
        self.assertEqual([row['stage'] for row in self.writes['post-failure-error.json']['errors']],
                         ['editor', 'host'])
        self.assertEqual({role: row['disposition'] for role, row in
                          self.writes['post-failure-roles.json']['roles'].items()},
                         {'editor': 'failed', 'host': 'failed'})

    def test_host_gets_its_own_budget_after_editor_observation_times_out(self):
        clock = FakeClock()
        def capture(probe):
            api = FakeApi(probe, r'C:\Pinned\Owned.exe')
            result = p.capture_owned(probe, _api=api, _clock=lambda: 100)
            if probe.pid == identities()['editor']['pid']:
                clock.now += 8
            return result
        common = dict(capture=capture, check_stop=lambda: None, clock=clock, sleep=clock.sleep,
                      write=lambda name, row: self.writes.__setitem__(name, deepcopy(row)))
        def editor(probe, binding):
            return h.observe(probe, binding=binding, sample_editor=lambda target: counters(), **common)
        def host(probe, binding):
            return h.observe_host(probe, binding=binding,
                sample_host=lambda target: {'process': identities()['host'], 'counters': counters()}, **common)
        self.assert_primary(self.observer(observe_fn=editor, observe_host_fn=host))
        self.assertEqual(self.writes['post-failure-roles.json']['roles'], {
            'editor': {'disposition': 'failed', 'error_code': 'S129_OBSERVATION_WALL_LIMIT'},
            'host': {'disposition': 'observed'}})
        self.assertIn('post-failure-editor-00-incomplete-pss.json', self.writes)
        self.assertNotIn('post-failure-editor-complete.json', self.writes)
        self.assertTrue(self.writes['post-failure-host-complete.json']['completed'])
        self.assertEqual(self.writes['post-failure-host-05.json']['elapsed_seconds'], 5)
        self.assertEqual(clock.now, 13)

    def test_stop_after_editor_failure_skips_host_and_retains_dispositions(self):
        stopped = False
        def editor(probe, binding):
            nonlocal stopped
            stopped = True
            raise h.ObservationError('S129_PSS_INCOMPLETE')
        def check_stop():
            if stopped:
                raise h.ObservationError('BENCHMARK_STOPPED')
        observer = self.observer(observe_fn=editor, check_stop=check_stop)
        self.assert_primary(observer)
        observer.observe_host_fn.assert_not_called()
        roles = self.writes['post-failure-roles.json']['roles']
        self.assertEqual(roles['editor']['disposition'], 'failed')
        self.assertEqual(roles['host'], {'disposition': 'skipped', 'reason': 'STOP_CHECK_REJECTED'})
        self.assertEqual(self.writes['post-failure-error.json']['errors'][-1],
                         {'stage': 'host_stop_check', 'code': 'BENCHMARK_STOPPED'})

    def test_stop_before_dispatch_skips_both_roles(self):
        observer = self.observer(check_stop=Mock(side_effect=h.ObservationError('BENCHMARK_STOPPED')))
        self.assert_primary(observer)
        observer.observe_fn.assert_not_called()
        observer.observe_host_fn.assert_not_called()
        self.assertTrue(all(row['disposition'] == 'skipped'
                            for row in self.writes['post-failure-roles.json']['roles'].values()))
