...
import validate_plans_s21 as validator
...
def manifest_for(inputs):
    ...
    return {'revision': 'S21', 'files': rows, 'manifest_sha256': digest}
...
class S21Tests(unittest.TestCase):
    ...
    def test_wrong_worker_model(self):
        self.check_mutation(0, 'CURRENT_IMPLEMENTER_WORKER_MODEL=gpt-6-astra', 'CURRENT_IMPLEMENTER_WORKER_MODEL=auto', 'S21 exact critic policy and bounded Astra implementers')

    def test_wrong_worker_effort(self):
        self.check_mutation(0, 'CURRENT_IMPLEMENTER_WORKER_MODEL=gpt-6-astra; REASONING_EFFORT=high;', 'CURRENT_IMPLEMENTER_WORKER_MODEL=gpt-6-astra; REASONING_EFFORT=medium;', 'S21 exact critic policy and bounded Astra implementers')
    ...
    def test_design_critics_header_cannot_claim_accept(self):
        self.check_mutation(0, 'PLAN_DESIGN_CRITICS=NO_ACCEPTED_VERDICT_S6_TO_S21', 'PLAN_DESIGN_CRITICS=ACCEPTED', 'PLAN_DESIGN_CRITICS')

    def test_design_critics_header_required_in_game_plan(self):
        self.check_mutation(1, 'PLAN_DESIGN_CRITICS=NO_ACCEPTED_VERDICT_S6_TO_S21\n', '', 'PLAN_DESIGN_CRITICS')
    ...
    def test_auxiliary_scope_cannot_expand(self):
        self.check_mutation(0, 'AUX_SCOPE=readonly-audit|isolated-helper<=3-files', 'AUX_SCOPE=unrestricted', 'S21 auxiliary scope cannot replace acceptance critics')

    def test_auxiliary_cannot_replace_critic(self):
    ...
    def test_pending_remint_is_required(self):
        self.check_mutation(0, 'GT01_PENDING=SOURCE_FREEZE,ARCHIVE_VERIFIER,INSTALLER_RECOVERY,RUNNER_ADMISSION,OFFICIAL_RUNTIME_R9,TQ01_TX12_TX14,TWO_CRITICS', 'GT01_PENDING=TWO_CRITICS', 'pending header')

    def test_pending_evidence_is_required(self):
        self.check_mutation(0, 'GT01_PENDING=SOURCE_FREEZE,ARCHIVE_VERIFIER,INSTALLER_RECOVERY,RUNNER_ADMISSION,OFFICIAL_RUNTIME_R9,TQ01_TX12_TX14,TWO_CRITICS', 'GT01_PENDING=TQ01_TX12_TX14', 'pending header')
    ...
    def test_provenance_guard(self):
        self.check_mutation(0, 'TAG_DRIFT=GAP', 'TAG_DRIFT=ALLOW', 'source provenance')

    def test_tools_monetization_guard(self):
        self.check_mutation(0, 'MONETIZATION_POLICY=OFF_ARTIFACT_SCAN_REQUIRED', 'MONETIZATION_POLICY=ON', 'monetization')

    def test_game_monetization_guard(self):
        self.check_mutation(1, 'MONETIZATION_POLICY=OFF_ARTIFACT_SCAN_REQUIRED', 'MONETIZATION_POLICY=OFF_NO_SCAN', 'monetization')

    def test_game_capacity_requires_profile(self):
        self.check_mutation(1, 'CAPACITY_CLAIMS=GAME_P8_03_MEASURED_SIGNED_CURRENT_ONLY', 'CAPACITY_CLAIMS=UNMEASURED', 'capacity claims')

    def test_tools_cannot_claim_game_capacity(self):
        self.check_mutation(0, 'CAPACITY_CLAIMS=NOT_APPLICABLE_TO_TOOLS', 'CAPACITY_CLAIMS=UNLIMITED', 'capacity claims')
...
if __name__ == '__main__':
    ...
    report = {'result': 'PASS' if result.wasSuccessful() else 'FAIL', 'tests_run': result.testsRun,
              ...
              'manifest_sha256': manifest_for({n: (validator.Z / n).read_bytes() for n in validator.NAMES})['manifest_sha256'],
              'limits': 'Static and rehashed mutation checks only; no runtime/critic/legal acceptance.'}
    (validator.OUT / 'mutation-s21.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
