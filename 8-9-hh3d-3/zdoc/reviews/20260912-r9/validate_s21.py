...
validator = importlib.import_module('validate_plans_s21')
mutations = importlib.import_module('test_review_s21')

def main():
    manifest = validator.freeze('S21')
    inputs = {n:(validator.Z/n).read_bytes() for n in validator.NAMES}
    result = validator.validate(manifest, inputs, validator.Z)
    (HERE/'freeze-s21.json').write_text(json.dumps(manifest,indent=2)+'\n',encoding='utf-8')
    (HERE/'static-s21.json').write_text(json.dumps(result,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    original = mutations.manifest_for
    def manifest_s21(values):
        value = original(values)
        value['revision'] = 'S21'
        return value
    mutations.manifest_for = manifest_s21
    test = unittest.TextTestRunner(verbosity=1).run(unittest.defaultTestLoader.loadTestsFromTestCase(mutations.S21Tests))
    ...
    (HERE/'selfcheck-s21.json').write_text(json.dumps(summary,indent=2)+'\n',encoding='utf-8')
