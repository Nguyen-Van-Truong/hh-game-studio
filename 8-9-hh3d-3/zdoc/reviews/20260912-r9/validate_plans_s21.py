"""Static S19 plan checks. These do not prove semantic/runtime/legal acceptance.

S17 (independent coordinator audit, 2026-09-10): both plans carry PLAN_DESIGN_CRITICS stating that
no design critic verdict has been ACCEPT from S6 to S21; the tools plan pins Godot 4.7.2-stable
...
def validate(manifest, inputs, root):
    ...
    for index, name in enumerate(NAMES):
        ...
        check(re.findall(r"(?m)^PLAN_DESIGN_CRITICS=(.+)$",s)==["NO_ACCEPTED_VERDICT_S6_TO_S21"], name+": PLAN_DESIGN_CRITICS header states no accepted design verdict")
    ...
    implementer="CURRENT_IMPLEMENTER_WORKER_MODEL=gpt-6-astra; REASONING_EFFORT=high; MAX_PARALLEL_WORKERS=3; LEASE_REQUIRED=1; FILE_SCOPE_REQUIRED=1; NO_COMPOSER/NO_HIDDEN_SUBAGENTS"
    check(g.count(policy)==1 and t.count(implementer)==1 and all("--no-subagents" in x for x in (t,g)),
          "S21 exact critic policy and bounded Astra implementers")
    check("AUX_SCOPE=readonly-audit|isolated-helper<=3-files" in t and "không thay critic Grok bắt buộc" in n(t),
          "S21 auxiliary scope cannot replace acceptance critics")
    pending=re.findall(r"(?m)^GT01_PENDING=(.+)$", t)
    check(len(pending)==1 and set(pending[0].split(","))=={"SOURCE_FREEZE","ARCHIVE_VERIFIER","INSTALLER_RECOVERY","RUNNER_ADMISSION","OFFICIAL_RUNTIME_R9","TQ01_TX12_TX14","TWO_CRITICS"},
        "S21 pending header includes current remint and evidence gaps")
    check(".uid" in specs["GT-01"] and "VCS" in specs["GT-01"] and "*.uid" not in specs["GT-01"],
          "S21 fixture UID metadata remains source")
    check("studio/contracts/" in between(t,"2.1 Cấu trúc source","2.2"),
          "S21 cross-tool contracts in source tree")
    check("PROVENANCE_POLICY=TAG_COMMIT_ASSET_HASH_BOUND" in t and "TAG_DRIFT=GAP" in t,
          "S21 immutable source provenance guard")
    check(all("MONETIZATION_POLICY=OFF_ARTIFACT_SCAN_REQUIRED" in x for x in (t,g)),
          "S21 monetization requires OFF and artifact scan")
    check("CAPACITY_CLAIMS=GAME_P8_03_MEASURED_SIGNED_CURRENT_ONLY" in specs["H2-P8-03"]
          and "CAPACITY_CLAIMS=NOT_APPLICABLE_TO_TOOLS" in specs["GT-10"],
          "S21 capacity claims belong to measured game release")
    ...
    check("GT01_EVIDENCE_STATUS=PARTIAL_RUNTIME_UNREVIEWED" in t and "GT01_PENDING=" in t,
          "S19 GT01 partial status and missing evidence are explicit")
...
def main():
    ...
    p.add_argument("--revision",default="S19");a=p.parse_args()
    if a.freeze:
