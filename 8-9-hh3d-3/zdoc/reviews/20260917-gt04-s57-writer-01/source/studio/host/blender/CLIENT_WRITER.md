...
optional `publication=exact_publication_owner` binds `export.publish` to that
same durable session, generation and source. Each protected owner has capacity
for one bundle. `scene.save` and `checkpoint.save` remain unconnected.
...
A common COMMITTED receipt does not restore a writable GUI. Save integration
and final same-closure verification remain required for acceptance.
