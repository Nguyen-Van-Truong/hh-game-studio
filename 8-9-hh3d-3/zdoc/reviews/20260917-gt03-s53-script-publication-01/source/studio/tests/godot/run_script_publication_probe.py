...
def frozen(output, binary, closure):
    ...
    try:
        ...
        # Refresh only the lease, not the expected script/project/scene identity.
        status, lease, _ = call('/v1/lease', {'project_id': owner.project_id, 'ttl_ms': 90_000})
        check('publication_lease_renewed', status == 200)
        ...
        check('selected_complete_eleven_inputs', len(selected.files) == 11
            and after['working_files'] == parse_json(selected.manifest_bytes)['files']
