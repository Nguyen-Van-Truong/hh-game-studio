"""60-second outer owned tree for the fixed trusted Godot startup diagnostic."""
import ctypes as C
...
for name in ('startup_probe.py','capture.py','project.godot','main.tscn','plugin.cfg','probe.gd'):
...
host = runner.run_process([sys.executable,'-B',str(BASE/'startup_probe.py'),str(out)],
    cwd=PRODUCT/'studio',output=out,timeout=60,label='godot-sandbox')
