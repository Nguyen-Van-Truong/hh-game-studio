...
def launch(executable, base, mode, sentinel, identity, package=None):
    ...
    out = {"mode": mode, "inherit_handles": inherited, "inherited_handle_allowlist_count": 1 if inherited else 0,
           ...
           "child_process_policy_requested": restricted, "console_mode": "DETACHED_PROCESS"}
    try:
        ...
        # This fixed program logs to its own file. It needs no console host.
        flags = 0x0000040c | (0x80000 if attribute_count else 0)
        out["process_creation_flags"] = flags
    ...
    if native.exists():
        ...
        out["native_log_bytes"] = len(native_bytes)
        try:
            out["native"] = json.loads(native_bytes)
        except ValueError as exc:
            out["native_parse_error"] = str(exc)
            out["native_partial_text"] = native_bytes[:16384].decode(errors="replace")
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="gt02-s32-boundary-") as tmp, ExitStack() as stack:
            ...
            image_directory = base / "native-image"
            make_directory(image_directory, common + "(A;OICI;GRGX;;;" + package_string + ")")
            executable = image_directory / "boundary_child.exe"
