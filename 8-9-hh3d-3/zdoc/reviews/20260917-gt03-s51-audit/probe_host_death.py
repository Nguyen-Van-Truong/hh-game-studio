"""S51 exact-owner host death, idle child, and independent process admission proof."""
...
SOURCE = Path(os.environ['HH_S51_FROZEN_STUDIO']).resolve()
sys.path.insert(0, str(SOURCE.parent))
spec = importlib.util.spec_from_file_location('s51_final_executor', SOURCE / 'godot-addon/linux_executor.py')
...
def project(root, idle):
    ...
    code='extends Node3D\n'
    if idle:
        code+='static func _static_init() -> void:\n\tOS.delay_msec(3000)\n\tprint("S51_IDLE_BODY_AFTER_HOST_DEATH")\n\twhile true:\n\t\tOS.delay_msec(100)\n'
...
def main(root):
    ...
    try:
        ...
        evidence['child_body_observed']=b'S51_IDLE_BODY_AFTER_HOST_DEATH' in observer[0][1]
