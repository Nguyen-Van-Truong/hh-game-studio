...
var _host_open_stage: String = ""
var _host_open_batch: int = -1
var _host_open_attempts: int = 0
var _host_open_first_error: int = OK
...
func _wait_host_start() -> void:
    ...
    if not FileAccess.file_exists(path):
        return
    var file: FileAccess = _open_host_input(path, "start")
    if file == null:
        return
    ...
    file.close()
    if raw.size() != size:
        _host_input_short_read("start", size, raw.size(), read_error)
        _fail("BENCHMARK_HOST_START_SHORT_READ")
        return
...
func _wait_host_ack() -> void:
    ...
    if not FileAccess.file_exists(path):
        return
    var file: FileAccess = _open_host_input(path, "ack")
    if file == null:
        return
    var size: int = file.get_length()
    if size < 1 or size > 8192:
    ...
    file.close()
    if raw.size() != size:
        _host_input_short_read("ack", size, raw.size(), read_error)
        _fail("BENCHMARK_HOST_ACK_SHORT_READ")
        return
    var decoded: Variant = JSON.parse_string(raw.get_string_from_utf8())
    if not decoded is Dictionary:
...
func _open_host_input(path: String, stage: String) -> FileAccess:
    # A published fixed-slot name can be temporarily unopenable. The callers
    # retain their original absolute deadlines and retry on normal frames.
    # Keep only primitive state for the current stage/batch, never file handles.
    if _host_open_stage != stage or _host_open_batch != _batch:
        _host_open_stage = stage
        _host_open_batch = _batch
        _host_open_attempts = 0
        _host_open_first_error = OK
        _host_open_first_us = 0
        _host_open_first_frame = 0
        _host_open_recovered = false
    _host_open_attempts += 1
    var file: FileAccess = FileAccess.open(path, FileAccess.READ)
    if file == null:
        var open_error: int = FileAccess.get_open_error()
        if _host_open_first_us == 0:
            _host_open_first_error = open_error
            _host_open_first_us = Time.get_ticks_usec()
            _host_open_first_frame = Engine.get_process_frames()
            _host_input_open_log("pending", "HOST_INPUT_OPEN_PENDING")
        return null
    if _host_open_first_us > 0 and not _host_open_recovered:
        _host_open_recovered = true
        _host_input_open_log("recovered", "HOST_INPUT_OPEN_RECOVERED")
    return file


func _host_input_open_log(event: String, code: String) -> void:
    # Separate from BENCHMARK markers; at most pending + recovered/terminal
    # per slot. A recovered open is not a validated START/ACK receipt.
    print("HH_GT06_HOST_INPUT_OPEN " + JSON.stringify({"event": event, "code": code,
        "stage": _host_open_stage, "batch": _host_open_batch,
        "first_error": _host_open_first_error, "first_mono_us": _host_open_first_us,
        "first_process_frame": _host_open_first_frame, "attempts": _host_open_attempts,
        "monotonic_us": Time.get_ticks_usec(), "process_frame": Engine.get_process_frames(),
        "scope": "open_only_not_receipt"}))


func _host_input_short_read(stage: String, expected: int, actual: int, read_error: int) -> void:
    print("HH_GT06_HOST_INPUT_SHORT_READ " + JSON.stringify({"stage": stage, "batch": _batch,
        "expected_bytes": expected, "actual_bytes": actual, "read_error": read_error,
        "monotonic_us": Time.get_ticks_usec(), "process_frame": Engine.get_process_frames()}))
...
func _fail(code: String) -> void:
    if _failed:
        return
    _failed = true
    if _host_open_first_us > 0 and not _host_open_recovered and _host_open_batch == _batch:
        if (_phase == Phase.HOST_START and _host_open_stage == "start") or (_phase == Phase.HOST_BARRIER and _host_open_stage == "ack"):
            _host_input_open_log("terminal", code)
...
const S95_ID_CAP: int = 32768
const S95_SNAPSHOT_CAP: int = 2

var _s95_baseline: Dictionary = {}
var _s95_baseline_objects: int = -1
var _s95_snapshot_count: int = 0


func _s95_counters() -> Dictionary:
    return {
...
func _s95_tree_items(tree: Tree, remaining: int) -> Dictionary:
...
func _s95_collect() -> Dictionary:
    ...
    for candidate: Node in found_trees:
        ...
        var row: Dictionary = _s95_tree_items(tree, S95_ID_CAP - total)
...
func _s95_difference(old_ids: PackedInt64Array, new_ids: PackedInt64Array) -> Dictionary:
...
func _s95_delta(current: Dictionary) -> Dictionary:
    var old_trees: Dictionary = _s95_baseline.trees
    ...
    for tree_id: int in tree_ids:
        ...
        var delta: Dictionary = _s95_difference(old_ids, new_ids)
        if not delta.added.is_empty() or not delta.removed.is_empty() or old_trees.has(tree_id) != new_trees.has(tree_id):
            ...
            trees[tree_id] = delta
    return {"trees": trees, "node3d_ids": _s95_difference(_s95_baseline.node3d_ids, current.node3d_ids),
        "target_net_count": int(current.id_count) - int(_s95_baseline.id_count)}


func _s96_before_ack(ack_file_sha256: String) -> void:
    if _mode != "full" or _s95_snapshot_count >= S95_SNAPSHOT_CAP:
        return
    var before: Dictionary = _s95_counters()
    var baseline: bool = _batch == 4 and _s95_snapshot_count == 0
    var growth: bool = _batch > 4 and _s95_baseline_objects >= 0 and int(before.objects) > _s95_baseline_objects
    if not baseline and not growth:
        return
    var started: int = Time.get_ticks_usec()
    var frame: int = Engine.get_process_frames()
    var static_before: int = int(Performance.get_monitor(Performance.MEMORY_STATIC))
    var current: Dictionary = _s95_collect()
    # _s95_collect returned; all Node arrays and local Object variables are gone.
    var after: Dictionary = _s95_counters()
    ...
    var delta: Dictionary = _s95_delta(current) if growth and complete else {}
    var point: Dictionary = {
        "schema_id": "hh-studio.gt06.s96-ack-sparse-attribution", "schema_version": "1.0.0",
        "run_id": _input.run_id, "ack_file_sha256": ack_file_sha256, "pid": OS.get_process_id(), "batch": _batch,
        "sequence": _s95_snapshot_count, "phase": "before_ack_counter_readback",
        "trigger": "baseline_ack_batch4" if baseline else "first_ack_object_growth",
        ...
        "id_cap": S95_ID_CAP, "snapshot_cap": S95_SNAPSHOT_CAP,
        ...
        "baseline_objects": int(before.objects) if baseline else _s95_baseline_objects,
        "object_delta_from_baseline": 0 if baseline else int(before.objects) - _s95_baseline_objects,
    ...
    var receipt: Dictionary = _write_new("sparse-%02d.json" % _s95_snapshot_count, point)
    if receipt.is_empty():
        _fail("S95_SPARSE_WRITE")
        return
    var after_publication: Dictionary = _s95_counters()
    var post: Dictionary = _write_new("sparse-post-%02d.json" % _s95_snapshot_count, {
        "schema_id": "hh-studio.gt06.s96-ack-sparse-post", "schema_version": "1.0.0",
        "run_id": _input.run_id, "ack_file_sha256": ack_file_sha256, "batch": _batch, "sequence": _s95_snapshot_count,
    ...
    if post.is_empty():
        _fail("S95_SPARSE_POST_WRITE")
        return
    if not complete:
        _fail("S95_ID_CAP")
        return
    if before != after or before != after_publication or before != _s95_counters():
        _fail("S95_COUNTER_SELF_DRIFT")
        return
    if baseline:
        _s95_baseline = current
        _s95_baseline_objects = int(before.objects)
    _s95_snapshot_count += 1
