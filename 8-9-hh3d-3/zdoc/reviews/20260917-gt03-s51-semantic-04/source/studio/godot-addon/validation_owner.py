...
class SemanticValidationReceipt:
    ...
    source_release_sha256: str
    validator_engine_sha256: str
    observation_sha256: str
    evidence_sha256: str
    run_id: str
    validation_started_ms: int
    validation_completed_ms: int
    context_kind: str = field(default='isolated_candidate', init=False)
    public_ack: bool = field(default=False, init=False)
    selected_state_verified: bool = field(default=False, init=False)
    live_editor_adoption_verified: bool = field(default=False, init=False)
...
class ValidationOwner:
    ...
    def validate(self, command_id, bundle, *, timeout_seconds=20):
        ...
        try:
            ...
            try:
                ...
                _need(completed_ms >= started_ms, 'VALIDATION_CLOCK_REGRESSION')
                self._records[command_id] = _Record(bundle, directory, raw_observation, evidence,
                    dict(self._source_files), receipt, canonical_bytes(asdict(receipt)), started_ms, completed_ms)
                return receipt
            except BaseException:
                self._held = True
                raise
    ...
    def observation(self, receipt, bundle):
        ...
        try:
            ...
            _need(canonical_bytes(asdict(receipt)) == record.issued, 'VALIDATION_REGISTERED_RECEIPT_CHANGED')
    ...
    def bind_semantics(self, receipt, bundle):
        """Bind a fresh manifest to an owned full snapshot without another run.

        All eleven engine-input bytes remain identical. The original validation
        receipt and bundle are preserved; they are never relabeled. Candidate
        observation remains distinct from actual live-editor adoption.
        """
        _need(self._mutex.acquire(timeout=2), 'VALIDATION_OWNER_BUSY')
        try:
            observation = self.observation(receipt, bundle)
            comparison = comparator.compare_observation(bundle, observation)
            _need(observation.get('schema') == 'hh-godot-profile-readback-2'
                  and comparison.get('semantic_observed') is True,
                  'VALIDATION_SEMANTIC_OBSERVATION_REQUIRED')
            state_bytes = comparator.semantic_state_bytes(observation['semantic'])
            revision = 'sha256:' + _sha(state_bytes)
            _need(comparison['semantic_revision'] == revision, 'VALIDATION_SEMANTIC_REVISION')
            original = self._records[receipt.command_id]
            issued = parse_json(original.issued)
            final = bundle_codec.create_bundle(dict(bundle.files), scene_revision=revision,
                                               engine_sha256=issued['engine_sha256'])
            _need(dict(final.files) == dict(original.bundle.files), 'VALIDATION_FINAL_INPUTS_CHANGED')
            input_hash = _sha(canonical_bytes(parse_json(final.manifest_bytes)['files']))
            value = SemanticValidationReceipt(issued['command_id'], final.project_revision, revision,
                _sha(final.manifest_bytes), input_hash, issued['source_release_sha256'],
                issued['engine_sha256'], issued['observation_sha256'], issued['evidence_sha256'],
                issued['run_id'], original.started_ms, original.completed_ms)
            existing = self._semantic.get(receipt.command_id)
            if existing is not None:
                _need(existing[0] == final and existing[1] == value and existing[2] is receipt,
                      'VALIDATION_FINAL_BINDING_CHANGED')
                return existing[0], existing[1]
            self._semantic[receipt.command_id] = (final, value, receipt)
            return final, value
        finally:
            self._mutex.release()

    def semantic_observation(self, receipt, bundle):
        """Recheck registered final provenance; does not observe selected state."""
        _need(self._mutex.acquire(timeout=2), 'VALIDATION_OWNER_BUSY')
        try:
            self._healthy()
            _need(type(receipt) is SemanticValidationReceipt and type(bundle) is bundle_codec.CompleteFixtureBundle,
                  'VALIDATION_REGISTERED_SEMANTIC_RECEIPT_REQUIRED')
            binding = self._semantic.get(receipt.command_id)
            _need(binding is not None and binding[1] is receipt and binding[0] == bundle,
                  'VALIDATION_REGISTERED_SEMANTIC_RECEIPT_REQUIRED')
            original = self._records[receipt.command_id]
            observation = self.observation(binding[2], original.bundle)
            final, rechecked = self.bind_semantics(binding[2], original.bundle)
            _need(final == bundle and rechecked is receipt, 'VALIDATION_FINAL_BINDING_CHANGED')
            return {'context_kind': 'isolated_candidate', 'command_id': receipt.command_id,
                'run_id': receipt.run_id, 'validation_started_ms': receipt.validation_started_ms,
                'validation_completed_ms': receipt.validation_completed_ms,
                'project_revision': bundle.project_revision, 'manifest_sha256': receipt.manifest_sha256,
                'validator_engine_sha256': receipt.validator_engine_sha256,
                'source_release_sha256': receipt.source_release_sha256,
                'semantic': observation['semantic'], 'public_ack': False,
