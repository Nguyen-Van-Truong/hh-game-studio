...
def manifest_for(inputs):
    ...
    return {'revision': 'S17', 'files': rows, 'manifest_sha256': digest}
...
if __name__ == '__main__':
    ...
    report = {'result': 'PASS' if result.wasSuccessful() else 'FAIL', 'tests_run': result.testsRun,
              ...
              'manifest_sha256': manifest_for(S17Tests.inputs)['manifest_sha256'],
              'limits': 'Static and rehashed mutation checks only; no runtime/critic/legal acceptance.'}
    (validator.OUT / 'selfcheck-s17.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
