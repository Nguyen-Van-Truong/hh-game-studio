"""Mutation checks for the S14 documentation freeze; never runtime acceptance."""
...
def manifest_for(inputs):
    ...
    return {'revision': 'S14', 'files': rows, 'manifest_sha256': digest}
...
if __name__ == '__main__':
    ...
    report = {'result': 'PASS' if result.wasSuccessful() else 'FAIL', 'tests_run': result.testsRun,
              ...
              'manifest_sha256': manifest_for(S14Tests.inputs)['manifest_sha256'],
              'limits': 'Static and rehashed mutation checks only; no runtime/critic/legal acceptance.'}
    (validator.OUT / 'selfcheck-s14.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
