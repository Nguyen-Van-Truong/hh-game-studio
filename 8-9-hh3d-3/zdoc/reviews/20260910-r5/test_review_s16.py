"""Mutation checks for the S16 documentation freeze; never runtime acceptance."""
...
def manifest_for(inputs):
    ...
    return {'revision': 'S16', 'files': rows, 'manifest_sha256': digest}
...
class S16Tests(unittest.TestCase):
    ...
    def test_diagnostic_not_acceptance(self):
        self.check_mutation(0, 'GT01_EVIDENCE_STATUS=DIAGNOSTIC_ONLY', 'GT01_EVIDENCE_STATUS=ACCEPTED', 'diagnostic status')
...
if __name__ == '__main__':
    ...
    report = {'result': 'PASS' if result.wasSuccessful() else 'FAIL', 'tests_run': result.testsRun,
              ...
              'manifest_sha256': manifest_for(S16Tests.inputs)['manifest_sha256'],
              'limits': 'Static and rehashed mutation checks only; no runtime/critic/legal acceptance.'}
    (validator.OUT / 'selfcheck-s16.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
