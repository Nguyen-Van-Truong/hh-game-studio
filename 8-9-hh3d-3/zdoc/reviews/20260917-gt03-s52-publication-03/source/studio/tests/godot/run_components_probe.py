"""Owned semantic binding, immutable staging, selector and readonly reopen.

Native selector bytes are verified; authenticated save/editor adoption is not.
"""
...
def frozen_run(output):
    ...
    records=[]; finals=[]; store=None; validator=None; reopened=None; native_owner=None; extra_cleanup=None
    extra_is_api=False
    completed=False
    try:
        ...
        for index,bundle in enumerate((first,second)):
            ...
            receipt=validator.validate(command,bundle)
            observation=validator.observation(receipt,bundle)
            final,semantic_receipt=validator.bind_semantics(receipt,bundle)
            semantic_observation=validator.semantic_observation(semantic_receipt,final)
            if final.files!=bundle.files or semantic_observation['semantic']['revision']!=final.scene_revision:
                raise ValueError('native semantic binding changed engine inputs')
            finals.append(final)
            intent=store.prepare(command,final)
            ...
            if actual.files!=final.files or actual.manifest_bytes!=final.manifest_bytes:
                raise ValueError('native staging bytes differ')
            descriptor=store.descriptor(staged)
            previous=store.inspect_selection()
            selection={'generation':index,'identity':'sha256:'+hashlib.sha256(final.manifest_bytes).hexdigest()}
            selection_intent=store.prepare_selection(staged,selection,expected=previous)
            selected=store.select(selection_intent)
            current=store.inspect_selection()
            if current.source_bytes!=selected.source_bytes or current.version!=selected.version:
                raise ValueError('actual selector readback changed')
            for prior,prior_bundle in zip(records,finals):
                if store.read_descriptor(prior['descriptor'])!=prior_bundle:
                    raise ValueError('selector invalidated an earlier immutable bundle')
            row={'command_id':command,'project_revision':final.project_revision,
                'final_manifest':json.loads(final.manifest_bytes),
                'descriptor':descriptor,'validation_receipt':asdict(receipt),
                'semantic_receipt':asdict(semantic_receipt),'semantic_observation':semantic_observation,
                'selector':json.loads(current.source_bytes),
                'observation':observation,'namespace_barrier_readback':True,
                'public_ack':False,'selected_state_verified':False,
                'selector_bytes_verified':True,'candidate_semantic_revision_verified':True,
            ...
            print('HH_COMPONENT_VERIFIED '+json.dumps({'command_id':command,
                'project_revision':final.project_revision}),flush=True)
        ...
        for index,bundle in enumerate(finals):
        ...
        if reopened.inspect_selection().source_bytes!=current.source_bytes:
            raise ValueError('readonly selector differs')
        rejected=False
        try:
    ...
    save(output/'components.json',{'passed':completed,'components':len(records),
        ...
        'selector_bytes_verified':completed,'candidate_semantic_revision_verified':completed})
