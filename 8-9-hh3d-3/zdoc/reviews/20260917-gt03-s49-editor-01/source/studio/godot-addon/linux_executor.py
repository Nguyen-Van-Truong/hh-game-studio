    ...
    def join_readers():
        for thread in readers:
            ...
            try:
                ...
                row['host_error'] = 'CLI_READER_JOIN_' + type(error).__name__
    try:
        ...
        if process:
            try:
                ...
                row['host_error'] = 'CLI_HELPER_CLEANUP_' + type(error).__name__
        if job:
        ...
        if process and not helper_exited and job and job.closed and job.zero_observed:
            try:
                ...
                row['host_error'] = 'CLI_HELPER_REAP_' + type(error).__name__
        join_readers()
        ...
        if process and helper_exited and all(not thread.is_alive() for thread in readers):
            ...
            try:
                ...
                row['host_error'] = 'CLI_PIPE_CLEANUP_' + type(error).__name__
