...
def validate_request(request):
    ...
    need(argv[0] == str(console) and argv[1] == '-B' and launcher.suffix == '.py'
         and '..' not in launcher.parts and launcher.is_relative_to(REVIEWS) and Path(argv[2]).is_absolute(), 'S129_FIXED_PYTHON_REVIEW_LAUNCHER')
    ...
    need(launcher == BASE / 'controller_probe.py' or request['source_files'], 'S129_NONPROBE_CLOSURE_REQUIRED')
    for name, expected in request['source_files'].items():
        path = plain(name)
        need(Path(name).is_absolute() and '..' not in path.parts and path.is_relative_to(HH3D) and sha(path) == expected, 'S129_CLOSURE_PIN')
