...
function Plain([string]$Path){
 ...
 while($cursor){
  if(Test-Path -LiteralPath $cursor){Need (((Get-Item -LiteralPath $cursor -Force).Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0) 'S129_TASK_REPARSE'}
...
Plain $s129TaskDir
Plain $s129RequestPath
if($Command -ceq 'status'){
 # Operational polling reads only COM and small retained task metadata.
 # It does not start Python, hash runtime/source closures, or alter evidence.
 $statusService=New-Object -ComObject 'Schedule.Service';$statusService.Connect()
 $statusTask=ReadTask ($statusService.GetFolder('\'))
 Need ($null -ne $statusTask -and $statusTask.Path -ceq ('\'+$s129TaskName)) 'S129_TASK_NOT_REGISTERED'
 $retainedXml=Join-Path $s129TaskDir 'registered.xml';Plain $retainedXml
 Need ((Get-Item -LiteralPath $retainedXml).Length -le 65536) 'S129_STATUS_XML_SIZE'
 Need ([string]$statusTask.Xml -ceq [IO.File]::ReadAllText($retainedXml)) 'S129_STATUS_DEFINITION_CHANGED'
 [ordered]@{task_path=$statusTask.Path;state=[int]$statusTask.State;instances=$statusTask.GetInstances(0).Count;last_result=[long]$statusTask.LastTaskResult;last_run=$statusTask.LastRunTime.ToString('o');observed_utc=[DateTime]::UtcNow.ToString('o');scope='Scheduler state only; launcher exit requires retained-handle receipt; missing observer terminal remains UNKNOWN';formal_acceptance=$false}|ConvertTo-Json
 exit 0
}
