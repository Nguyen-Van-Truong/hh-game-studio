...
$wrapper=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'diagnose_sequence_s85.py'))
...
$taskName='HHStudio.GT06.gt06-s85-attribution-01'
...
$request=[ordered]@{schema='HH-GT06-S85-RECOVERY-TASK-1';task_path='\'+$taskName;run_id='gt06-s85-attribution-01';wrapper_sha256=(Hash $wrapper);pythonw_sha256=(Hash $pythonw);command='--supervisor';working_directory=$studio;formal_acceptance=$false}
...
$definition=$service.NewTask(0);$definition.RegistrationInfo.Author=$sid;$definition.RegistrationInfo.Description='HH Studio bounded GT06 S85 recovery';$definition.Principal.Id='HHStudioCurrentUser';$definition.Principal.UserId=$sid;$definition.Principal.LogonType=3;$definition.Principal.RunLevel=0;$definition.Settings.Enabled=$true;$definition.Settings.AllowDemandStart=$true;$definition.Settings.AllowHardTerminate=$true;$definition.Settings.RestartCount=0;$definition.Settings.ExecutionTimeLimit='PT24H';$definition.Settings.StartWhenAvailable=$false;$definition.Settings.RunOnlyIfIdle=$false;$definition.Settings.RunOnlyIfNetworkAvailable=$false;$definition.Settings.DisallowStartIfOnBatteries=$false;$definition.Settings.StopIfGoingOnBatteries=$false;$action=$definition.Actions.Create(0);$action.Id='s85-supervisor';$action.Path=$pythonw;$action.Arguments='-B "'+$wrapper+'" --supervisor';$action.WorkingDirectory=$studio
