...
func run(adapter: Adapter) -> void:
	...
	_check_script(snapshot, "initial")
	if _mode == "contract":
	...
	_check_script(snapshot, "after_undo")
...
func _parent_identity_probe(snapshot: Dictionary) -> void:
	...
	_adapter.get_undo_redo().get_history_undo_redo(history_id).clear_history()
	# History custody owns the original detached node and frees it exactly once.
	_check("parent_custody_freed_detached_original", not is_instance_valid(old_parent))
	_check("parent_custody_preserves_replacement", is_instance_valid(replacement) and replacement.get_parent() == root)
...
func _check_script(snapshot: Dictionary, label: String) -> void:
	var root: Node = EditorInterface.get_edited_scene_root()
	var source: String = FileAccess.get_file_as_string("res://scripts/fixture_actor.gd")
	var script: GDScript = root.get_script() as GDScript
	var stored: Dictionary = _row(snapshot, "root").get("stored", {})
	var script_row: Dictionary = stored.get("script", {})
	_check(label + "_script_attached", script != null and script.resource_path == "res://scripts/fixture_actor.gd")
	_check(label + "_script_exact_source_hash", script != null and script.source_code.sha256_text() == source.sha256_text() and script_row.get("source_sha256") == source.sha256_text())
	_check(label + "_script_export_preserved", root.get("fixture_value") == 23 and stored.get("fixture_value") == 23)
