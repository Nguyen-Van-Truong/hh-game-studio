"""Report-only AppContainer launch with an owned temporary profile lifecycle.
...
profile created by this probe may be deleted; all native handles close first.
"""
...
def main():
    out = {"run_id": "GT02-S29-APPCONTAINER-LIFECYCLE-02", "timestamp_utc": datetime.now(timezone.utc).isoformat(),
           "os": platform.platform(), "probe_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
           "scope": "TEMP synthetic cmd.exe diagnostic; unique owned profile lifecycle; not adapter acceptance"}
    ...
    try:
        ...
        moniker = "hh-gt02-s29-" + uuid.uuid4().hex
        ...
        create_hr = U.CreateAppContainerProfile(moniker, moniker, "Owned HH Studio report-only disposable diagnostic", None, 0, C.byref(created_sid))
        out["create_profile_hresult"] = "0x%08X" % (create_hr & 0xffffffff)
        if create_hr != 0:
            raise RuntimeError("Profile create failed; do not delete/reuse pre-existing profile")
        profile_created = True
        assert sid_text(created_sid) == package_string
        A.FreeSid(created_sid)
        folder = W.LPWSTR()
        folder_hr = U.GetAppContainerFolderPath(package_string, C.byref(folder))
        try:
            assert folder_hr == 0 and Path(folder.value).resolve() == profile_path
        finally:
            if folder:
                O.CoTaskMemFree(folder)
        out["profile_folder_exists_after_create"] = profile_path.exists()
        out["profile_mapping_exists_after_create"] = mapping_exists()
        temp_context = tempfile.TemporaryDirectory(prefix="gt02-s29-appcontainer-")
        with temp_context as tmp:
            base = Path(tmp).resolve()
            base.relative_to(Path(tempfile.gettempdir()).resolve())
            assert base.name.startswith("gt02-s29-appcontainer-")
        ...
        if profile_created:
            delete_hr = U.DeleteAppContainerProfile(moniker)
            out["delete_profile_hresult"] = "0x%08X" % (delete_hr & 0xffffffff)
            out["profile_folder_absent_after_delete"] = not profile_path.exists()
            out["profile_mapping_absent_after_delete"] = not mapping_exists()
            if delete_hr != 0 or profile_path.exists() or mapping_exists():
                error = {"type": "CleanupGap", "message": "Owned profile cleanup not proved; inspect exact profile only"}
