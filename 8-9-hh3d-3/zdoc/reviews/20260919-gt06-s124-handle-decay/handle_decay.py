...
ROOT = BASE.parents[2]
RUN_ID = 'gt06-s124-handle-decay-01'
CLOSURE = '739447bd1d812b7e4e76754d7c6659a2ba9c1bc53e9045f1cfc2f2b6379dbf08'
...
def check(root=ROOT):
    ...
    util.need(util.sha(root / 'zdoc/reviews/20260919-gt06-s105-handles/pss_adapter.py') == PSS_SHA256, 'S124_PSS_PIN')
...
def launch():
    ...
    freeze = {'schema': 'S117.coupled-journal.freeze.1', 'run_id': RUN_ID,
        ...
        'scope': 'S124 original campaign prefix, unmodified native, additive host timings; stop at original rejection or after batch6 gate; on editor handle rejection capture immediate/5s type counts and 0/1/3/5s counters without changing gate'}
...
def child(run):
    ...
    observation = load(helpers / 'post_failure_handles.py', 'S124_observation')
    pss = load(helpers / 'pss_adapter.py', 'S124_pss')
    ...
    def binding(sample):
        ...
        util.need(joint['processes']['editor'] == {'pid': probe.pid, 'process_start': probe.process_start}, 'S124_RETAINED_IDENTITY')
    ...
    with ExitStack() as stack:
        for target, name, value in ((commands, 'Journal', MeasuredJournal), (os, 'fsync', fsync),
                                    ...
                                    (campaign, 'screen_sample', screen), (campaign, 'open_probe', open_probe)):
