    ...
    def lookup(self, body, *, authorization, catalog_digest):
        ...
        try:
            ...
            if error.outcome_unknown:
                # A derived-index/cache failure cannot establish that the
                # command was absent. Preserve the retry/reconcile boundary
                # instead of turning it into an ordinary 400 rejection.
                self.sessions.halt()
                return self._receipt(body['command_id'], 'UNKNOWN',
                    'REPLAY_RECONCILE_REQUIRED',
                    next_action='lookup_same_command_id_do_not_replay')
