...
def _operation_descriptor(operation: str) -> dict[str, Any]:
    ...
    publication = operation == "export.publish"
    return {
        ...
        "native_protocol": publication_state.SCHEMA if publication else queue.SCHEMA,
        "writer_lease_required": True,
        "expected_revision": "exact live native revision; owner must compare",
        "dry_run": "schema validation only; no live precondition or effect prediction",
        "diff_supported": False,
        "affected_files": (
            [] if edit else list(publication_state.NAMES) + ["protected manifest and selector"]
            if publication else [_SAVE_SLOTS[operation] + ".blend (owned fixed slot)"]
        ),
        "undo_policy": (
            "native revision-checked transient UI history; newer manual edits must not be overwritten"
            if edit else "no file undo; native save is not protected publication"
            if not publication else "private publication owner policy; no writable restart or automatic replay"
        ),
...
DESCRIPTOR = {
    ...
    "scene_durable": False,
    "native_checkpoint_protected_publication": False,
    "public_ack": False,
    "external_inputs": [],
    "unsupported": [
        "arbitrary paths or blend intake", "linked libraries or textures", "remote code execution",
        "validated native before/after diff", "writable restart", "restored undo history", "durable public ACK",
    ],
}
