"""Owned immutable flat bundle files with a native namespace barrier, no ACK.

The exact ProtectedFileRoot transfers on successful construction. No public
worker paths, selection effect, engine execution or recovered write authority.
"""
...
class ProtectedBundleStore:
    ...
    def prepare(self, command_id: str, bundle) -> ProtectedBundleIntent:
        ...
        with self._mutex:
            ...
            self._records[command_id] = _Record(intent, intent.names, bundle, inventory, size)
            return intent
    ...
    def stage(self, intent: ProtectedBundleIntent) -> ProtectedBundleReceipt:
        with self._mutex:
            ...
            try:
                ...
                _need(self._inventory() == record.inventory, 'PROTECTED_BUNDLE_INVENTORY_CHANGED')
                self._files.check_mutation_available()
                ...
                self._read_record(record)
                _need(self._inventory() == {**record.inventory,
    ...
    def read_descriptor(self, value: dict):
        ...
        with self._mutex:
            ...
            try:
                ...
                _need(self._inventory() == inventory, 'PROTECTED_BUNDLE_INVENTORY_CHANGED')
                return result
            except BaseException as exc:
    ...
    def snapshot(self) -> tuple[ProtectedBundleAttempt, ...]:
        ...
        with self._mutex:
            return tuple(ProtectedBundleAttempt(r.intent.command_id, r.intent.project_revision, r.status,
                r.names, len(r.names), r.reserved_bytes, r.attempted_writes, tuple(r.versions))
