...
class ManagedPipeService:
    ...
    def __init__(self, owner, broker, work, control, credential, *,
        ...
        if (type(session_timeout_ms) is not int or not 1 <= session_timeout_ms <= 60_000
                ...
                or not 1 <= broker.limits.request_timeout_ms <= 30_000):
        ...
        try:
            while not self._stop.is_set():
                if time.monotonic() >= self._deadline:
                    ...
                    return
                server.serve_one(deadline=self._deadline)
    ...
    def _watch_deadline(self):
        # The phase pump can be inside synchronous native I/O. A separate
        # bounded watcher must revoke admission/cancel pipes on wall-clock
        # expiry even while that already admitted phase is unable to return.
