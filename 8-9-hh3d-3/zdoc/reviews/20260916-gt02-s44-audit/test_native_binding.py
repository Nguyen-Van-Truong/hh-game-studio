"""Both native lanes must reject stale or incomplete frozen-source binding."""
import copy
import json
...
results = []
for folder in (verify.IPC, verify.BOUNDARY):
    path = folder/'native-stdout.txt'
    ...
    for case in ('stale_closure', 'file_changed_digest_retained', 'map_missing', 'digest_missing'):
        ...
        if case == 'stale_closure':
            ...
            corrupt['runtime_closure']['files']['host/core/safe_replace.py'] = '0' * 64
