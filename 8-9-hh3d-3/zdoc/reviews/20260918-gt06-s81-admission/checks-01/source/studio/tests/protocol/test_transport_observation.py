...
from studio.host.core.transport import FixtureClient, SessionCredential
...
class TransportObservationTests(unittest.TestCase):
        ...
        for stage in ("request", "getresponse", "read"):
            for error_type, category in errors:
                with self.subTest(stage=stage, error=error_type.__name__):
                    ...
                    with mock.patch("studio.host.core.transport.http.client.HTTPConnection",
                                    return_value=self.connection) as constructor, \
                         mock.patch("studio.host.core.transport.time.monotonic_ns",
    ...
    def test_observation_is_defensive_copy_and_read_only(self):
        ...
        with mock.patch("studio.host.core.transport.http.client.HTTPConnection", return_value=self.connection):
    ...
    def test_next_call_clears_failure_before_request_and_success_keeps_none(self):
        ...
        with mock.patch("studio.host.core.transport.http.client.HTTPConnection", return_value=self.connection):
            ...
            with mock.patch("studio.host.core.transport.time.monotonic_ns", return_value=5_000_000) as clock:
