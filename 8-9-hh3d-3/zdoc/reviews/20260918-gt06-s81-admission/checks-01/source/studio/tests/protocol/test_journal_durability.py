"""Terminal I/O failures through sockets and fresh interpreter recovery.

Promoted from the independent S30 preparatory diagnostic. These regressions
exercise real disposable journals/sockets and inject only the selected I/O
failure; they do not simulate successful recovery or mutate an engine.
"""
...
@contextmanager
def fixture():
    with tempfile.TemporaryDirectory(prefix='gt02-durability-') as directory:
        root = Path(directory)
...
class JournalDurabilityTests(unittest.TestCase):
    def test_reload_barrier_failure_across_fresh_process_and_archive(self):
        scenario('pre_barrier')

    def test_terminal_write_failure_across_fresh_process_and_archive(self):
        scenario('write_before')

    def test_partial_terminal_record_never_authorizes_success_or_retry(self):
        scenario('write_partial')

    def test_terminal_flush_failure_across_fresh_process_and_archive(self):
        scenario('flush_after')

    def test_terminal_fsync_failure_across_fresh_process_and_archive(self):
        scenario('terminal_fsync')
