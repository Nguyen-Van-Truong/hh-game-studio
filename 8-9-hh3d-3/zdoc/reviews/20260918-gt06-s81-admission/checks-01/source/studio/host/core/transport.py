...
class FixtureClient:
    ...
    def __init__(self, port: int, control_port: int, credential: SessionCredential, *, timeout: float = 2.0) -> None:
        ...
        self._last_transport_failure: dict[str, str | float] | None = None

    @property
    def last_transport_failure(self) -> dict[str, str | float] | None:
        """Local bounded observation; never part of a protocol response."""
    ...
    def _call(self, path: str, body: Mapping[str, Any], *, control: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            # Never retain exception text, headers, body, credentials, paths or
            # IDs. The single observation cannot change UNKNOWN or retry policy.
            ...
            self._last_transport_failure = {
                "endpoint": _CLIENT_ENDPOINTS.get(path, "unknown"), "stage": stage,
