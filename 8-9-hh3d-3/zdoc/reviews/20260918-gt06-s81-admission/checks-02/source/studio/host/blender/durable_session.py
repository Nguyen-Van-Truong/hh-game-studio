...
from .writer_journal import BlenderWriterJournal,MAX_WAITERS
...
class DurableBlenderSession:
    ...
    def __init__(self,directory:Path,*,host=None,clock=epoch_ms):
        ...
        self.journal=BlenderWriterJournal(self.directory/'blender-journal.jsonl',limits=JournalLimits(
            max_bytes=16*1024**2,max_records=512,max_pending_commands=MAX_WAITERS+9,
    ...
    def acquire_writer(self,writer,*,ttl_ms=10000):
        ...
        with self._lock:
            ...
            lease=self.journal.acquire_immediate(writer=writer,now_ms=self.clock(),ttl_ms=ttl_ms)
    ...
    def request_writer(self,ticket_id,writer,*,wait_ms=10000,lease_ms=10000):
        queue.c.identifier(ticket_id);queue.c.identifier(writer)
        need(self.host is not None and not self._held and not self._stopped,'BLENDER_SESSION_READONLY_OR_HELD')
        return self._writer_call(self.journal.enqueue_writer,ticket_id=ticket_id,writer=writer,generation=self.host._session,
            now_ms=self.clock(),wait_ms=wait_ms,lease_ms=lease_ms)

    def _writer_call(self,operation,**kwargs):
        try:return operation(**kwargs)
        except JournalError as error:
            if error.outcome_unknown:self._held=True
            raise

    def writer_status(self,ticket_id,writer):
        queue.c.identifier(ticket_id);queue.c.identifier(writer)
        return self._writer_call(self.journal.lookup_writer,ticket_id=ticket_id,writer=writer,now_ms=self.clock())

    def cancel_writer(self,ticket_id,writer):
        queue.c.identifier(ticket_id);queue.c.identifier(writer)
        return self._writer_call(self.journal.cancel_writer,ticket_id=ticket_id,writer=writer,now_ms=self.clock())

    def pump_writers(self):
        need(self.host is not None and not self._held and not self._stopped,'BLENDER_SESSION_READONLY_OR_HELD')
        grant=self._writer_call(self.journal.prepare_next_writer,generation=self.host._session,now_ms=self.clock())
        if grant is None:return None
        wire={name:grant['lease'][name] for name in ('fencing_epoch','expires_ms')}
        try:
            need(self.host.arm_lease(wire)==wire,'BLENDER_LEASE_NATIVE_READBACK')
            return self.journal.complete_writer(grant=grant,now_ms=self.clock())
        except BaseException:
            # Keep the durable grant intent and epoch. Do not blindly arm again.
            self._held=True;raise
    ...
    def stop(self):
        ...
        self._stopped=True
        result=self.host.stop() if self.host is not None else {'stopped':True,'public_ack':False}
        self.journal.stop_writers(now_ms=self.clock())
        return result
