...
def preparation_command(request,config):
    request=validate_request(request)
    operation,slot=preparation(config)
    return {'schema':queue.SCHEMA,'command_id':'pub-'+sha(canonical_bytes(request))[:20],
        'operation':operation,'expected_revision':request['expected_revision'],
        'expected_context':request['expected_context'],'payload':{'slot':slot}}
