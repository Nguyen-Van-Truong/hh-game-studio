...
class IndependentAdmission(SelectorPublicTests):
    def test_adversarial_variants_never_append_or_publish(self):
        ...
        def add(name, mutate):
            ...
            mutate(body)
            body.pop('digest',None)  # Optional derived digest must not mask the targeted rejection.
        ...
        (HERE/'admission-rows-v2.json').write_text(json.dumps(rows,indent=2)+'\n')
...
if __name__=='__main__':
    ...
    result['after']=verify()
    (HERE/'verification-v2.json').write_text(json.dumps(result,indent=2)+'\n')
