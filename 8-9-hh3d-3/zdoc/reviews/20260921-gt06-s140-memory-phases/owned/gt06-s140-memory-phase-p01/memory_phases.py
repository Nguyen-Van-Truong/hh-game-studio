...
PHASE_ORDER = (
    "command_completed", "released_gc", "native_batch_marker", "native_parse",
...
def _read_private_counters(probe: Any) -> _PrivateCounters:
    ...
    try:
        ...
        if not probe.p.GetProcessMemoryInfo(probe.handle, ctypes.byref(counters), counters.cb):
        ...
        return _PrivateCounters(
            ...
            int(counters.page_fault_count), _allocated_blocks(), None,
        )
    except BaseException as error:
        return _PrivateCounters(None, None, None, None, type(error).__name__)
...
class PhaseMemoryRecorder:
    ...
    def bind_host(self, probe: Any = None, identity: Mapping[str, Any] | None = None) -> None:
        ...
        self._identities["host"] = _identity(identity) or _probe_identity(probe)

    def bind_editor(self, probe: Any = None, identity: Mapping[str, Any] | None = None) -> None:
        self._editor_probe = probe
        self._identities["editor"] = _identity(identity) or _probe_identity(probe)
...
class _GCProxy:
    ...
    def collect(self, *args: Any, **kwargs: Any) -> int:
        ...
        self._recorder.record("released_gc")
        return result
...
def install_phase_memory_wrappers(campaign: Any, recorder: PhaseMemoryRecorder) -> Iterator[PhaseMemoryRecorder]:
    ...
    screen_sample invokes the original gate first. A successful boundary only
    ...
    try:
        ...
        def sample_editor(probe: Any, *args: Any, **kwargs: Any) -> Any:
            ...
            recorder.record("native_parse", index)
        ...
        def screen_sample(sample: Mapping[str, Any], baseline: Any, *args: Any, **kwargs: Any) -> Any:
            ...
            if type(index) is int and index >= recorder.prefix - 1:
                raise BoundedPhaseStop(recorder.run_id)
            return result

        patches.extend((
            _Patch(producer_cls, "run_batch", run_batch),
            _Patch(native_log_cls, "wait", wait),
            _Patch(campaign, "sample_editor", sample_editor),
            _Patch(campaign, "assemble_sample", assemble_sample),
            _Patch(campaign, "screen_sample", screen_sample),
            _Patch(campaign, "gc", _GCProxy(recorder, original_gc)),
        ))
        yield recorder
    finally:
