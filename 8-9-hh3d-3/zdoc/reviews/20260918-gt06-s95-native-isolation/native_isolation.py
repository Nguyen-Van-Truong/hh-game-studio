...
def patch_driver(raw, helper, dimensions):
    ...
    prologue = 'func _write_batch() -> void:\n    var ended: int = Time.get_ticks_usec()\n    _heartbeat(true)\n'
    anchor = '    var objects: float = Performance.get_monitor(Performance.OBJECT_COUNT)\n'
    batch_anchor = '    var batch_data: Dictionary = {'
    inserted = ('    _s95_before_batch()\n    if _failed:\n        return\n'
                '    ended = Time.get_ticks_usec()\n')
    heartbeat = '    _heartbeat(true)\n'
    # Retain the original snapshot/root/barrier lifetime at the census. Refresh
    # the real readback time after it; a second genuine heartbeat follows the
    # captured counters, covering hook duration even on the final batch.
    need(section.startswith(prologue) and section.count(anchor) == 1
         and section.count(batch_anchor) == 1
         and section.index(anchor) < section.index('    var memory: Dictionary = {') < section.index(batch_anchor),
         'ISOLATION_COUNTER_PATCH')
    section = section.replace(anchor, inserted + anchor, 1)
    section = section.replace(batch_anchor, heartbeat + batch_anchor, 1)
    ...
    # Reverse each insertion at its exact anchor, preserving the stock heartbeat.
    restored = result[:-(len(body.split(marker, 1)[1]) + 1)]
    restored = restored.replace(heartbeat + batch_anchor, batch_anchor, 1).replace(inserted, '', 1).replace(new, old, 1)
