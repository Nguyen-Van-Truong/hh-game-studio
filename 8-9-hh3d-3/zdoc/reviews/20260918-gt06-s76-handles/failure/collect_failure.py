"""Read-only failed S75 launch-1 inputs; writes only this evidence directory.
...
ROOT = OUT.parents[3]
CAMPAIGN = ROOT / "studio/.local/reviews/gt06-s75-campaign-01"
...
SOURCE_CHECKPOINT = "979f98ef27dbca1613c2382a917b4dd4f73fecb6"
...
write("raw-locator-hashmaps.json", {"created_utc": utc(), "algorithm": "sha256",
    "scope": "Complete inventory of both named raw roots, including caches. No raw files excluded. Bytes remain at raw locators unless selected below.",
...
for name in ["input.json", "input/ack-08.json", "input/start-08.json", "out/ready-08.json",
             "out/ready-09.json", "out/batch-08.json"]:
...
for capture_path in sorted(ATTEMPT.glob("batch-capture-*.json")):
    for key, ref in read(capture_path).items():
        if key != "index":
            source = ATTEMPT / ref["file"]
            assert source.resolve().is_relative_to(ATTEMPT)
            selected.append(("campaign", source))
selected.extend(("campaign", CAMPAIGN / "source/studio" / name)
                for name in read(CAMPAIGN / "campaign.json")["source_files"])
...
for name, expected in campaign["source_files"].items():
    ...
    result = subprocess.run(["git", "show", f"{baseline}:8-9-hh3d-3/studio/{name}"], cwd=ROOT,
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
    row["head_git_show_exit"] = result.returncode
    row["head_git_blob_sha256"] = hashlib.sha256(result.stdout).hexdigest() if result.returncode == 0 else None
    row["head_git_blob_matches"] = row["head_git_blob_sha256"] == expected
    source_rows.append(row)
source_closure = closure(campaign["source_files"])
assert source_closure == "638304f54851366ac2a79a792a8009406fc927c3c6c410ad8f65cc2dd9b14995"
assert campaign["profile_sha256"] == context["profile_sha256"] == "0cd5b53055853b592d4376a6b8a89d69c70ea5facc40dfc25ebd6fcd6dbf4d85"
assert digest(CAMPAIGN / "benchmark-profile.json") == context["profile_sha256"]
assert digest(ATTEMPT / "benchmark-profile.json") == context["profile_sha256"]
...
assert all(row["head_git_blob_matches"] for row in source_rows)
write("source-map-verification.json", {"created_utc": utc(), "source_checkpoint": SOURCE_CHECKPOINT,
    ...
    "all_head_git_blobs_match": True, "git_head": baseline, "all_frozen_files_match": True, "all_checkpoint_git_blobs_match": True,
...
for name in ["editor-host/process-exit.json", "editor-host/capture.json", "host-owner/capture.json", "child-result.json",
             ...
             "project/benchmark/out/index.json", "project/benchmark/input/start-09.json", "project/benchmark/input/ack-09.json"]:
...
facts = {"observed_utc": utc(), "git_head_at_preservation": baseline, "status_launcher_sha256": launcher_digest,
    ...
    "measured_sample_screen_pass_count": 3, "completed_full_runs": 0, "partial_command": failure["partial_command"],
...
assert [r["index"] for r in screens if r["applicable_screen_breaches"]] == [8]
assert screens[8]["applicable_screen_breaches"][0]["metric"] == "held_handles"
write("completeness-and-screen.json", {"observed_utc": utc(),
    ...
    "warmup_batches": 5, "collected_measured_batches": 4, "screen_passed_measured_batches": 3,
    "complete_runs": 0, "baseline_memory": baseline_memory, "failed_sample_memory": samples[8]["memory"],
    "primary_failing_comparison": screens[8]["applicable_screen_breaches"][0],
    "additional_observed_comparison": screens[8]["applicable_screen_breaches"][1],
    ...
    "samples": screens, "next_native_ready_exists": (ATTEMPT / "project/benchmark/out/ready-09.json").is_file(),
...
print(json.dumps({"out": str(OUT), "raw_files": facts["raw_file_count"], "raw_bytes": facts["raw_total_bytes"],
    ...
    "primary_comparison": screens[8]["applicable_screen_breaches"][0], "additional_comparison": screens[8]["applicable_screen_breaches"][1]}))
