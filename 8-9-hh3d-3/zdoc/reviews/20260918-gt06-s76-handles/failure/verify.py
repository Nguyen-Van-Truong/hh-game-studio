"""Separate read-only static recheck (not an independent critic); only emits new reports/seal in this folder."""
import datetime
import hashlib
import json
import os
...
attempt = campaign_root / 'run-00-attempt-01'
...
assert host_start['pid'] == host_exit['pid'] == 16220 and host_exit['exit_code'] == 1
assert editor_start['pid'] == 13024 and not os.path.lexists(attempt / 'editor-host/process-exit.json')
assert import_start['pid'] == import_exit['pid'] == 35452 and import_exit['exit_code'] == 0
...
for index in range(9):
    ...
    assert len(sample['cycles']) == 100
    assert len(sample['effects_per_admission']) == 200 and set(sample['effects_per_admission']) == {1}
    assert sample['dropped_commands'] == sample['dropped_telemetry'] == 0
    assert {k: len(v) for k, v in sample['latency_ms'].items()} == {'admitted': 200, 'inspect': 500, 'rejected': 300}
    ordered = sorted(sample['latency_ms']['inspect'])
    offset = (len(ordered) - 1) * 0.95
    low = int(offset)
    p95 = ordered[low] + (ordered[low + 1] - ordered[low]) * (offset - low)
    workload_rows.append({'index': index, 'measured': index >= 5, 'command_count': 1000,
        'native_cycles': 100, 'inspect_p95_ms': p95,
        'inspect_p95_over_500': p95 > 500, 'max_status_gap_ms': sample['max_status_gap_ms'],
        'stop_receipt_ms': sample['stop_receipt_ms'],
        'note': 'Per-batch descriptive statistics only; not a complete run or dataset percentile verdict'})
...
for index in range(10):
    for number in range(1, 4):
        ...
        stop_count += 1
assert existing_slots == ['run-00-attempt-01']
...
assert scheduler['launch_number'] == 1 and scheduler['campaign_id'] == campaign_root.name
...
result = {'verified_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'status': 'PASS',
    ...
    'batch_references': references, 'sample_identity_rows': 9, 'processes': identities,
    'actual_host_exit': host_exit, 'actual_editor_exit': None, 'actual_import_exit': import_exit,
    'owner_checks': owner_rows, 'sample_workload_rows': workload_rows, 'read_only_observer_exit_zero_count': observations,
...
if '--check' in sys.argv:
    manifest = read(OUT / 'package-manifest.json')
    ...
    assert (OUT / 'package-manifest.sha256').read_text().strip() == manifest_hash + '  package-manifest.json'
    print(json.dumps({'status': 'PASS', 'scope': result['scope'], 'manifest_sha256': manifest_hash,
        'sealed_files': len(manifest['files']), 'raw_files': raw_count,
        'exact_copies': len(copies), 'batch_references': references, 'read_only': True}))
    raise SystemExit(0)
write('verification-recheck.json', result)
project_root = OUT.parents[3]
source_status = []
for name, expected in source.items():
    path = project_root / 'studio' / name
    observed = sha(path) if path.is_file() else None
    source_status.append({'path': name, 'frozen_sha256': expected,
        'live_sha256': observed, 'matches': observed == expected})
head = subprocess.run(['git', 'rev-parse', 'HEAD'], cwd=project_root,
    stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
status = subprocess.run(['git', 'status', '--porcelain=v1', '--untracked-files=no', '--', 'studio'],
    cwd=project_root, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
assert head.returncode == status.returncode == 0
write('final-source-status.json', {'observed_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
    'git_head': head.stdout.decode().strip(), 'git_head_observer_exit': head.returncode,
    'git_tracked_studio_status': status.stdout.decode(), 'git_status_observer_exit': status.returncode,
    'all_live_files_match': all(row['matches'] for row in source_status), 'files': source_status,
    'scope': 'Observation only; concurrent coordinator changes are not changes made by preservation worker'})
