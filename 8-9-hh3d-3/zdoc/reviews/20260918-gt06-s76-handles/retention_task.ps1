...
$name = 'HHStudio.GT06.gt06-s76-retention-01'
$scriptPath = Join-Path $PSScriptRoot 'run_retention_probe.py'
...
if ($Command -eq 'run') {
    if ($null -ne $task -or (Test-Path -LiteralPath (Join-Path $PSScriptRoot 'retention-owner-01'))) { throw 'DIAGNOSTIC_ALREADY_EXISTS' }
    $definition = $service.NewTask(0)
    $definition.RegistrationInfo.Description = 'HH3D S76 supplemental8x100 native retention diagnostic; not full campaign or acceptance.'
    ...
    $record | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $PSScriptRoot 'retention-launch.json') -Encoding utf8
}
