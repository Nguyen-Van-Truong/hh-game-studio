"""Disposable eight x hundred semantic cycles with externally captured PSS points.
...
def run(fast):
    ...
    n.write(root/'diagnostic.json',{'formal_acceptance':False,'full_benchmark':False,'run_id':run_id,
        'source_files':source,'initial_project_files':initial,'native_batches':8,'cycles_each':100,
        'background_cadence_override':6900,'uses_current_guarded_config':True,'idle_seconds':60,
    ...
    try:
        ...
        assert set(observed)=={f'batch-{i:02}' for i in range(8)}|{'idle'}
