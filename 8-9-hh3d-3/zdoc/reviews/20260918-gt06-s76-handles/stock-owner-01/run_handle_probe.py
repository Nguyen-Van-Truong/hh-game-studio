...
arm='responsive' if a.responsive else 'stock';out=base/(arm+'-owner-01');out.mkdir(exist_ok=False)
...
(out/'invocation.json').write_text(json.dumps({'source_files':before,'argv':argv,
    'runner_sha256':hashlib.sha256(runner.read_bytes()).hexdigest(),'timeout_seconds':960,
    'formal_acceptance':False},indent=2)+'\n',encoding='utf-8')
result=owned.run_process(argv,cwd=root,output=out,timeout=960,label='probe')
