...
class HandleProbe:
    ...
    def snapshot(self):
        ...
        try:
            ...
            captured=D()
            self._ok(self.k.PssQuerySnapshot(snapshot,4,C.byref(captured),C.sizeof(captured)), 'PssQuerySnapshot')
            ...
            while True:
                ...
                type_raw=self._wide(entry.type_name,entry.type_length) if entry.flags&1 else None
                typ=type_raw.removesuffix('\0') if type_raw is not None else None
                ...
                row={'handle':int(entry.handle or 0),'flags':entry.flags,'type':typ,
                     'type_name_terminated':type_raw.endswith('\0') if type_raw is not None else None,
                ...
                rows.append(row)
            if len(rows)!=captured.value: raise RuntimeError('PSS_CAPTURED_COUNT_MISMATCH')
        finally:
        ...
        return {'identity':self.identity,'capture_flags':0x3c,'access_mask':self.access,
                ...
                'before_handle_count':before,'after_handle_count':after,'handles_captured':captured.value,'entries':rows,
    ...
    def close(self):
        ...
        if self.handle:
            if not self.k.CloseHandle(self.handle): errors.append('CloseHandle:'+str(C.get_last_error()))
