...
ROOT = BASE.parents[2]
RUN_ID = 'gt06-s145-coupled-64k-candidate-01'
CLOSURE = 'fce149cd3a62e102aaba225794b9a5078cd84d7647cb359f6bbcacb8895ddcde'
...
def helper_layout(root, base, run_id):
    ...
    if owned.is_relative_to((root / 'studio').resolve()):
        raise RuntimeError('S145_HELPER_INSIDE_RUNTIME_SCOPE')
    return owned
...
def boundary_screen(original, sample, baseline, gate_rows):
    ...
    if len(gate_rows) == PREFIX_BATCHES:
        raise Boundary('S145_BOUNDARY_CAPTURED')


def check(root=ROOT):
    support_path = root / 'zdoc/reviews/20260919-gt06-s102-observability/preflight.py'
    util = load(support_path, 'S145_support')
    campaign, _, _, sources = util.load_campaign(root)
    util.need(campaign.closure(sources) == CLOSURE, 'S145_SOURCE_DRIFT')
    util.need(util.sha(root / 'studio/tests/replay/benchmark_native.gd') == NATIVE_HASH, 'S145_NATIVE_DRIFT')
    ...
    util.need(util.sha(binary) == lock['gui_sha256'], 'S145_BINARY_DRIFT')
...
def launch():
    ...
    util.need(not run.exists() and not owned_helpers.exists(), 'S145_FRESH_ID_REQUIRED')
    ...
    freeze = {'schema': 'S145.coupled-journal.freeze.1', 'run_id': RUN_ID,
        ...
        'scope': '64KiB journal-read candidate, original native/gates; stop at original rejection or after batch6 gate'}
    ...
    try:
        ...
        while owner.tick() is None:
            util.need(time.monotonic() - started < OUTER_SECONDS, 'S145_OUTER_LIMIT')
            time.sleep(.1)
        ...
        for name in ('source_unchanged', 'execution_unchanged'):
            if observations.get(name) is not True:
                errors.append((name, RuntimeError('S145_PIN_CHANGED')))
        util.write(run / 'result.json', {'schema': 'S145.coupled-journal.result.1', 'run_id': RUN_ID,
...
def child(run):
    ...
    util = load(helpers / 'support.py', 'S145_support')
    root = Path(freeze['original_root'])
    util.need(Path(__file__).resolve() == helpers / 'coupled_journal.py', 'S145_COPIED_CHILD_REQUIRED')
    util.need(not helpers.resolve().is_relative_to((root / 'studio').resolve()), 'S145_HELPER_INSIDE_RUNTIME_SCOPE')
    execution = json.loads((run / 'execution-source-files.json').read_bytes())
    util.need(all(util.sha(root / p) == h for p, h in execution.items()), 'S145_EXECUTION_DRIFT')
    campaign, _, _, sources = util.load_campaign(root)
    util.need(campaign.closure(sources) == CLOSURE, 'S145_SOURCE_DRIFT')
    ...
    with patch.object(campaign, 'screen_sample', screen):
        try:
            ...
            errors.append(('unexpected_full_run', RuntimeError('S145_BOUNDARY_NOT_REACHED')))
    ...
    util.write(run / 'diagnostic-summary.json', {'schema': 'S145.coupled-candidate.summary.1',
