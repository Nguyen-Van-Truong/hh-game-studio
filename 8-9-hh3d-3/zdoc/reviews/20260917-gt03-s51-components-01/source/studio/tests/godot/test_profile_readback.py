...
class ProfileReadbackTests(unittest.TestCase):
    ...
    def test_actual_saved_observation_matches_without_public_or_process_claim(self):
        # Preserve the S49 native fixture's original source identity. Its data
        # projection remains a useful regression, not a run of the S51 release.
        current = {k:hashlib.sha256(v).hexdigest() for k,v in self.bundle.files.items()}
        historical = self.saved['input_hashes']
        changed = 'addons/hh_studio/scene_commands.gd'
        self.assertEqual(set(current), set(historical))
        self.assertEqual(historical[changed], 'ac4bf9e239b082efe9dd542f14c6cb8e202597bdfa1b8160e5da7905413aa0ea')
        self.assertNotEqual(current[changed], historical[changed])
        self.assertEqual({k:v for k,v in current.items() if k != changed},
                         {k:v for k,v in historical.items() if k != changed})
    ...
    def test_actual_scenestate_paths_differ_from_instantiated_node_paths(self):
        for name in ('transformed_box','typed_scene_override'):
            saved=json.loads(Path(__file__).with_name('fixtures').joinpath('profile-'+name+'.json').read_bytes())
            bundle=factory.compose(saved['scene'].encode(),saved['script'].encode(),
                scene_revision=self.bundle.scene_revision,engine_sha256=self.bundle.engine_sha256)
            self.assertEqual(readback.compare_observation(bundle,saved['observation'])['nodes'],2)
            bad=copy.deepcopy(saved['observation'])
            bad['scene']['nodes'][1]['path']='BoxOne'
            with self.assertRaises(readback.ReadbackError):readback.compare_observation(bundle,bad)
