...
class JournalV2Tests(unittest.TestCase):
    ...
    def test_cancellation_during_native_bootstrap_preserved_after_cleanup(self):
        ...
        with self.assertRaises(journal.PublicationJournalV2Error):self.reopen()
    def test_reopen_rejects_deleted_selector_despite_intact_event_history(self):
        owner=self.create();active=owner._files.root/'active.json';owner.close()
        self.assertEqual(active.parent.parent,self.base)
        active.unlink()
        with self.assertRaises(journal.PublicationJournalV2Error):self.reopen()
    def test_reopen_rejects_recreated_same_bytes_selector_identity(self):
        owner=self.create();active=owner._files.root/'active.json';raw=active.read_bytes();owner.close()
        self.assertEqual(active.parent.parent,self.base)
        active.unlink();active.write_bytes(raw)
        with self.assertRaises(journal.PublicationJournalV2Error):self.reopen()
    def test_uncertain_child_prepare_retains_outer_lifecycle_owner(self):
        owner=self.create();request=prepared(owner)
        error=journal.store_model.ProtectedBundleError('injected',outcome_unknown=True,cleanup_owner=owner._store)
        with mock.patch.object(owner._store,'prepare',side_effect=error):
            with self.assertRaises(journal.PublicationJournalV2Error) as raised:owner.prepare(request,bundle('scene-two'))
        self.assertIs(raised.exception.cleanup_owner,owner)
        with self.assertRaisesRegex(journal.PublicationJournalV2Error,'JOURNAL_HELD'):owner.snapshot()
    def test_uncertain_unused_cancellation_retains_outer_lifecycle_owner(self):
        owner=self.create();request=prepared(owner);request['before_project_revision']=rev('wrong')
        error=journal.store_model.ProtectedBundleError('injected',outcome_unknown=True,cleanup_owner=owner._store)
        with mock.patch.object(owner._store,'cancel_unused_prepare',side_effect=error):
            with self.assertRaises(journal.PublicationJournalV2Error) as raised:owner.prepare(request,bundle('scene-two'))
        self.assertIs(raised.exception.cleanup_owner,owner)
        with self.assertRaisesRegex(journal.PublicationJournalV2Error,'JOURNAL_HELD'):owner.snapshot()
    def test_interrupt_preserved_when_unused_cancellation_also_fails(self):
        owner=self.create();request=prepared(owner)
        cleanup=journal.store_model.ProtectedBundleError('cleanup',outcome_unknown=True,cleanup_owner=owner._store)
        with mock.patch.object(journal.state_model,'reduce_event',side_effect=KeyboardInterrupt), mock.patch.object(owner._store,'cancel_unused_prepare',side_effect=cleanup):
            with self.assertRaises(KeyboardInterrupt) as raised:owner.prepare(request,bundle('scene-two'))
        self.assertIs(raised.exception.cleanup_owner,owner)
        with self.assertRaisesRegex(journal.PublicationJournalV2Error,'JOURNAL_HELD'):owner.snapshot()
