...
class GodotPublicationOwner:
    ...
    def lease(self, body, *, authorization, catalog_digest):
        grant = self._auth(body, 'scene.save', authorization, catalog_digest, {'project_id','ttl_ms'})
        with self._lock:
    ...
    def _save(self, request, grant, lease, before, validated, record):
        ...
        try:
            ...
            terminal = self._journal.commit(command,digest,observed_ms=epoch_ms())
