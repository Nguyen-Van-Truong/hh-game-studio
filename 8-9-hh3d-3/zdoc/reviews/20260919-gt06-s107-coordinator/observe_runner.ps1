# Passive outer Python exit observer. The S107 runner owns/bounds engine Jobs.
param(
  [Parameter(Mandatory=$true)][ValidatePattern('^gt06-s107-preview-[a-z0-9][a-z0-9-]{0,30}$')][string]$RunId,
  [Parameter(Mandatory=$true)][ValidatePattern('^[0-9a-f]{64}$')][string]$ExpectedHelperSha256
)
Set-StrictMode -Version Latest
$ErrorActionPreference='Stop'
$taskRoot=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..\..'))
$taskHelper=Join-Path $taskRoot 'zdoc\reviews\20260919-gt06-s107-preview-cost\run_probe.py'
$taskObservedHash=(Get-FileHash -LiteralPath $taskHelper -Algorithm SHA256).Hash.ToLowerInvariant()
if($taskObservedHash -cne $ExpectedHelperSha256){throw 'S107_OUTER_HELPER_HASH'}
$taskPython=[string](Get-Command python -CommandType Application | Select-Object -First 1 -ExpandProperty Source)
if([string]::IsNullOrWhiteSpace($taskPython) -or !(Test-Path -LiteralPath $taskPython -PathType Leaf)){throw 'S107_OUTER_PYTHON_UNRESOLVED'}
$taskOutput=Join-Path $taskRoot ('studio\.local\reviews\'+$RunId+'-outer')
if(Test-Path -LiteralPath $taskOutput){throw 'S107_OUTER_OUTPUT_EXISTS'}
...
Write-NewReceipt 'request.json' @{
  run_id=$RunId; mode='preview-cost'; observed_utc=[DateTime]::UtcNow.ToString('o');
...
Write-Output ('S107_OUTER_ACTUAL_EXIT='+$taskExit+' RUN='+$RunId)
exit $taskExit
