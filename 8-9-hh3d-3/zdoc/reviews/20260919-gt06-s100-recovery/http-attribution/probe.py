"""Supplemental HTTP attribution on copied S98 history; never acceptance.
...
import threading
import time
from phase_observer import (PhaseRecorder, observed_journal_type, observed_host_type,
                            observed_client_type, observed_connection_type)
...
def main():
    ...
    if (source_before != freeze['source_files'] or history_before != freeze['history_sha256']
            ...
            or any(sha(BASE / name) != digest for name, digest in freeze['helpers'].items())):
    ...
    transformed_snapshot = instrument_snapshot(b.Journal._snapshot, metrics)

    class Recorder(PhaseRecorder):
        def _snapshot(self):
            row = super()._snapshot()
            row['snapshot_subphases'] = metrics.snapshot()
            return row

        def first_failure_seen(self):
            with self._lock:
                return self._first_failure is not None

    recorder = Recorder(event_capacity=8192, active_capacity=64)

    class SnapshotJournal(b.Journal):
        def _snapshot(self, *, synchronize):
            return metrics.run(transformed_snapshot, self, synchronize=synchronize)

    journal_type = observed_journal_type(SnapshotJournal, recorder)
    ...
    def host(_project, path, value):
        return Host('gt06.s100.http-attribution.01', path, value)

    producer = None
    report = {'schema': 'HH-GT06-S100-HTTP-ATTRIBUTION-1',
              ...
              'scope': 'New project and producer over exact postterminal S98 history; up to twenty ten-command HTTP diagnostics, not an S98 replay or benchmark',
    ...
    try:
        ...
        producer = b.CommandProducer(output / 'commands', 'gt06.s100.http-attribution.01')
        for index in range(GROUPS):
            timing.phase = metrics.phase = 'http'
            try:
            ...
            # Observe only after the unchanged group/retry path has returned.
            if recorder.first_failure_seen():
                report['stopped_after_first_lookup_failure'] = True
                break
        report['completed'] = report['groups_completed'] == GROUPS
        ...
        timing.phase = metrics.phase = 'close'
        ...
        # Patches stay installed through cleanup; neither trace dump is inside
        # delegated HTTP/journal work. Keep the first-failure window unchanged.
        write(output / 'phases.json', recorder.snapshot())
        report['snapshot_subphases'] = metrics.snapshot()
        stack.close()
    ...
    return 0 if (report['completed'] and report['closed'] and not any(report.get('threads_alive', []))
                 ...
                 and report['loaded_source_matches_freeze'] and report['helpers_unchanged']) else 1
