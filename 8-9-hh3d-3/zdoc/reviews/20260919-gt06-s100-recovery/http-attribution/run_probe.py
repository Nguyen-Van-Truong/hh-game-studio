...
ROOT = BASE.parents[3]
ORIGINAL = ROOT / 'studio/.local/reviews/gt06-s98-campaign-01/run-00-attempt-01'
...
def main():
    if sys.argv[1:] != ['--launch']:
        ...
        return 2
    run = BASE / 'run-01'
    prepared = json.loads((BASE / 'prepared-manifest.json').read_text(encoding='utf-8'))
    helper_hashes = {name: sha(BASE / name) for name in HELPERS}
    if prepared['helpers'] != helper_hashes:
        raise ProbeFailure('RUNNER_COPY_MISMATCH')
    if (sha(Path(sys.executable)) != prepared['python_sha256']
            or sha(ROOT / 'studio/build/bootstrap/run_fixture.py') != prepared['runner_sha256']):
        raise ProbeFailure('RUNNER_COPY_MISMATCH')
    ...
    campaign.load_fixture()  # Import only: include the formal factory/bundle closure.
    originals = campaign.source_files()
    if len(originals) != 51 or originals != prepared['current_studio_source_files']:
        raise ProbeFailure('CURRENT_CLOSURE_CHANGED_DURING_FREEZE')
    frozen = {}
    ...
    if history_before != prepared['history_sha256']:
    ...
    freeze = {'source_files': frozen, 'current_studio_source_files': originals,
              ...
              'source_kind': 'current source51 frozen at launch; S98 supplies exact postterminal history only',
              's98_history_provenance_manifest_sha256': sha(ORIGINAL / 'source-files.json'),
              ...
              'helpers': helper_hashes, 'prepared_manifest_sha256': sha(BASE / 'prepared-manifest.json'),
    ...
    spec = importlib.util.spec_from_file_location('s100_owned_runner', runner)
    ...
    result.update(formal_acceptance=False, eligible_for_dataset=False, engine_runs=0,
        ...
        helpers_unchanged=all(sha(BASE / name) == digest for name, digest in helper_hashes.items()))
    ...
    ok = (result.get('exit_code') == 0 and result.get('wrapper_exit_code') == 0
          ...
          and all(result[key] for key in ('source_original_unchanged', 'source_copy_unchanged',
                  'history_unchanged', 'runner_original_unchanged', 'runner_copy_unchanged', 'probe_unchanged', 'helpers_unchanged')))
