...
def selected(row):
    # Full frozen runtime stays hashed in place; its security implementation
    # contains credential-format literals. Do not duplicate it into the packet.
    return not row['path'].startswith('source/studio/')
...
def collect():
    ...
    write('preserved-byte-manifest.json', dict(created_utc=utc(), files=copies,
        excluded_from_copy=[dict(raw_root=key, **row, reason='Frozen runtime source retained in original and fully hashed, not duplicated') for key, group in before.items() for row in group['files'] if not selected(row)],
        raw_copy_policy='All eligible non-cache/non-sensitive raw evidence except frozen runtime source; three launch records and two executed diagnostic helpers copied; exact unmodified bytes',
    ...
    write('collection.json', dict(created_utc=utc(), formal_acceptance=False, status='PRESERVATION_COLLECTED',
        nonexcluded_raw_files=sum(len(v['files']) for v in before.values()), exact_copies=len(copies), metadata_only_excluded=sum(len(v['excluded_metadata_only']) for v in before.values()),
...
def verify():
    ...
    assert {(r['raw_root'], r['raw_path']) for r in rows} == {(key, row['path']) for key, group in index['inventories'].items() for row in group['files'] if selected(row)}
    ...
    result = dict(status='PRESERVATION_RECHECK_VERIFIED', formal_acceptance=False, raw_nonexcluded_files=sum(len(v['files']) for v in index['inventories'].values()), exact_copies=len(rows),
