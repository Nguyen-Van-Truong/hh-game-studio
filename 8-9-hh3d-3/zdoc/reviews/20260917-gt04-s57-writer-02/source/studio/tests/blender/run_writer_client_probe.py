...
CLIENT_CHECKS = frozenset({
    ...
    "undo_observed_diff", "redo_observed_diff", "export_protected_committed",
    "export_exact_retry", "export_common_lookup",
})
...
def client():
    ...
    try:
        ...
        def call(path, body, *, role="writer", port=None):
            ...
            limit = 25 if body.get('operation') == 'export.publish' else 5
        ...
        check("writer_discovery_exact", status == 200 and discovery.project_id == config["project_id"]
              ...
              and {cap.operation for cap in discovery.capabilities} == catalog.EDIT_OPERATIONS | {catalog.READ, 'export.publish'}
        ...
        def request(key, operation, state, arguments=None, *, expired=False):
            ...
            maximum = (catalog.MAX_LEASE_MS if operation == 'export.publish' else
        ...
        export_request = request('Writer.Export', 'export.publish', final)
        ...
        check('export_protected_committed', status == 200 and export_response.status.value == 'COMMITTED'
        ...
        check('export_exact_retry', status == 200 and repeated == export_wire)
        status, _, repeated = call('/v1/lookup', {**project, 'command_id': export_request.command_id})
        check('export_common_lookup', status == 200 and repeated == export_wire)
...
def frozen(output, binary, run_id, closure):
    ...
    try:
        ...
        publisher = BlenderPublicationOwner.create(output, session=durable)
        owner = BlenderWriterClientOwner.from_session(durable, publication=publisher)
        operations = catalog.EDIT_OPERATIONS | {catalog.READ, 'export.publish', "control.lookup", "control.stop"}
...
def unit_patterns(snapshot, suite):
    ...
    patterns = ["test_writer_client_probe.py", "test_client_write_catalog.py", "test_client_writer_session.py",
                ...
                'test_client_writer_publication.py', 'test_publication_deadline.py']
