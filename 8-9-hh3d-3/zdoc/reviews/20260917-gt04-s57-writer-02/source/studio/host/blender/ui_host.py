...
class BlenderUIHost:
    ...
    def preview(self, command, *, lease, deadline_ms, ttl_ms=1000):
        """Bounded private observation; the original apply ID remains unused."""
        need(type(deadline_ms) is int and 1 <= deadline_ms < 2**53, 'BLENDER_ABSOLUTE_DEADLINE_LIMIT')
        need(type(ttl_ms) is int and 1 <= ttl_ms <= 5000, 'BLENDER_PREVIEW_TTL_LIMIT')
        need(type(lease) is dict and set(lease) == {'fencing_epoch', 'expires_ms'}
             and all(type(lease[name]) is int and 1 <= lease[name] < 2**53 for name in lease),
             'BLENDER_PREVIEW_LEASE_REQUIRED')
        need(not getattr(self, '_recovery_readonly', False), 'BLENDER_RECOVERY_READONLY')
        need(not self._stopped, 'BLENDER_STOPPED')
        need(source_files() == self._source, 'BLENDER_SOURCE_CHANGED')
        self._check_project()
        return self._ask('data', 'preview', {
            'command': command, 'ttl_ms': ttl_ms, 'lease': lease, 'deadline_ms': deadline_ms,
        }, timeout=min(5, ttl_ms / 1000 + 1))
