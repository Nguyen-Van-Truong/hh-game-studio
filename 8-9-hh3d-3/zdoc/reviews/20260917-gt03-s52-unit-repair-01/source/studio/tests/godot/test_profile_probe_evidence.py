...
class ProfileProbeEvidenceTests(unittest.TestCase):
    def setUp(self):
        ...
        # Historical S49 property rows remain historical. Two trusted addon
        # implementations changed by S52; all fixture data stays byte-exact.
        # This synthetic lifecycle is not native execution of today's addons.
        changed = {
        ...
        for path, historical in changed.items():
            self.assertEqual(saved['input_hashes'][path], historical)
            self.assertNotEqual(hashes[path], historical)
        self.assertEqual({k:v for k,v in hashes.items() if k not in changed},
                         {k:v for k,v in saved['input_hashes'].items() if k not in changed})
