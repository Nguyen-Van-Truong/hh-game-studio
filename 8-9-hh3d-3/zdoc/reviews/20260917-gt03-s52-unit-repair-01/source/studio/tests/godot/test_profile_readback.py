...
class ProfileReadbackTests(unittest.TestCase):
    ...
    def test_actual_saved_observation_matches_without_public_or_process_claim(self):
        # Preserve original S49 bytes. This is only a pure property replay;
        # the two changed addon implementations have separate native S52 proof.
        ...
        changed = {
            ...
            'addons/hh_studio/plugin.gd': '2a5eeb10e9adf0b705293545936f2abd8e352cfbe6b4df7d13411b0cf610243b',
        }
        self.assertEqual(set(current), set(historical))
        for path, original in changed.items():
            self.assertEqual(historical[path], original)
            self.assertNotEqual(current[path], original)
        self.assertEqual({k:v for k,v in current.items() if k not in changed},
                         {k:v for k,v in historical.items() if k not in changed})
