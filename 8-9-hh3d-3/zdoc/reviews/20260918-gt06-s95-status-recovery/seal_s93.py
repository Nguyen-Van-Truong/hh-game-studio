"""Preserve terminal S93 evidence; never launches or accepts a benchmark."""
...
ROOT = BASE.parents[2]
RAW = ROOT / 'studio/.local/reviews/gt06-s93-sparse-attribution-01'
HELPER = ROOT / 'zdoc/reviews/20260918-gt06-s93-sparse-attribution'
...
def seal():
    ...
    assert failure['code'] == 'CAMPAIGN_STATUS_GAP' and failure['completed_batches'] == 7
    ...
    task = subprocess.run(['schtasks.exe', '/Query', '/TN', r'\HHStudio.GT06.gt06-s93-sparse-attribution-01', '/XML'], capture_output=True)
    ...
    sources = {p for p in sources if not p.name.startswith(('command-', 'sample-preview-')) or p.name in ('command-06.json', 'sample-preview-06.json')}
    ...
    for source in sorted(sources):
        ...
        parent = RAW if domain == 'raw' else OBSERVER if domain == 'observer' else HELPER / 'launch-01'
...
if __name__ == '__main__':
    if sys.argv[1:] == ['--verify']:
        ...
        raise SystemExit('Usage: seal_s93.py [--verify]')
