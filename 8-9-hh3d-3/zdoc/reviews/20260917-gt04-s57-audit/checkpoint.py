"""Exact S57 byte inventory; excludes private working roots and lock files."""
...
def inventory():
    ...
    source=json.loads((REVIEWS/'20260917-gt04-s57-writer-02/source-closure.json').read_bytes())['files']
    for name,digest in source.items():add(ROOT/'studio'/name,digest)
    partial=REVIEWS/'20260917-gt04-s57-writer-01'
    ...
    for name in ('source-closure.json','unit-host.json','unit-stdout.txt','unit-stderr.txt',
                 'native-stdout.txt','native-stderr.txt','interruption.json'):add(partial/name)
    for name in ('.gitattributes','AGENTS.md','zdoc/8-9-godot-blender-agent-studio-plan.txt'):add(ROOT/name)
    add(REVIEWS/'20260917-gt04-s56-audit/git-HEAD.json')
    for path in HERE.iterdir():
        if path.is_file() and path.name not in ('files.json','paths.nul','git-index.json','git-HEAD.json'):add(path)
