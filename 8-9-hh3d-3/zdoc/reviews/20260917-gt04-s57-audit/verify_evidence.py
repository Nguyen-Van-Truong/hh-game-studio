"""File-only S57 integration audit; frozen imports and explicit portable files."""
...
PACKAGE = REVIEWS / '20260917-gt04-s57-writer-02'
SOURCE = PACKAGE / 'source/studio'
CLOSURE = 'c7eb8fdf0fa9d564259657cbe2e3eaec7205eee36f4c5db22b500b2923e384d4'
...
def verify():
    ...
    capture, report, gui, launch, runtime = base.package_facts(PACKAGE, manifest, runner, 540, 'writer-native.json')
    ...
    calls = client['calls']
    need(len(calls) == 32 and len(client['checks']) == 45 and len(report['checks']) == 10, 'bounded full integration trace')
    values = []
    for call in calls:
    ...
    need({v['operation'] for v in discoveries[0]['capabilities']} == catalog.EDIT_OPERATIONS|{catalog.READ,'export.publish'}
         and [v['operation'] for v in discoveries[1]['capabilities']] == [catalog.READ]
         and discoveries[2]['capabilities'] == [], 'connected/granted discovery and Stop')
    ...
    need(len(records)==17 and len(history)==8 and all(e['response'] is not None for e in history.values()), 'eight exact intent/terminal pairs')
    ...
    for call,value in zip(calls,values):
        ...
        if request.operation=='export.publish': continue
        ...
        previous=scene
    need(len(successful)==8 and len(previews)==5, 'all effect and preview bindings present')
    need({'BLENDER_WRITER_GRANT_REQUIRED','BLENDER_LEDGER_COMMAND_CONFLICT','BLENDER_LEDGER_COMMAND_NOT_FOUND',
         'BLENDER_LEDGER_COMMAND_OWNER','BLENDER_OPERATION_FORBIDDEN','BLENDER_DEADLINE_EXPIRED','UNSUPPORTED_ROUTE'}<=denials,
    ...
    return {'passed':True,'formal_acceptance':False,'source_closure_sha256':CLOSURE,'source_files':len(manifest['files']),
        'unit_tests':540,'client_checks':45,'native_checks':10,'http_calls':len(calls),'common_commands':len(history),
        'preview_operations':len(previews),'assertions':len(base.CHECKS),'native_handles_opened':0,
        'scope':'GT04 S57 component integration; save/checkpoint and final two critics still pending'}
