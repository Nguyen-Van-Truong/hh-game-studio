...
def classify_cycle(key, group, raw, lifecycle):
    ...
    # S103 emits a process-entry frame before the save call and advances the
    # authored frame while the save dispatch is in progress.  Requiring
    # process_entry_frame == save_process_exit_frame rejects valid records.
    # Keep the original monotonic/next-dispatch binding, but permit the
    # observed in-dispatch advance and retain the signal-frame bounds.
...
def analyze(stdout_path, native_source, batch_paths=(), lifecycle_path=None, binding_path=None):
    ...
    return {"schema_id": "hh-studio.s103-native-save-reading", "schema_version": "1.0.0",
            ...
            "reader_version": "s104-frame-order-v1",
