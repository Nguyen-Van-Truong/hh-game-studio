...
All runtime and original probe code comes from the S60 snapshot. This supplement
...
STOP = ('stop_export','stop_before_selector','stop_after_selector','stop_after_terminal')
...
def ui(output):
    ...
    for lane in ('gui','background'):
        ...
        for key, folder in [('BLENDER_USER_RESOURCES','user'),('TEMP','temp'),('TMP','temp')]:
            path=output/(lane+'-'+folder); path.mkdir(exist_ok=True); env[key]=str(path)
    ...
    print('S60_UI_COMPLETE '+json.dumps({'passed':True,'checks':sum(checks.values())}),flush=True)
    return 0
...
def child(lane, output):
    ...
    if lane in MODULES:
        ...
        if lane=='deadline': return module.frozen(output,BINARY,'GT04-S60-MATRIX-DEADLINE',CLOSURE)
...
def evaluate(lane, output, host):
    ...
    if lane in CRASH+STOP:
        ...
        if lane=='ui':
            checks=sum(report['checks'].values()); marker='S60_UI_COMPLETE '
        else:
...
def run(lane):
    binding=verify()
    output=ROOT/'studio/.local/reviews/20260917-gt04-s60-matrix'/lane
...
if __name__=='__main__':
    ...
    if args.child: raise SystemExit(child(args.child,ROOT/'studio/.local/reviews/20260917-gt04-s60-matrix'/args.child))
