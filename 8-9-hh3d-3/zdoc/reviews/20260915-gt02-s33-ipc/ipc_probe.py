...
def run_case(executable, scratch, store, owner, package, mode):
    ...
    sddl = f'D:P(A;;FA;;;{owner})(A;;0x12019b;;;{package_string})S:(ML;;NW;;;LW)'
    ...
    security, rendered = C.c_void_p(), W.LPWSTR()
    try:
        assert A.GetSecurityInfo(pipe,1,0x17,None,None,None,None,C.byref(security))==0
        checked(A.ConvertSecurityDescriptorToStringSecurityDescriptorW(security,1,0x17,C.byref(rendered),None))
        record['effective_pipe_sddl']=rendered.value
    finally:
        if rendered:K.LocalFree(rendered)
        if security.value:K.LocalFree(security)
    ...
    try:
        ...
        assert record['native']['second_pipe_instance_error']==5
...
def main():
    ...
    out['imported_source_sha256']={name:digest(PREVIOUS/name) for name in ('boundary_probe.py','windows_api.py')}
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='gt02-s33-ipc-') as tmp:
            ...
            saved_binary=raw_dir/(RUN_ID+'-'+run_uuid+'.exe')
            with saved_binary.open('xb') as stream:stream.write(executable.read_bytes())
            out['native_executable_relative']=saved_binary.relative_to(PRODUCT).as_posix()
    ...
    public=json.loads(re.sub(r'S-1-5-21-\d+-\d+-\d+-\d+','<HOST_ACCOUNT_SID>',json.dumps(public)))
