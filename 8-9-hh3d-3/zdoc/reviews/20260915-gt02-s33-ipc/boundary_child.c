...
int wmain(void) {
    ...
    file = CreateNamedPipeW(endpoint, PIPE_ACCESS_DUPLEX | FILE_FLAG_FIRST_PIPE_INSTANCE,
                            PIPE_REJECT_REMOTE_CLIENTS, 1, 4096, 4096, 3000, NULL);
    fprintf(log, "\"second_pipe_instance_error\":%lu,", file == INVALID_HANDLE_VALUE ? GetLastError() : 0);
