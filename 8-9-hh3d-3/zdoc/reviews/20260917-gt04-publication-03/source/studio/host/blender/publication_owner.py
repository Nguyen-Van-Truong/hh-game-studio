...
class BlenderPublicationOwner:
    ...
    def _native(self,key,operation,lease,request=None):
        command={'schema':queue.SCHEMA,'command_id':key,'operation':operation,
            ...
            'payload':{'slot':'export'} if operation=='export.prepare' else {}}
        return self.host.execute(command,lease={'fencing_epoch':lease.fencing_epoch,'expires_ms':lease.expires_ms})
    ...
    def publish(self,request,lease):
        ...
        with self._mutex:
            ...
            self._authorize(lease);self.files.check_mutation_available()
            ...
            try:
                ...
                token='pub-'+digest[:20]
                prepared=self._native(token,'export.prepare',lease,request)
                ...
                prepared=prepared['result'];self._authorize(lease)
                self._job=ExportJob(self.host,prepared);export=self._job.run()
                ...
                after=self._native(token+'-after','scene.inspect',lease)
                need(after['state']=='COMPLETED' and after['result']==prepared['after'],'PUBLICATION_SCENE_CHANGED_DURING_EXPORT')
                self._authorize(lease)
                artifacts={name:self._put(name,raw) for name,raw in zip(model.NAMES,(blend,glb))}
                manifest={'schema':model.SCHEMA,'command_id':key,'request_sha256':digest,
                    ...
                    'public_ack':False}
                staged=self._put('manifest.json',canonical_bytes(manifest))
                ...
                self._authorize(lease);selected=model.selection(self._state['staged'])
                self._append('SELECTING',selector=selected);self._authorize(lease)
    ...
    def _read_bundle(self,staged):
        ...
        model.exact(manifest,('schema','command_id','request_sha256','generation','scene_revision','snapshot','snapshot_native_json','artifacts',
