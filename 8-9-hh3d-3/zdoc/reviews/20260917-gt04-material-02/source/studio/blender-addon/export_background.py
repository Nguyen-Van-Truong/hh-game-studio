...
def main():
    ...
    profile.need(len(args)==1 or (len(args)==2 and args[1] in ('--admission-probe','--pause-probe','--oom-probe')),
                 'EXPORT_FIXED_ARGUMENTS')
    ...
    negative=None
    if len(args)==2 and args[1]=='--admission-probe':
        path=HERE.parent/'tests/blender/export_negative_probe.py'
        spec=importlib.util.spec_from_file_location('_gt04_native_admission_probe',path)
        module=importlib.util.module_from_spec(spec);exec(compile(path.read_bytes(),str(path),'exec'),module.__dict__)
        negative=module.run(bpy,profile,root)
        profile.need(owner.inspect()==before,'EXPORT_PROBE_RESTORE')
    elif len(args)==2:
        import os,time
        (root/'fault-ready.json').write_text(json.dumps({'pid':os.getpid(),'phase':'validated-before-export',
            'fault':args[1],'public_ack':False})+'\n',encoding='utf-8')
        if args[1]=='--pause-probe':time.sleep(10)
        else:bytearray(3*1024**3)  # Exceeds the verified 2-GiB owned Job limit.
    result=bpy.ops.export_scene.gltf(filepath=str(target),check_existing=False,export_format='GLB',
        export_materials='EXPORT' if admitted['materials'] else 'NONE',export_animations=False,export_skins=False,export_morph=False,
    ...
    report={'schema':'HH-BLENDER-EXPORT-RESULT-1','profile':admitted,'input_sha256':expected['input_sha256'],
        ...
        'public_ack':False,'native_finished':True,'context_unchanged':True,'admission_probe':negative}
