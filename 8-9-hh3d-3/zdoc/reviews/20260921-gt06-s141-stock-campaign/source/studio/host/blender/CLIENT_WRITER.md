...
transport keeps its route allowlist. Preview accepts the same typed mutation
...
File preview also checks the attached publication owner's empty capacity and
protected namespace, then the live bounded export profile and fixed save slot.
It reports planned bundle creation and the staging slot; artifact hashes remain
unknown until actual background reopen/export and protected publication. It
creates no files or ledger rows and does not consume the apply command ID.
Admission and publication repeat their own checks; a preview is not a permit.
...
Background Stop before releasing the child gate reports `EXPORT_STOPPED`;
Stop after gate release reports `EXPORT_CANCELLED` and drains the same owned
process/Job. Neither outcome claims a completed export.
