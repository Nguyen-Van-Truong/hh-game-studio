...
def stage_fixture_release(store: PrivateBlobStore, *, project_id: str, source_revision: str,
                          ...
                          assets: dict[str, bytes], release_id: str | None = None) -> StagedRelease:
    ...
    # A trusted selector may mint/persist this ID before staging begins. This
    # argument is not a wire capability and cannot choose a filesystem name.
    if release_id is None:
        release_id = 'release-' + uuid.uuid4().hex
    _matches(_RELEASE_ID, release_id, 'INVALID_RELEASE_DESCRIPTOR')
    try:
        records = []
