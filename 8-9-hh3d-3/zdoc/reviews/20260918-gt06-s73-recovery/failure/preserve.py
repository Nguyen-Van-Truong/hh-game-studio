"""Read-only S71 inputs; write only this bounded failure-evidence directory."""
...
ROOT = OUT.parents[3]
CAMPAIGN = ROOT / "studio/.local/reviews/gt06-s71-campaign-01"
...
for pattern in ["joint-*.json", "batch-capture-*.json"]:
    selected.extend(("campaign", p) for p in sorted(ATTEMPT.glob(pattern)))
for name in ["input.json", "input/ack-32.json", "input/start-32.json", "out/ready-32.json",
             "out/ready-33.json", "out/batch-32.json"]:
...
for name in ["editor-host/process-exit.json", "editor-host/capture.json", "host-owner/capture.json", "child-result.json",
             ...
             "project/benchmark/out/index.json", "project/benchmark/out/batch-33.json", "project/benchmark/out/batch-34.json",
             "project/benchmark/input/start-33.json", "project/benchmark/input/ack-33.json"]:
...
write("terminal-facts.json", {"observed_utc": utc(), "git_head_at_preservation": baseline,
    ...
    "measured_batches_completed": failure["completed_batches"] - 5,
    "last_partial_command": failure["partial_command"]["commands"][-1],
