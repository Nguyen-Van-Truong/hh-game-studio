...
def main():
    ...
    try:
        ...
        before=old.inspect()
        report['initial_script_native']=old._exchange('script_state',{})
        initial_script=old.script_observation()
        ...
        created_before_retry={path.name for path in output.glob('editor-*') if path.is_dir()}
        try:
            owner.EditorOwner.open_selected_generation(old,retired,output,candidate,editor_binary=binary,
                selection=selection,semantic_bytes=expected_bytes)
        except owner.EditorOwnerError as error:
            check('same_retirement_second_successor_rejected',error.code=='EDITOR_SUCCESSOR_ALREADY_ATTEMPTED')
        else:
            check('same_retirement_second_successor_rejected',False)
        check('second_successor_no_native_launch_directory',created_before_retry==
            {path.name for path in output.glob('editor-*') if path.is_dir()})
