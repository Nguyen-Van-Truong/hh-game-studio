...
class JournalV3Tests(unittest.TestCase):
    ...
    def capture(self, owner, value=None, command='command.save'):
        ...
        admission = {'session_id': 'synthetic-auth', 'catalog_digest': rev('catalog'),
                     ...
                     'deadline_ms': t + state.MAX_SAVE_ADMISSION_MS,
                     'lease_expires_ms': t + state.MAX_SAVE_ADMISSION_MS}
    ...
    def test_expired_stage_deadline_rejects_before_native_create(self):
        owner = self.create(); value = self.capture(owner)
        history = [parse_json(raw) for raw in owner._state.events]
        admission = history[1]['admission']
        self.assertEqual(admission['deadline_ms'] - admission['admitted_ms'], 90000)
        state.replay(history)  # The exact upper bound is accepted.
        admission['deadline_ms'] += 1; admission['lease_expires_ms'] += 1
        with self.assertRaisesRegex(state.PublicationError, 'ADMISSION'): state.replay(history)
        owner.prepare('command.save', rev('command.save'), value, observed_ms=now(owner))
        command = owner.lookup('command.save')
        deadline = command['capture_prepared']['admission']['deadline_ms']
        before = set(p.name for p in owner._files.root.iterdir()); head = owner._head
        with mock.patch.object(owner._store, 'stage', side_effect=AssertionError('must not stage')) as stage:
            with self.assertRaisesRegex(state.PublicationError, 'DEADLINE'):
                owner.stage_prepared('command.save', rev('command.save'), observed_ms=deadline)
        stage.assert_not_called()
        self.assertEqual(set(p.name for p in owner._files.root.iterdir()), before)
        self.assertEqual(owner._head, head)
        self.assertEqual(owner.lookup('command.save')['phase'], 'PREPARED')
