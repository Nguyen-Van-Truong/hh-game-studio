...
class RecoveryJournalNativeTests(unittest.TestCase):
    ...
    def test_before_cas_load_plan_restores_only_unchanged_last_good(self):
        ...
        snapshot,command,bundle,context=recovered.inspect_command('command.one',helpers.fixtures.rev('command.one'))
        self.assertEqual(snapshot['selected'],context['selected'])
        self.assertEqual(bundle.manifest_bytes,canonical_bytes(snapshot['selected']['bundle_manifest']))
        self.assertEqual(command['phase'],'ACTIVATING')
        context['selected']['selector_version']['file_id']='f'*32
        self.assertEqual(recovered.inspect_command('command.one',helpers.fixtures.rev('command.one'))[3]['selected'],snapshot['selected'])
        ...
        # A STOPPED record beyond custody must not erase the earlier witnessed
        # recovered response when reopen appends its mandatory HOLD.
        with mock.patch.object(again._custody,'persist_binding',side_effect=OSError('stop custody cut')):
            with self.assertRaises(helpers.journal.PublicationJournalV4Error):
                again.stop_recovery(reason='user.stop',observed_ms=clock())
        again.close();stopped=self.open_recovery()
        self.assertIs(stopped.snapshot()['stopped'],True)
        self.assertEqual(stopped.snapshot()['recovery']['attempts'][-1]['phase'],'TERMINAL')
        self.assertEqual(stopped.recovered_response('command.one',helpers.fixtures.rev('command.one')),response)
        self.assertEqual(stopped.selection_facts(),old)
        self.assertIs(stopped.inspect_command('command.one',helpers.fixtures.rev('command.one'))[0]['stopped'],True)
        # A previous successful inspection cannot mask a lost native owner.
        stopped._blobs.close()
        with self.assertRaises(ValueError):stopped.inspect_command('command.one',helpers.fixtures.rev('command.one'))
    ...
    def test_stop_before_reconcile_survives_readonly_reopen_and_denies_admission(self):
        owner=self.create();old=owner.selection_facts();self.activate(owner);owner.close()
        recovered=self.open_recovery()
        recovered.stop_recovery(reason='user.stop',observed_ms=clock())
        self.assertIs(recovered.snapshot()['stopped'],True)
        head=recovered._head
        recovered.stop_recovery(reason='user.stop',observed_ms=clock())
        self.assertEqual(recovered._head,head)
        recovered.close();again=self.open_recovery()
        self.assertIs(again.snapshot()['stopped'],True)
        self.assertEqual(again._head,head);self.assertEqual(again.selection_facts(),old)
        with self.assertRaisesRegex(recovery.RecoveryError,'RECOVERY_STOPPED'):
            self.reconcile_synthetic(again)
        self.assertEqual(again._head,head)
