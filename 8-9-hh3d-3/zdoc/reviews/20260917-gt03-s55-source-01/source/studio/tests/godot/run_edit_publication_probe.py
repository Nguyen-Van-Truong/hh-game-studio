    ...
    try:
        ...
        def call(path,body,*,control=False,bearer=None):
            connection = http.client.HTTPConnection('127.0.0.1',
                server.stop_port if path == '/v1/stop' else server.control_port if control else server.port,timeout=95)
            try:
        ...
        def edit(command,operation,target,payload):
            ...
            # Each separately admitted command gets a full lease budget. Native
            # journal barriers across six edits must not exhaust one old lease.
            if commands:
                previous_fence=lease['fencing_epoch']
                credential=owner.sessions.rotate(credential)
                status,lease,_=call('/v1/lease',{'project_id':owner.project_id,'ttl_ms':90_000})
                check(command+'_fresh_fence',status==200 and lease['fencing_epoch']>previous_fence)
        ...
        stop_body={'project_id':owner.project_id,'command_id':'control.idle.stop'}
        began=time.monotonic()
        status,stopped,_=call('/v1/stop',stop_body,control=True)
        stop_ms=(time.monotonic()-began)*1000
        save(output/'idle-stop-initial.json',{'response':stopped,'response_ms':stop_ms})
        check('idle_stop_control_response_bounded',status==200 and stopped['stopped'] and stop_ms<500)
        deadline=time.monotonic()+25
        while stopped.get('stop_persistence')=='PENDING' and time.monotonic()<deadline:
            time.sleep(.05)
            status,stopped,_=call('/v1/stop',stop_body,control=True)
        save(output/'idle-stop-terminal.json',stopped)
        check('idle_stop_native_journal_barrier',status==200 and stopped.get('stop_persistence')=='DURABLE'
            and owner._journal.snapshot()['stopped'] is True)
        status,historical,wire=call('/v1/commands',commands[0][0].as_dict())
        check('stopped_owner_replays_original_edit_ack',status==200 and wire==commands[0][2])
        ...
        reopened = module._load('publication_journal_v5').PublicationJournalV5.reopen(storage_id,
            project_id='project.fixture')
        check('readonly_reopen_source_binding',reopened.snapshot()['source_closure_sha256']==closure)
        check('readonly_reopen_preserves_idle_stop',reopened.snapshot()['stopped'] is True)
