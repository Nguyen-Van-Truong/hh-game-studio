...
class GodotPublicationOwner:
    ...
    def create(cls, parent: Path, *, project_id: str, initial_bundle, editor_binary: Path,
        ...
        owner.fifo_model = _load('publication_fifo')
        owner._fifo = owner.fifo_model.PublicationFifo(owner.sessions)
    ...
    def discover(self, body, *, authorization, catalog_digest):
        grant=self._auth(body, 'scene.inspect', authorization, catalog_digest, {'project_id'})
        self._same_release()
        ...
        limits={'max_owner_commands':self.auth_model.MAX_COMMANDS,
            'max_editor_effects':self.editor_model.MAX_EFFECTS,
            'max_validation_attempts':self.validation_model.MAX_RUNS,
            'validation_attempts_reserved_for_save':1,'editor_effects_reserved_for_save':2}
        fifo=self._writer_fifo().snapshot()
        limits.update(max_writer_waiters=fifo['max_waiters'],max_writer_tickets=fifo['max_tickets'],
            max_writer_wait_ms=fifo['max_wait_ms'],writer_waiters=fifo['queued'],
            writer_tickets_remaining=max(0,fifo['max_tickets']-fifo['retained']))
        if limits['writer_tickets_remaining']==0:
            # Retained ticket outcomes consume lifetime capacity. An existing
            # exact lease may still finish/save; no caller can acquire a new one.
            with self._lock:lease=self._leases.get(grant.session_id)
            current=frozenset()
            operations=enabled & self.auth_model.WRITE_OPERATIONS & frozenset(grant.operations)
            if lease is not None and operations:
                try:
                    self.sessions.check(grant,lease,deadline_ms=lease.expires_ms,operation=sorted(operations)[0])
                    current=operations
                except SafetyViolation:pass
            enabled=(enabled & self.auth_model.READ_OPERATIONS)|current
        # Do not wait for native work to advertise scheduling state. Capacity
        # is an observation; admission always rechecks it under the work gate.
        acquired=self._work.acquire(blocking=False)
        limits['owner_busy']=0 if acquired else 1
        if acquired:
            try:
                _need(not self._closed and self._editor is not None and self._validator is not None,
                      'GODOT_OWNER_CLOSED')
                available=self.validation_model.MAX_RUNS-len(self._validator.snapshot()['attempts'])
                slots=self.editor_model.MAX_EFFECTS-len(self._editor._intents)
                commands=self.auth_model.MAX_COMMANDS-len(self._jobs)
                limits.update(validation_attempts_remaining=max(0,available),
                    editor_effects_remaining=max(0,slots),owner_commands_remaining=max(0,commands))
                if available<=0 or slots<2 or commands<=0:
                    enabled=enabled & self.auth_model.READ_OPERATIONS
                elif slots<4:
                    enabled=enabled-self.auth_model.EDIT_OPERATIONS
            finally:self._work.release()
        else:
            enabled=frozenset()
        discovery=self.contract.discovery(self.project_id,implemented=implemented,runtime_enabled=enabled)
        return replace(discovery,limits={**discovery.limits,**limits})
    ...
    def lease(self, body, *, authorization, catalog_digest):
        ...
        fifo=self._writer_fifo()
        ticket=fifo.enqueue(grant,request_id='legacy.'+uuid.uuid4().hex,
            ttl_ms=body['ttl_ms'],wait_ms=90000)
        if ticket['state']!='GRANTED':
            fifo.cancel(grant,ticket['ticket_id'])
            raise PublicationOwnerError('GODOT_LEASE_BUSY')
        self._register_ticket(grant,ticket)
        return ticket['lease']

    def _writer_fifo(self):
        with self._lock:
            if getattr(self,'_fifo',None) is None:
                self.fifo_model=_load('publication_fifo')
                self._fifo=self.fifo_model.PublicationFifo(self.sessions)
            return self._fifo

    def _fifo_auth(self,body,authorization,catalog_digest,keys):
        _need(type(body) is dict and set(body)==keys,'GODOT_INVALID_ENVELOPE')
        grant=self.sessions.authenticate(authorization)
        operations=set(grant.operations) & (_load('publication_session').WRITE_OPERATIONS-{'project.reconcile'})
        _need(bool(operations),'GODOT_OPERATION_FORBIDDEN')
        self._auth(body,sorted(operations)[0],authorization,catalog_digest,keys)
        return grant

    def _register_ticket(self,grant,ticket):
        if ticket['state']=='GRANTED':
            try:lease=self._writer_fifo().granted_lease(grant,ticket['ticket_id'])
            except SafetyViolation as error:
                # Exact historical ticket replay is valid after lease expiry or
                # Stop; it cannot restore authority for a new mutation.
                if error.code not in ('GODOT_STALE_LEASE','GODOT_DEADLINE_EXPIRED','GODOT_STOPPED'):
                    raise
            else:
                with self._lock:self._leases[grant.session_id]=lease
        return ticket

    def lease_enqueue(self,body,*,authorization,catalog_digest):
        grant=self._fifo_auth(body,authorization,catalog_digest,{'project_id','request_id','ttl_ms','wait_ms'})
        with self._lock:_need(not self._held,'GODOT_OWNER_HELD')
        return self._register_ticket(grant,self._writer_fifo().enqueue(grant,
            request_id=body['request_id'],ttl_ms=body['ttl_ms'],wait_ms=body['wait_ms']))

    def lease_poll(self,body,*,authorization,catalog_digest):
        grant=self._fifo_auth(body,authorization,catalog_digest,{'project_id','ticket_id'})
        return self._register_ticket(grant,self._writer_fifo().poll(grant,body['ticket_id']))

    def lease_cancel(self,body,*,authorization,catalog_digest):
        grant=self._fifo_auth(body,authorization,catalog_digest,{'project_id','ticket_id'})
        return self._register_ticket(grant,self._writer_fifo().cancel(grant,body['ticket_id']))
    ...
    def stop(self, body, *, authorization, catalog_digest):
        ...
        fifo=getattr(self,'_fifo',None)
        if fifo is not None:fifo.halt()
        with self._lock:
            _need(not self._closed,'GODOT_OWNER_CLOSED')
