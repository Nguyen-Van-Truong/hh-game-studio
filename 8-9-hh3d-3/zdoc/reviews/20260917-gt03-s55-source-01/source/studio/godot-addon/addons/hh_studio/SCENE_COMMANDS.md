...
Godot native vector/matrix and packed-array values use typed component arrays
inside the JCS state; nonfinite components are rejected and signed zero follows
JCS normalization. Resource path and
...
maximum depth 16 including the root, inspection limit 1..64 and operation
canonical cap 8192.

The supported scene consists of those two exact native node classes and
local, unshared BoxMesh resources. Mesh resource paths must be empty or an
embedded subresource of the fixed fixture scene. External mesh files and
sharing one mesh instance across nodes are rejected: their link/alias semantics
are outside this finite state model. Generated subresource IDs remain excluded
from revision so packing and reopening do not manufacture scene changes.
No arbitrary Resource, node class, script path, property
...
manually replaced object. Reattachment resolves the retained parent against the
current stable-ID map and requires the recorded owner to remain the current
root before the first effect. All mutations are synchronous and use fixed native
properties; there is no await/yield between guard and effect.
The first do callback also verifies requested target values, actual attach or
detach and ownership, and exact preservation of every other row (allowing only
the expected sibling-index shift). A later undo/redo must additionally restore
the recorded exact before/after revision. Unexpected readback holds the adapter
with `SCENE_POSTCONDITION_UNKNOWN`; it is never reported as an effect-free reject.

Transforms/names and original/new mesh resources are retained. Mesh updates
operate on a duplicated BoxMesh. Both resource snapshots are hashed, checked
for local resource scope, and checked against other live node meshes before
applying them later. Node remove retains its actual node,
parent, sibling index and scene owner. Restore sets owner after reattachment.
Detached-node signatures include groups and reject persistent connections,
queued deletion, and stored-value changes before an off-tree node is restored.
Restoration also rejects a retained mesh now used by another live node.
