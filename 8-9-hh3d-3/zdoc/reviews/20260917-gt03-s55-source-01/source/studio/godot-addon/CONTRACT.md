...
`operations.json` catalog `gt03-godot-5` defines nine operations. Its static `implemented` and
...
Discovery build `2.0` binds this declaration through the shared `schema_digest`
field. The declaration names the separate scene/project inspection revisions;
the default discovery still enables no runtime operation.
...
observed complete semantic scene revision. Every non-inspect payload also
requires `expected_project_revision` in the same SHA-256 format, matching the
trusted revision of the complete selected scene+script bundle. This host CAS
also applies to both the outer preview and its nested mutation. A script-only
bundle replacement invalidates stale mutation/save admission even if its
semantic scene hash is unchanged. Dirty in-memory node changes alter the scene
revision while the selected base project revision stays unchanged until publish.
Save checks both revisions and the existing raw `expected_files` hashes.

`expected_generation` is an integer reload epoch in 1–2147483647 in every
payload; it must match the trusted observation. It is neither a project version
nor a persistent engine-session identity. A host must separately fence restarted
sessions and publication generations. The addon payload contract changed; the
shared wire envelope/schema and token handling are unchanged.

`ValidationContext` is a frozen trusted-call structure containing project,
semantic `revision`, required `project_revision`, generation, lease ID/fence/expiry, current epoch milliseconds,
...
The project revision is supplied by the trusted bundle/publication owner; it
must not be inferred from the editable node subset, native scene hash, or two
caller-provided hashes. The bundle codec computes the complete candidate
revision, but does not prove that candidate has been selected or read back.
...
New admission requires matching project, scene revision, project revision for
non-inspect commands, generation, lease and
fence, with `now < deadline <= min(lease expiry, now + 30000ms)`; complete scene
save and script replacement allow 90000ms. Stop permits inspect/preview reads.
The host must authenticate reads too; inspection does not
imply a write scope. A successful validation is neither an ACK nor permission
to call an arbitrary native/API method.

Inspection needs no project CAS input: its host validation report (the
`ValidatedCommand.preview` metadata) exposes `scene_revision` and
`project_revision` from the trusted context distinctly. Every dry-run report
contains these two observations. The existing native engine inspect result
only knows the semantic scene revision; the managed host inspection backend
combines it with the independently verified selected-bundle observation.
These pure reports and catalog declarations do not enable that runtime backend.

The request digest includes the project precondition because it is a payload
field. Changing it on a same-ID request changes that digest. The managed dispatcher
retains the originally admitted scene/project/generation observations and
look up the original receipt before new admission. Same-ID exact retry returns
that historical outcome without rerunning an effect; a changed payload conflicts.
Refreshing only the envelope scene revision/lease/deadline does not create a new
command because those fields are outside the shared operation digest. This
validator has no receipt store and intentionally rejects stale new admission;
it does not implement or prove retry/recovery behavior.

## Exact operation shapes

All rows include `expected_generation` in their payload. All rows except
`scene.inspect` also require `expected_project_revision`; this includes both
levels of `scene.preview`. The stable root ID is
...
not wire targets.
Local command IDs use the same lowercase, 64-character pattern as stable IDs,
which is deliberately narrower than the shared envelope's general ID syntax.
Sibling-name comparisons reject ASCII case aliases as the engine does.
...
| `scene.preview` | `{"stable_id":"root"}` | `operation`, `target`, `payload` for one node create/update/remove or declared script replacement; nested generation and project revision must agree with the trusted context |
...
save, undo or redo. Managed script preview supplies a bounded actual text diff
for the closed declarative grammar. Preview checks the same proposed mutation against current context;
...
After validation, `expected_project_revision` is removed from the outer and
nested projection payloads. It remains bound by the original request digest
and host report; it is never presented as engine authority or a node property.
...
the exact request digest and observed scene/project revisions and generation. Node diffs contain
...
`context_from_snapshot(snapshot, *, project_revision=...)` requires an explicit
trusted bundle revision. `generate_vectors(actual_snapshot,
project_revision=..., file_hashes=...)` rejects missing/invalid project revisions;
the runner supplies the actual bundle revision independently of the engine
snapshot. No-argument vector generation is still `SYNTHETIC_TEST_ONLY`, including
a tagged synthetic project-revision placeholder. Vector schema
`hh-gt03-contract-vectors-2` and the catalog digest identify this changed contract.
S47 process evidence belongs to its frozen source and does not verify S48 changes.
