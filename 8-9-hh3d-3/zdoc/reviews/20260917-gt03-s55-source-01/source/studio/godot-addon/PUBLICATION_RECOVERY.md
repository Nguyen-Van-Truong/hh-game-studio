...
Command inspection folds and verifies the native stream once under the journal
lock, deriving snapshot, command, selected bundle and authority context from
that same observed head. It returns detached values and retains no trusted view
between calls. The facade still requires the exact native journal owner; caller
dictionaries cannot supply this observation. Every authority phase, append
readback, selected-file barrier and live editor check remains independently
verified. This removes repeated full folds within one observation without
changing the 30-second deadline or caching across effects.
