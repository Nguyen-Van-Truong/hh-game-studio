...
PACK = ROOT / 'zdoc/reviews/20260916-gt02-s38-01'
STUDIO = ROOT / 'studio'
DIAGNOSTIC_MANIFEST_HASH = 'c6276ad32aada15550f6cae75abdafd34d70c70b4df8032fb93fdff584077a63'
...
def verify_cuts(output):
    rows = [json.loads(line.removeprefix('HH_GT02_SELECTOR_CUT '))
            for line in output.split('\n') if line.startswith('HH_GT02_SELECTOR_CUT ')]
    expected = {'after_INTENT':('INTENT',0), 'after_STAGING':('STAGING',0),
    ...
    assert len(rows) == len(expected) and {r['cut'] for r in rows} == set(expected)
    for row in rows:
        phase, count = expected[row['cut']]
        assert type(row['host_pid']) is int and row['host_pid'] > 0 and row['host_exit'] == 81
        assert row['armed_marker'] == {'marker':'SELECTOR_CUT_ARMED','cut':row['cut'],'pid':row['host_pid']}
        assert row['phase_on_reopen'] == phase and row['blob_count_on_reopen'] == count
        if row['cut'] == 'after_STAGING':
            assert row['generation_after_reconcile'] == 0 and row['blob_count_after_reconcile'] == 0
            assert row['ready'] is False and row['receipt']['status'] == 'CANCELED'
            assert row['receipt']['postconditions'] == {'selection_effect':False,'staging_may_exist':True}
        elif phase == 'RESTORING':
            assert row['generation_after_reconcile'] == 2 and row['blob_count_after_reconcile'] == 3
            assert row['ready'] is True and row['stopped'] is True
            assert row['receipt']['status'] == 'UNKNOWN' and row['receipt']['postconditions']['restored'] is True
        else:
            assert row['generation_after_reconcile'] == 1 and row['blob_count_after_reconcile'] == 3
            assert row['ready'] is True and row['stopped'] is False and row['receipt']['status'] == 'COMMITTED'
            assert row['receipt']['postconditions']['generation'] == 1
        assert row['receipt']['command_id'] == 'cmd-one'
    return rows
...
def verify():
    ...
    diagnostic_counts = []
    for number, expected in (('01',17), ('02',21), ('03',21), ('04',24), ('05',27)):
        folder = ROOT / ('zdoc/reviews/20260916-gt02-s38-selector-' + number)
        ...
        actual_host = read(folder / host['host'])
        assert actual_host['exit_code'] == 0 and actual_host['target_pid'] == host['target_pid']
        assert actual_host['started_at']
        for name, digest in invocation['source_hashes'].items():
            assert sha(folder / Path(name).name) == digest
            if number == '05':
                assert sha(STUDIO / name) == digest
        assert sha(STUDIO / 'build/bootstrap/run_fixture.py') == invocation['runner_sha256']
        output = (folder / host['stdout']).read_text(encoding='utf-8')
        markers = [json.loads(line.removeprefix('SELECTOR_TEST_COMPLETE '))
                   for line in output.split('\n') if line.startswith('SELECTOR_TEST_COMPLETE ')]
        assert markers == [{'run':expected,'failures':0,'errors':0,'skips':0}]
        cuts = verify_cuts(output) if number in ('03','04','05') else []
        diagnostic_counts.append({'run':number, **markers[0], 'recorded_process_cuts':len(cuts)})
    ...
    mapping['S38-FIXTURE-SELECTOR'] = ['intent_precedes_staging_and_selection_precedes_readback_receipt', 'duplicate_after_deadline_and_lease_change_returns_original_receipt', 'same_id_changed_payload_and_wrong_project_never_reuse_receipt', 'invalid_request_has_no_journal_or_blob_effect', 'two_same_parent_commands_admit_one_then_reject_stale_parent', 'late_revision_and_fence_changes_refuse_selection', 'reopen_requires_explicit_reconcile_and_does_not_restage', 'stop_cancels_preselection_and_stays_stopped_after_reopen', 'selected_stop_requires_restore_preserving_monotonic_generation', 'restore_refuses_new_owner_revision_without_overwrite', 'lost_terminal_reply_replays_receipt_and_never_adopts_twice', 'consumer_changed_before_failed_terminal_requires_readback_reconcile', 'false_consumer_readback_never_commits_or_marks_ready', 'expired_adoption_needs_fresh_recovery_lease', 'partial_staging_is_cancel_only_and_files_are_preserved', 'whole_operation_capacity_reserved_before_intent', 'fresh_runtime_load_is_explicit_and_does_not_reselect_or_restage', 'real_process_cuts_preserve_phase_and_resume_without_duplicate_files', 'staged_discovery_rejects_ambiguous_manifest_without_creating_files', 'valid_checksum_cannot_authorize_semantically_invalid_event', 'stop_signal_blocks_selection_while_waiting_for_selector_lock', 'blob_slots_are_reserved_before_intent_and_failure_does_not_poison', 'two_selectors_cannot_reserve_the_same_store_and_failed_init_unbinds', 'explicit_recovery_stage_and_selection_bind_fresh_fence_in_history', 'consumer_cannot_be_shared_by_two_independent_selectors', 'stop_during_quota_scan_prevents_intent_or_staging', 'stop_arriving_during_pin_prevents_later_selection_and_adoption']
    selector_cuts = verify_cuts((PACK / 'protocol-stdout.txt').read_text(encoding='utf-8'))
    coverage = []
    ...
    native['scope'] = 'S35 native counter source subset unchanged; S36 events, S37 release graph and S38 selector are not exercised by that native run'
    ...
    return {'schema': 'hh-gt02-coordinator-verification-v1', 'status': 'COORDINATOR_LOGIC_VERIFIED',
            ...
            'selector_process_cuts': selector_cuts, 'focused_diagnostics': diagnostic_counts, 'diagnostic_artifacts': len(diagnostics['files']),
            ...
            'limits': ['P-02/P-21 serializer conformance; Python host owns raw wire admission',
                       ...
                       'S36 storage and S38 typed selector are internal; the new transaction has no confined-client integration proof',
                       'S37/S38 completeness and adoption cover an inert JSON fixture, not engine resources; clocks/revisions are trusted observations',
