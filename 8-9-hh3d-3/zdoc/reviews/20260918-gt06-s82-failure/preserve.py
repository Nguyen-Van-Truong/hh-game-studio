...
for index, sample in enumerate(samples):
    ...
    evidence = dict(profile_sha256=PROFILE, source_closure_sha256=EXPECTED, run_id=context['run_id'],
                    index=index, processes=sample['processes'],
                    artifacts={name: ref for name, ref in capture.items() if name != 'index'},
                    barrier_receipt=joint['barrier_receipt'])
    evidence_hash = hashlib.sha256(json.dumps(evidence, sort_keys=True, separators=(',', ':'), allow_nan=False).encode('utf-8')).hexdigest()
...
write('completeness-and-screen.json', dict(created_utc=utc(), formal_acceptance=False,
    ...
    sample_evidence_hashes_verified=16,
    sample_evidence_hash_domain='SHA256(UTF8(json.dumps(evidence, sort_keys=True, separators=(comma,colon), allow_nan=False))); evidence binds source/profile/run/index/processes, all six exact artifact refs and barrier_receipt; distinct from raw batch-capture file hash'))
