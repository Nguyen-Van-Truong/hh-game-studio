...
class UIAdapter(base.FixtureAdapter):
    def __init__(self, *, owned_root=None, external_poll=False):
        ...
        self.queue = queue_module.CommandQueue(self._dispatch, preview=self._preview)
    ...
    def _save_preflight(self, command):
    ...
    def _edit_history_segment(self, before):
    ...
    def _history_target(self, operation, before):
    ...
    def _preview(self, command):
        """Observe validated intent without an operator, mode change or history write."""
        self._guard()
        if self._stopped:
            raise c.Rejected("STOPPED")
        command = queue_module.parse(c.canonical(command))
        operation = command["operation"]
        if operation not in queue_module.PREVIEW_OPERATIONS:
            raise c.Rejected("PREVIEW_OPERATION_UNSUPPORTED")
        before = self.inspect()
        self._check_preconditions(command, before)
        target = None
        files = operation in ('checkpoint.save', 'export.prepare')
        if files:
            # Publication preview requires the complete bounded export profile;
            # neither this inspection nor slot inspection creates a file.
            self._export_preflight()
            self._save_preflight(command)
        elif operation in ("history.undo", "history.redo"):
            _, target = self._history_target(operation, before)
        else:
            self._preflight_edit(command, before)
            self._edit_history_segment(before)
        return {
            "schema": queue_module.PREVIEW_SCHEMA,
            "command_id": command["command_id"], "command_digest": c.digest(command), "operation": operation,
            "before": before, "requested_changes": command["payload"], "history_target": target,
            "affected_files": [command['payload']['slot']+'.blend'] if files else [],
            "undo_policy": "immutable-private-copy-no-file-undo" if files else "native-revision-checked-session-history",
            "value_policy": "native-preflight-only; artifact-hashes-require-publication" if files else "requested-values; apply-readback-uses-native-binary32",
    ...
    def _dispatch_checked(self, command):
        ...
        with self._view():
            ...
            self._preflight_edit(command, before)
            new_segment = self._edit_history_segment(before)
    ...
    def _history_step(self, operation, before):
        undo, target = self._history_target(operation, before)
        try:
            if before["context"]["mode"] == "EDIT_MESH":
                self._mode("OBJECT")
