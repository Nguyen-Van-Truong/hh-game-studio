...
class PrivateEventLog:
    ...
    def read(self, sequence: int) -> EventRecord:
        ...
        with self._locked():
            ...
            try:
                ...
                if self._inspect().size != self._index[-1][2].size:
                    raise EventLogError('EVENT_HISTORY_CHANGED')
                self._witness = self._index[-1][2]
    ...
    def fold(self, initial, reducer):
        """Internal pure reducer over one validated, guarded history snapshot.

        The callable is trusted broker code, never wire data. It must not call
        back into this log or perform effects. Only one event body is held by
        this iterator; the reducer decides its bounded projection/index shape.
        """
        with self._locked():
            self._scan()
            state, previous = initial, _ZERO
            try:
                for offset, size, head in self._index:
                    body = self._read_at(offset + 4, size)
                    event = self._decode(body, bytes.fromhex(head.sha256), head.sequence, previous)
                    state = reducer(state, EventRecord(head, canonical_json(event)))
