...
assemble_dataset(provenance, runs) applies the unchanged profile v2 validator.
...
def assemble_run(root, manifest):
    """Assemble a parent-finalized run; its host exit must already be captured.
...
"""
    ...
    for name in required:
        ...
        read_artifact(root, {'file': 'project/' + name, 'sha256': native_index['source_files']['res://' + name], 'size_bytes': path.stat().st_size})
    ...
    return {'run_id': manifest['run_id'], 'index': manifest['index'], 'processes': manifest['processes'],
...
def assemble_dataset(provenance, runs):
    """Require all ten independently captured runs; never promote diagnostics."""
    return profile.validate_dataset({'schema_id': 'hh-studio.tools-ux-benchmark', 'schema_version': '1.1.0',
        'profile_sha256': profile.PROFILE_SHA256, 'evidence_kind': 'native', 'provenance': provenance, 'runs': runs})
