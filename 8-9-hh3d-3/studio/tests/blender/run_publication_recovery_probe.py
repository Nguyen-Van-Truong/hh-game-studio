"""Native crash/Stop cuts for the unchanged publication03 runtime.
...
RUNTIME_SHA256='acd74ca7a166e08e7270f0f97d352d75404acfb7ae4ae987916230f523f8e717'
CRASH_CASES=('after_intent','before_selector','after_selector','before_terminal_witness','after_terminal_witness')
STOP_CASES=('stop_export','stop_before_selector','stop_after_selector')
...
def setup(output,binary):
    ...
    from studio.protocol.core import canonical_bytes
    need(model.sha(canonical_bytes(source_files()))==RUNTIME_SHA256,'publication03 runtime changed')
...
def reopen_proof(output,ready):
    ...
    try:
        try:owner=BlenderPublicationOwner.reopen(ready['storage_id'])
            ...
            committed=case in ('after_terminal_witness','stop_after_selector')
...
def stop_case(output,binary,case):
    ...
    try:
        ...
        def append_hook(kind,**fields):
            result=append(kind,**fields)
            if case=='stop_before_selector' and kind=='STAGED':gate()
            return result
        def create_hook(name,raw):
            result=create(name,raw)
        ...
        raw=publisher.lookup_bytes('publish');response=json.loads(raw);committed=case=='stop_after_selector'
        ...
        image=image_state(publisher);need(('active.json' in image['artifacts']) is committed,'Stop selector cut')
        if case=='stop_export':
