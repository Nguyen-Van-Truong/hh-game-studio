...
func _should_skip(path: String) -> bool:
	...
	if p.contains("/r6w6/") or p.contains("r6w6/"):
		return true
	if p.contains("/r6w7/") or p.contains("r6w7/"):
		return true
	if p.contains("/r7w1/") or p.contains("r7w1/"):
		return true
	if p.contains("/r7w2/") or p.contains("r7w2/"):
		return true
	if p.contains("/r7w3/") or p.contains("r7w3/"):
		return true
	if p.contains("/r7w4/") or p.contains("r7w4/"):
		return true
	if p.contains("/r7w5/") or p.contains("r7w5/"):
		return true
	return false
