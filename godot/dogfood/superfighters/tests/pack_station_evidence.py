#!/usr/bin/env python3
"""Assemble VF5-WP4 §18.3 evidence after official Signal Court runs.
...
def _still_ok(row: dict, zones: set[str] | None = None) -> bool:
    ok = (
        bool(row.get("alive", False))
        and bool(row.get("on_floor", False))
        and not bool(row.get("lose_visible", True))
    )
...
def main() -> int:
    ...
    machine = outcome_verdict(outcomes, "machine")
    floor_v = outcome_verdict(outcomes, "floor")
    ...
    floor_row = outcomes.get("floor", {}) if isinstance(outcomes.get("floor"), dict) else {}
    court_still = outcomes.get("court_still", {}) if isinstance(outcomes.get("court_still"), dict) else {}
    floor1_still = outcomes.get("floor1_still", {}) if isinstance(outcomes.get("floor1_still"), dict) else {}
    floor2_still = outcomes.get("floor2_still", {}) if isinstance(outcomes.get("floor2_still"), dict) else {}
    floor3_still = outcomes.get("floor3_still", {}) if isinstance(outcomes.get("floor3_still"), dict) else {}
    machine_still = outcomes.get("machine_still", {}) if isinstance(outcomes.get("machine_still"), dict) else {}
    live_real = str(live_row.get("hud", "")) == "Signal Court"
    machine_real = bool(machine_row.get("jammed", False)) and int(machine_row.get("jams", 0)) >= 1
    ...
    east_top = bool(zone_row.get("east_top_live", False) or p1_hits.get("east_top", False))
    stills_live = (
        _still_ok(court_still, {"court_mid", "court_low", "court_ground"})
        and _still_ok(floor1_still, {"west_hall", "east_hall"})
        and _still_ok(floor2_still, {"west_loft", "sky_bridge", "east_mid"})
        and _still_ok(floor3_still, {"east_top"})
        and _still_ok(machine_still, {"court_ground", "court_low", "west_hall"})
    )
    live_reach_ok = p1_all and climb_ok and east_top and stills_live
    ...
    events_ok = bool({"stat_name", "stat_zone", "stat_p1", "stat_machine", "stat_floor"} <= event_kinds)
    ...
    court_png = pick_still(screens, "court")
    floor1_png = pick_still(screens, "floor1")
    floor2_png = pick_still(screens, "floor2")
    floor3_png = pick_still(screens, "floor3")
    machine_png = pick_still(screens, "machine")
    stills_ok, still_hashes, still_errors = stills_pairwise_distinct(
        [p for p in (setup_png, title_png, court_png, floor1_png, floor2_png, floor3_png, machine_png) if p is not None]
    )
    if None in (setup_png, title_png, court_png, floor1_png, floor2_png, floor3_png, machine_png):
        stills_ok = False
        still_errors.append("need setup/title/court/floor1/floor2/floor3/machine window stills")
    verdict = (
        "PASS"
        if exits_ok
        and outcomes_ok
    ...
    for label, src in (
        ("run_station.headless.log", Path(args.headless_log)),
        ("run_station.window.log", Path(args.window_log)),
        ("run_all.headless.log", Path(args.run_all_log)),
        ("check_station.log", Path(args.check_log)),
    ):
        if src.is_file():
    ...
    run = {
        ...
        "verify": {
            "official": [
                ...
                "%LOCALAPPDATA%\\HHGodotAgent\\tooling\\godot-4.7.1-stable\\bin\\Godot_v4.7.1-stable_win64_console.exe --headless --path <worktree>/godot/dogfood/superfighters --script res://tests/run_station.gd",
                "%LOCALAPPDATA%\\HHGodotAgent\\tooling\\godot-4.7.1-stable\\bin\\Godot_v4.7.1-stable_win64.exe --path <worktree>/godot/dogfood/superfighters --script res://tests/run_station.gd",
    ...
    verdict_md = f"""# VF5-WP4 verdict

{verdict} Signal Court station-like arena (V-A18 / §18.3).
...
- NAME={name_v} GRAPH={graph} MACHINE={machine} FLOOR={floor_v} SPAWN={spawn} COVER={cover} DOOR={door} CAMERA={camera} P1={p1} P2={p2} BOT={bot} ZONE={zone} LIVE={live} REPLAY={replay}
- LIVE hud={live_row.get("hud")} real={live_real}
- P1 hits={p1_row.get("hits")} P2 hits={p2_row.get("hits")} BOT hits={bot_row.get("hits")}
- live_reach p1_all={p1_all} climb_up_on_ladder={climb_ok} east_top={east_top}
- court_still={court_still} floor1={floor1_still} floor2={floor2_still} floor3={floor3_still} machine_still={machine_still}
- MACHINE jammed={machine_row.get("jammed")} jams={machine_row.get("jams")} real={machine_real}
- FLOOR interior={floor_row.get("interior_solid")} floors={floor_row.get("floors")}
- events kinds include name/zone/p1/machine/floor={events_ok} kinds={sorted(event_kinds)}
...
"""
    ...
    print(
        f"  NAME={name_v} GRAPH={graph} MACHINE={machine} FLOOR={floor_v} "
        f"SPAWN={spawn} COVER={cover} DOOR={door} CAMERA={camera} "
    ...
    print(
        ...
        f"east_top={east_top} machine_real={machine_real}"
    )
