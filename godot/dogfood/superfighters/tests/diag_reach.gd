...
func _boot() -> void:
	...
	root.add_child(app)
	var ids: PackedStringArray = PackedStringArray(["rooftops", "hazardous", "gauge", "storage", "police", "lantern"])
	...
	while i < ids.size():
		...
		print(
			"HH_DIAG MAP id=%s reach=%s reason=%s moved=%.1f toward=%.1f goal=%.1f wp=%.1f engage=%.1f closest=%.1f gun=%s melee=%s pit_blocks=%s pit_reroutes=%s dead=%s"
			% [
				mid,
