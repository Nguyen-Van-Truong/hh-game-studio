#!/usr/bin/env python3
"""Assemble VF6-WP3 evidence after official stage runs.
...
def _evidence_dir_value(log_text: str) -> str:
    prefix = "HH_VF_STAGE EVIDENCE_DIR="
    for line in log_text.splitlines():
...
def parse_exit_from_log(text: str) -> int | None:
    ...
    if any(token in lowered for token in ("warning:", "error:", "script warning", "parse error")):
        return None
    if "FAIL: Vault Fighters stage progression" in text:
        return 1
    if "HH_VF_STAGE FINISHED=1" in text and "HH_VF_STAGE PROCESS_EXIT=0" in text:
        return 0
    if "HH_VF_STAGE FINISHED=1" in text and "HH_VF_STAGE PROCESS_EXIT=1" in text:
        return 1
    return None
...
def main() -> int:
    ...
    keys = ("schema", "load", "advance", "loss", "hash", "continue", "reset", "live")
    verdicts = {key: outcome_verdict(outcomes, key) for key in keys}
    apply_info = outcomes.get("apply", {})
    advance_row = outcomes.get("advance", {}) if isinstance(outcomes.get("advance"), dict) else {}
    loss_row = outcomes.get("loss", {}) if isinstance(outcomes.get("loss"), dict) else {}
    hash_row = outcomes.get("hash", {}) if isinstance(outcomes.get("hash"), dict) else {}
    load_row = outcomes.get("load", {}) if isinstance(outcomes.get("load"), dict) else {}
    continue_row = outcomes.get("continue", {}) if isinstance(outcomes.get("continue"), dict) else {}
    ...
    live_logs_ok = (
        ...
        and int(_field(headless_log_text, "HH_VF_STAGE ", "USED_STEP_FIXED") or "1") == 0
        and int(_field(window_log_text, "HH_VF_STAGE ", "USED_STEP_FIXED") or "1") == 0
        and int(_field(headless_log_text, "HH_VF_STAGE ", "USED_FORCE_KILL") or "1") == 0
        and int(_field(window_log_text, "HH_VF_STAGE ", "USED_FORCE_KILL") or "1") == 0
    )
    leftover_ok = (
        leftover_proof
    ...
    rows = load_row.get("rows", [])
    load_ok = (
        isinstance(rows, list)
        and len(rows) == 4
        and [str(r.get("map_id", "")) for r in rows] == ["rooftops", "storage", "police", "hazardous"]
        and all(int(r.get("bots", -1)) == int(r.get("want_bots", -2)) for r in rows)
    )
    ...
    win_rows = advance_row.get("win_rows", []) if isinstance(advance_row.get("win_rows"), list) else []
    win_maps = [str(r.get("fight_map", "")) for r in win_rows if isinstance(r, dict)]
    events_ok = (
        events_path.is_file()
        and events_path.stat().st_size > 0
        and "death" in events_text
        and isinstance(timeline_rows, list)
        and len(timeline_rows) >= 8
    )
    catalog_wins_ok = (
        "rooftops" in win_maps
    ...
    ux_ok = (
        ...
        and str(advance_row.get("after_win1_map", "")) == "police"
        and str(advance_row.get("after_win2_map", "")) == "hazardous"
        and str(advance_row.get("rematch_map", "")) == "police"
        and str(loss_row.get("stayed_map", "")) == "rooftops"
        and str(loss_row.get("death_cause", "")) == "damage"
        and int(loss_row.get("pit_loss", 1)) == 0
        and str(hash_row.get("hash_win0", "")) != ""
        and str(hash_row.get("hash_win1", "")) != ""
        and str(hash_row.get("hash_win0", "")) != str(hash_row.get("hash_win1", ""))
        and str(advance_row.get("hash_after_loss", "")) == str(advance_row.get("hash_win1", ""))
        and str(continue_row.get("map_id", "")) == "hazardous"
        and bool(continue_row.get("cold", False))
        and continue_row.get("map_id", "") != "rooftops"
    ...
    packed = {
        "schema": "vault-fighters.vf6-wp3.pack.v1",
        "run_id": RUN_ID,
    ...
    verdict = (
        ...
        f"ORDER rooftops→storage→police→hazardous (approximation)\n"
        f"ADVANCE {advance_row.get('after_win0_map')}→{advance_row.get('after_win1_map')}→"
        f"{advance_row.get('after_win2_map')} REMATCH={advance_row.get('rematch_map')}\n"
        f"LOSS stayed={loss_row.get('stayed_map')} cause={loss_row.get('death_cause')} "
        f"HASH win0!=win1 "
        f"loss_stable={advance_row.get('hash_after_loss') == advance_row.get('hash_win1')}\n"
    ...
    if not verdict_ok:
        print("FAIL: VF6-WP3 packer rejected official package")
        ...
        return 1
    print("PASS: VF6-WP3 evidence packed")
