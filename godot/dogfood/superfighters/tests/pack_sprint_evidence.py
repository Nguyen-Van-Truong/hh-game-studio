#!/usr/bin/env python3
"""Assemble VF2-WP3 §18.3 evidence after official sprint/roll runs.
...
TRACE_FILES = (
    "tests/traces/sprint/double_tap_sprint.json",
    "tests/traces/sprint/tap_window_miss.json",
    "tests/traces/sprint/crouch_roll.json",
    "tests/traces/sprint/stamina_drain.json",
)
...
def main() -> int:
    ...
    tap = outcome_verdict(outcomes, "tap")
    ...
    run = {
        ...
        "verify": {
            "official": [
                ...
                "%LOCALAPPDATA%\\HHGodotAgent\\tooling\\godot-4.7.1-stable\\bin\\Godot_v4.7.1-stable_win64_console.exe --headless --path <worktree>/godot/dogfood/superfighters --script res://tests/run_sprint.gd",
                "%LOCALAPPDATA%\\HHGodotAgent\\tooling\\godot-4.7.1-stable\\bin\\Godot_v4.7.1-stable_win64.exe --path <worktree>/godot/dogfood/superfighters --script res://tests/run_sprint.gd",
            ...
            "sprint_headless_exit": args.headless_exit,
            "sprint_window_exit": args.window_exit,
    ...
    verdict_md = f"""# VF2-WP3 verdict

{verdict} sprint, stamina, and roll evidence (V-A18 / §18.3).
...
- TAP={tap} STAMINA={stamina} ROLL={roll} INVULN={invuln} DUP={dup} LIVE={live} REPLAY={replay}
...
"""
    ...
    for label, src in (
        ("run_sprint.headless.log", Path(args.headless_log)),
        ("run_sprint.window.log", Path(args.window_log)),
        ("run_all.log", Path(args.run_all_log)),
        ("check_sprint.log", Path(args.check_log)),
    ):
        if src.is_file():
    ...
    print(
        f"  TAP={tap} STAMINA={stamina} ROLL={roll} INVULN={invuln} "
        f"DUP={dup} LIVE={live} REPLAY={replay} APPLY={succeeded}/{attempted}"
    )
