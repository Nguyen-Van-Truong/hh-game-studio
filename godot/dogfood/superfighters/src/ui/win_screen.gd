...
func _ready() -> void:
	...
	bg.color = Color(0.07, 0.14, 0.14, 0.92)
	bg.mouse_filter = Control.MOUSE_FILTER_STOP
	root.add_child(bg)
	title_label = Label.new()
	title_label.name = "TitleLabel"
	title_label.text = "Last standing"
	title_label.position = Vector2(80, 200)
	title_label.size = Vector2(800, 48)
	title_label.add_theme_font_size_override("font_size", 32)
	title_label.add_theme_color_override("font_color", UiTheme.TEAL)
	root.add_child(title_label)
	sub_label = Label.new()
	sub_label.name = "Subtitle"
	sub_label.text = "You cleared the arena."
...
func show_win(headline: String, detail: String, token: int = -1) -> void:
