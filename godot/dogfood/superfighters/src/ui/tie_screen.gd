...
func _ready() -> void:
	...
	bg.color = Color(0.12, 0.11, 0.08, 0.92)
	...
	title_label.add_theme_color_override("font_color", UiTheme.BRASS)
	root.add_child(title_label)
	sub_label = Label.new()
	sub_label.name = "Subtitle"
	sub_label.text = "No one left standing."
...
func show_tie(headline: String, detail: String, token: int = -1) -> void:
