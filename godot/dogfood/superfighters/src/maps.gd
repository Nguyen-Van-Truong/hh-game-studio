...
static func _world() -> Dictionary:
	if _world_cache.is_empty():
		_world_cache = SimConstants.load_json(WORLD_PATH)
	return _world_cache


static func _moving() -> Dictionary:
	if _moving_cache.is_empty():
		_moving_cache = SimConstants.load_json(MOVING_PATH)
