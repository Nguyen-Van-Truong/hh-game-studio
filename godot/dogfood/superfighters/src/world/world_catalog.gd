...
## (assumption). Break stays schema-only. RL-NADE-PROP stays deferred.
...
static func validate_payload(row: Dictionary) -> PackedStringArray:
	...
	if bool(row.get("break_implemented", true)):
	...
	if bool(row.get("throw_implemented", true)):
		errors.append("dynamic throw must stay unimplemented this WP")
