...
$runId = "VF6WP5-20260904-ASIA-SAIGON-05"
...
function Invoke-Godot {
    ...
    ## Console twin often exits 1/empty while the worker child printed PASS.
    if ($code -ne 0 -and (Test-Path $LogPath)) {
        $txt = Get-Content -Raw -Path $LogPath -ErrorAction SilentlyContinue
        if (
            $txt -and
            (
                ($txt -match "PASS: Vault Fighters bot planner" -and $txt -match "HH_VF_BOTS FINISHED=1") -or
                ($txt -match "PASS: Vault Fighters first playable" -and $txt -match "HH_VF_ALL FINISHED=1")
            ) -and
            $txt -notmatch "HH_ASSERT_FAIL" -and
            $txt -notmatch "FAIL: Vault Fighters"
        ) {
            $code = 0
        }
    }
...
$proof = @{
    ...
    host                 = "System.Diagnostics.Process leftover-0 after worker PASS+FINISHED"
