...
$godot = $godotConsole
$runId = "VF6WP5-20260905-ASIA-SAIGON-10"
$commandId = "cmd.vf6-wp5.bots.10"
...
if ($packExit -ne 0) {
    ...
    if (Test-Path $verdictPath) {
        $forced = (Get-Content -Raw -Path $verdictPath) -replace "READY_FOR_CRITICS=yes", "READY_FOR_CRITICS=no"
        [System.IO.File]::WriteAllText($verdictPath, $forced)
    }
    throw "packer exit $packExit; READY forced no"
}
exit $packExit
